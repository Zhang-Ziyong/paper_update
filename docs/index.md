...
<thead><tr><th>日期</th><th>标题</th><th>摘要</th></tr></thead>
<tbody>
<tr><td>2026-06-04</td><td>S23DR 2026 Winning Solution<br><a href='http://arxiv.org/pdf/2606.06695'>论文</a></td><td>本文介绍了 S23DR 2026 结构化三维线框重建挑战赛的冠军方案,该方法基于稀疏 SfM 点云、拟合深度图和语义分割信息,从中重建建筑物的三维线框结构。作者将顶点建模为一个条件集合,通过流匹配(flow-matching)DiT 模型对 64 个顶点 token 进行去噪生成,并以 Perceiver 风格的场景 token 作为条件输入。整体流程采用全局预测加局部精化的两阶段设计,并辅以多样本一致性机制以稳定随机采样过程。最终系统在私有排行榜上取得 HSS = 0.654 的成绩,位列第一。

◆ 将三维线框顶点建模为条件集合,使用流匹配 DiT 对固定数量的顶点 token 进行去噪生成,提出了一种新颖的集合化生成式重建范式
◆ 采用 Perceiver 风格的场景 token 编码方式,将稀疏 SfM、深度和语义分割等多模态稀疏输入统一压缩为条件表示
◆ 设计两阶段由粗到精的预测流程:首先全局预测粗结构,然后基于凸包裁剪进行第二阶段精化,有效提升局部几何精度
◆ 引入轻量级多样本一致性投票机制,缓解流匹配随机采样器输出不稳定的问题
◆ 在 S23DR 2026 挑战赛中以 HSS = 0.654 的成绩获得私有排行榜第一名,验证了方案的有效性...[摘要不完整，待更新]</td></tr>
...
<thead><tr><th>日期</th><th>标题</th><th>摘要</th></tr></thead>
<tbody>
<tr><td>2026-06-03</td><td>Multi-Camera AR Guidance System for Surgical Instrument Handling and Assembly: Investigating Workload and Efficiency<br><a href='http://arxiv.org/pdf/2606.04992'>论文</a></td><td>这篇论文针对手术器械装配过程中刷手护士面临的高认知负荷问题,提出了一种基于多摄像头与增强现实的手术器械操作引导系统,并通过技术评估和用户研究验证了其在膝关节置换术模拟中的有效性,结果显示该系统显著降低了主观工作负荷,任务完成时间缩短了21.3%,尤其对器械不熟悉的护士帮助明显。

◆ 提出无需额外标记的多摄像头6D姿态估计方法,结合头戴显示器实现术中原位AR可视化引导

◆ 利用已知物体实现姿态估计和摄像头标定,避免了传统标记物对手术环境的干扰

◆ 6D姿态估计网络完全基于合成数据训练,提升了模型在真实手术场景中的泛化能力和适用性

◆ 集成了基于视线选择与脚踏板的多模态交互方式,使护士能够在保持无菌操作的前提下切换装配步骤

◆ AR引导提供工具尖端定位提示和分步装配动画,在技术指标上优于现有最先进的6D姿态估计方法

◆ 通过29名刷手护士参与的大规模临床模拟用户研究,系统验证了AR引导在主客观层面对术中器械装配性能的实际改进效果...[摘要不完整，待更新]</td></tr>
...
<tbody>
<tr><td><a href='https://github.com/hku-mars/FAST_LIO'>FAST_LIO</a></td><td>4762</td><td>A computationally efficient and robust LiDAR-inert</td></tr>
<tr><td><a href='https://github.com/hku-mars/FAST-LIVO2'>FAST-LIVO2</a></td><td>4161</td><td>FAST-LIVO2: Fast, Direct LiDAR-Inertial-Visual Odo</td></tr>
...
<tr><td><a href='https://github.com/hku-mars/LiDAR_IMU_Init'>LiDAR_IMU_Init</a></td><td>1415</td><td>[IROS2022] Robust Real-time LiDAR-inertial Initial</td></tr>
...
<tr><td><a href='https://github.com/hku-mars/Point-LIO'>Point-LIO</a></td><td>1231</td><td>Point-LIO</td></tr>
<tr><td><a href='https://github.com/hku-mars/SUPER'>SUPER</a></td><td>926</td><td>SUPER</td></tr>
<tr><td><a href='https://github.com/hku-mars/FAST-Calib'>FAST-Calib</a></td><td>917</td><td>A Handy Extrinsic Calibration Tool for LiDAR-camer</td></tr>
...
<tr><td><a href='https://github.com/ethz-asl/voxblox'>voxblox</a></td><td>1639</td><td>A library for flexible voxel-based mapping, mainly</td></tr>
