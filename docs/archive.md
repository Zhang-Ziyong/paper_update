...
<tr><td>2026-07-10</td><td>Glob3R: Global Structure-from-Motion with 3D Foundation Models<br><a href='http://arxiv.org/pdf/2607.09225'>论文</a> | <a href='https://junyuandeng.github.io/Glob3r'>代码</a></td><td>Glob3R是一个基于3D基础模型的全局SfM重建框架,针对现有前馈式重建方法精度不足、长序列处理易累积漂移的问题,提出将基础模型的几何预测与全局优化显式结合的方案。大量实验表明该方法在室内、室外、大规模驾驶和无序SfM基准上均能实现鲁棒且精确的重建,优于前馈基础模型基线和近期可扩展重建方法,精化后的位姿还能进一步提升神经渲染质量。

◆ 在冻结的Pi3X骨干上增加轻量密集匹配头,预测参考帧与邻帧间的图像形变,并转化为稀疏可靠的多视图特征轨迹,作为全局优化的对应约束
◆ 提出基于关键帧的滑动窗口关联策略,跨重叠窗口传播轨迹与相对位姿,实现可扩展的长序列或无序图像集重建
◆ 结合全局运动平均与光束法平差,显式优化前馈几何预测结果,降低尺度不一致并恢复稠密场景几何
◆ 验证了基础模型先验与全局几何优化互补的优势,精化后的位姿可显著提升下游神经渲染质量...[摘要不完整，待更新]</td></tr>
<tr><td>2026-07-10</td><td>Empirical Pedestrian Safety Assessment in a Mobile Robot Using a Predictive Social Force Model<br><a href='http://arxiv.org/pdf/2607.09192'>论文</a></td><td>本文针对移动机器人在人行道与行人共享空间的安全导航问题,提出将预测社会力向量在有限时间域上积分的预测社会力模型PSFM及其与PTTC结合的PTSFM,并在非完整约束移动机器人平台上系统对比了SFM、TSFM、PSFM和PTSFM四种变体。研究综合采用最小PTTC、平均速度、最小距离、横向距离和最大轨迹曲率等客观指标,以及Likert量表主观问卷对真实志愿者交互场景进行评估。实验结果表明,引入PTTC能显著提升客观安全指标,而预测机制的额外贡献有限且仅在部分子指标上偶尔显现,主观舒适度方面Mann-Whitney检验未发现显著差异,说明预测策略在单行人场景中收益有限。

◆ 提出PSFM和PTSFM,将预测社会力向量沿有限时间窗口积分,扩展传统社会力模型的预测能力
◆ 在真实非完整约束移动机器人上实现并系统对比SFM、TSFM、PSFM和PTSFM四种变体,填补了预测型SFM实证研究的空白
◆ 构建融合客观运动学指标与主观Likert问卷的综合安全评估框架
◆ 实证验证了PTTC集成对安全性的提升作用,并揭示预测机制在单行人场景中的有限增益...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-06-22</td><td>G-MASt3R-SfM: Graph-based View Pruning and Multi-stage Optimization for Robust SfM<br><a href='http://arxiv.org/pdf/2606.22856'>论文</a></td><td>本文针对基于MASt3R的Structure from Motion方法中存在的非重叠图像对错误匹配问题,提出了G-MASt3R-SfM鲁棒重建流程。该方法的核心思想是通过图结构剪枝去除不可靠的视图,并采用多阶段优化策略逐步提升相机参数精度。

◆ 基于图的视图剪枝模块(GVP):利用匹配置信度构建场景图,通过几何一致性对外视图进行剪枝,有效过滤非重叠图像对产生的错误匹配。

◆ 多阶段优化模块(MSO):采用由局部一致性到全局一致性的渐进式优化策略,逐步精化相机参数,避免离群点对全局优化的干扰。

在ETH3D数据集上的实验表明,该方法在相机位姿估计和三维重建精度上均达到当前最优水平,有效抑制了由离群匹配引起的噪声。</td></tr>
...
<tr><td>2026-08-03</td><td>Biconvex Optimization for Smooth Minimum-Time Trajectories around Convex Obstacles<br><a href='http://arxiv.org/pdf/2608.02834'>论文</a> | <a href='https://wernerpe.github.io/bmtp-website/'>代码</a></td><td>本文提出了一种面向凸障碍物的最小时间运动规划双凸优化方法,具备收敛保证和任意时刻可终止的特性,并支持任意阶导数约束。该方法通过变量替换将最小时间目标和全部导数约束联合凸化,采用时变分离平面处理碰撞约束,将问题整体转化为双凸规划。算法通过交替计算最大间隔分离平面与优化轨迹来求解,且仅针对当前发生碰撞的障碍添加新平面,使轨迹能跳跃绕障并跳出局部极小值。理论证明仅需以一条简单的无碰撞折线初始化,算法即可保证收敛。在无人机导航与双臂清仓等实验中,该方法生成的轨迹质量高,计算时间与最先进分解式规划器相当,但适用范围更广,对初始解也更为鲁棒。

◆ 通过变量替换将最小时间目标与任意阶导数约束联合凸化,降低问题求解难度
◆ 将避障建模为时变分离平面,转化为可交替优化的双凸规划问题
◆ 按需添加分离平面,使轨迹具备跨障碍跳跃、跳出局部极小的能力
◆ 只需简单无碰撞折线初始化,即可获得理论收敛保证
◆ 计算效率与现有最优方法相当,但适用范围更广、对初始化更鲁棒...[摘要不完整，待更新]</td></tr>
<tr><td>2026-08-03</td><td>The Field Knows: Cross-Dimensional Geometry from Navigation to Black Holes<br><a href='http://arxiv.org/pdf/2608.07566'>论文</a></td><td>本文提出了一种统一的连续度量场框架,通过单一的因果对比损失进行训练,将场景编码为固定对称矩阵基的系数,组装为李代数元素后指数化为黎曼或洛伦兹度量。该框架跨越多个维度,自动发现从机器人导航中的避障测地线(平面与机械臂构型空间)到洛伦兹时空中黑洞事件视界的完整几何结构谱。大量零样本泛化实验表明,该方法捕获的是可迁移的几何结构而非对特定配置的过拟合记忆。

◆ 统一的跨维度几何学习框架,同一架构、同一因果对比损失、同一训练协议即可在欧氏导航与弯曲时空中涌现截然不同的几何结构。

◆ 基于李代数参数化设计,通过对固定对称矩阵基的系数进行指数化,自然生成黎曼或洛伦兹度规,避免手工指定度规类型。

◆ 单一因果对比损失具备维度无关的通用性,在无任何物理先验的条件下,自发涌现出具备正确洛伦兹号差的黑洞状结构。

◆ 模型捕获可迁移的几何结构,展现出优异的零样本泛化能力,真正学习了几何本身而非记忆特定配置。</td></tr>
...
<tr><td>2026-08-06</td><td>Plan-and-Avoid: Real-Time Aircraft Trajectory Coordination in a Multi-Agent Environment<br><a href='http://arxiv.org/pdf/2608.06648'>论文</a></td><td>本文提出了一个面向多智能体空域运行的实时计划与避让框架(Plan-and-Avoid, PAA),用于在优先航迹周围协调合作式航空器运行。框架通过预测不确定性感知的间隔违犯,在优先航迹无法单独维持安全间隔时,生成受飞行器能力约束的单向避让建议,修改邻近航空器航迹以保证所有交通的良态间隔。基于华盛顿特区真实ADS-B数据,在900多个强制着陆案例、140多小时仿真中得到验证,在个人电脑上最差端到端响应时间为5.7秒,93.5%的建议满足RTCA DO-365规定的35秒阈值要求。

◆ 提出以优先航迹为核心的实时计划与避让协同框架,将航迹规划与冲突避免统一在同一决策环路中。
◆ 引入不确定性感知的良态间隔违犯预测机制,提高复杂空域中安全判断的鲁棒性。
◆ 设计受飞行器性能约束的单向避让建议生成方法,在保护优先任务的同时避免对非优先航空器提出不可执行指令。
◆ 使用真实ADS-B流量结合应急着陆规划场景进行大规模验证,展示了低于6秒的低延迟协同能力。</td></tr>
...
<tr><td>2026-07-20</td><td>Does Robust VIO Need More Learning? Geometry-Verified Visual Measurements under Distribution Shift<br><a href='http://arxiv.org/pdf/2607.17956'>论文</a></td><td>本文针对分布偏移下VIO的鲁棒性问题,质疑了过度学习的必要性,提出一种最小化学习的立体VIO框架,将学习局限在视觉测量生成环节,几何验证与状态估计保持显式。

◆仅使用SEA-RAFT在立体图像上提出稠密对应并预测对应不确定性
◆在稀疏特征点采样稠密光流,通过预测不确定性与立体极线一致性双重过滤
◆将筛选后的光流作为不确定性加权重投影因子融入滑动窗口立体惯性估计器
◆不确定性经三角化传播,用于下游各向异性3D高斯建图
◆在EuRoC、VIODE、4Seasons上验证了运动模糊、动态场景、光照变化及室内外分布偏移下的鲁棒性,消融实验证明学习匹配加几何验证优于单纯依赖学习光流...[摘要不完整，待更新]</td></tr>
<tr><td>2026-07-20</td><td>SLAM in Low-Light Environments: Project Report<br><a href='http://arxiv.org/pdf/2607.17699'>论文</a></td><td>本文针对低光照环境下基于标准RGB相机的视觉SLAM难题，系统性地评估了六种代表性系统在夜间场景中的表现。研究在LaMARia数据集的五个不同难度与光照序列上，对涵盖特征点法、直接法、滤波法和学习法四大范式的ORB-SLAM3、DSO、Kimera-VIO、OpenVINS、DPVO和DPV-SLAM进行了基准测试，并采用绝对位姿误差、相对位姿误差和控制点召回率三类指标进行综合分析。

实验结果表明，Kimera-VIO是唯一能在全部五个序列上完整跟踪的系统，凭借惯性融合取得最低相对误差，但因缺乏回环闭合导致绝对误差持续累积；DPVO与DPV-SLAM虽跟踪稳定却在低光下出现约100米的绝对偏差；而经典单目与滤波方法在困难和低光序列上几乎全部失败或发散。

◆首次系统性地在多个低光照序列上横向比较了跨越四大范式的六种主流SLAM系统，填补了该场景下基准评测的空白。

◆揭示了仅靠RGB相机实现稳定低光跟踪需同时具备惯性融合与全局优化两大条件这一关键结论。

◆指出当前方法的局限并展望了低光专用学习前端或互补传感器的未来发展方向。</td></tr>
