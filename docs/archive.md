...
<tr><td>2026-07-19</td><td>DROID-ANCHOR: Odometry-Anchored Recurrent Metric Depth Estimation<br><a href='http://arxiv.org/pdf/2607.17058'>论文</a></td><td>本文针对单目视觉SLAM系统存在的尺度模糊和尺度漂移问题，提出了一种基于本体感测距的端到端循环架构Metric-DROID，将视觉SLAM锚定到物理现实。核心思路是将高频里程计信息融入DROID-SLAM框架，使系统在保持视觉几何先验的同时获得真实物理尺度，并具备零样本度量对齐能力。

◆ LSTM更新算子：将高频里程计序列编码为空间特征图，为迭代精修提供持续的度量偏置，使视觉重建能够即时获得物理尺度参考。

◆ 不确定性感知度量后端（BA_odom）：将里程计视为带学习异方差协方差的几何锚点，通过回归时变度量不确定性Σ_o，自适应平衡视觉重投影残差与度量平移残差，有效抑制轮打滑和传感器噪声的干扰。

◆ 选择性残差微调策略：在保留预训练几何先验的前提下实现零样本度量对齐，避免了对原有视觉特征表示的破坏。

实验表明该方法在多个数据集上取得了优于现有度量深度估计方法的精度和鲁棒性，为自主机器人导航提供了可靠的尺度感知SLAM方案。</td></tr>
...
<tr><td>2026-07-10</td><td>Empirical Pedestrian Safety Assessment in a Mobile Robot Using a Predictive Social Force Model<br><a href='http://arxiv.org/pdf/2607.09192'>论文</a></td><td>本文针对移动机器人在人行道与行人共存的场景，系统研究了社会力模型（SFM）在安全性方面的扩展与改进。核心工作是在传统SFM和PTTC-based SFM（TSFM）基础上引入有限时间域内的预测社会力向量，提出了预测型SFM（PSFM）和预测型TSFM（PTSFM）。作者将四种模型部署到非完整约束移动机器人上，通过与志愿者面对面对峙场景的实测试验，从客观安全指标（最小PTTC、平均速度、最小距离、横向距离、最大轨迹曲率）和主观舒适度（Likert量表问卷）两个维度进行系统对比评估。实验结论表明，集成PTTC能显著提升机器人导航的安全性能，但预测机制的额外贡献在单行人场景下较为有限，主观评分未呈现统计显著差异。

◆ 提出预测型社会力模型PSFM和PTSFM，在有限时间域内集成预测社会力向量
◆ 首次在非完整约束移动机器人上系统实现并对比SFM、TSFM、PSFM、PTSFM四种变体
◆ 结合客观安全指标与Likert主观问卷，对机器人行人交互进行实证评估
◆ 揭示PTTC集成能有效提升安全指标，而预测扩展在单行人场景中增益有限...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-09</td><td>Deep Spectroscopic Follow-Up of Maisie&#x27;s Galaxy -- A Typical Galaxy in the Early Universe<br><a href='http://arxiv.org/pdf/2607.08749'>论文</a></td><td>本论文对高红移星系 Maisie&#x27;s Galaxy 进行了深度光谱观测,结合两个JWST第三周期NIRSpec G395M项目共19小时以上的曝光数据以及MIRI/LRS观测,获得了丰富的信息。

◆ 给出该星系更精确的红移测量结果 z = 11.408 ± 0.005,并首次通过[OII]双线测量了电子密度 n_e = 108.56 与[OII]推得的恒星形成率 SFR = 1.3,表明该星系位于恒星形成主序上。

◆ 基于[OIII]和[NeIII]发射线测定了关键比值 log(Ne3O2) 和 log(O32),并由此估算了金属丰度 Z/Z☉ = 0.17 和电离参数 log(U) = -2.26。

◆ 提出 Maisie&#x27;s Galaxy 是早期宇宙中一颗较为典型而非极端的星系,挑战了此前对 z&gt;10 亮源多为爆发性恒星形成或AGN主导的认知。

◆ 将其置于其他z&gt;10同类源样本中进行对比,发现其物理参数与SED拟合结果存在差异。

◆ 强调未来需要对更多高红移源进行更深度的光谱观测,以揭示早期宇宙中典型星系的真实性质。</td></tr>
...
<tr><td>2026-06-02</td><td>SAMatcher: Co-Visibility Modeling with Segment Anything for Robust Feature Matching<br><a href='http://arxiv.org/pdf/2606.03406'>论文</a></td><td>SAMatcher提出了一种将对应关系估计重新建模为共可见性建模问题的特征匹配框架，核心思路是先预测跨视图联合可见的区域掩码和边界框，作为结构化先验引导后续的特征匹配。方法基于Segment Anything Model（SAM）构建，并通过对称的跨视图交互机制实现双向特征交换和跨视图语义对齐，突破了传统方法仅在像素或补丁层面进行匹配的局限。

◆ 将共可见性区域掩码和边界框作为结构化先验引入特征匹配流程，先预测共可见区域再进行对应估计
◆ 设计对称跨视图交互机制，实现双向特征交换与跨视图语义对齐
◆ 提出统一监督方案，联合优化掩码预测、边界框回归及掩码-框一致性约束
◆ 将原本面向单图分割的基础模型扩展到多视图对应推理任务，开辟结构化表征学习的新视角

在多个挑战性基准上的实验表明，SAMatcher在大视角和尺度变化场景下显著优于现有匹配流水线，验证了共可见性建模策略的有效性。</td></tr>
...
<tr><td>2026-07-27</td><td>Hybrid Artificial Potential Fields and Spatio-Temporal Transformers for Real-Time AUV Path Planning<br><a href='http://arxiv.org/pdf/2607.25056'>论文</a></td><td>本文针对自主水下航行器在复杂非结构化环境中的路径规划难题，对十三种算法开展了系统性的综合比较评估，涵盖A*、Dijkstra等经典图搜索方法，RRT*等采样方法，PSO、GA、ACO、BCO等元启发式方法以及学习型架构，并在五种高分辨率水下地形导航场景中进行测试验证，所有算法均实现了100%的任务完成率，但路径最优性、避障性能与计算负载之间存在显著权衡。

◆ 提出一种融合人工势场法与时空Transformer的混合路径规划架构，将反应式局部避障能力与基于学习的全局路径最优性相结合，弥补了单一方法在动态水下环境中的不足。

◆ 构建跨经典、采样、元启发式与学习式四类共十三种算法的统一比较评测框架，从路径长度、碰撞率与计算时间三维度揭示各类方法的适用边界。

◆ 混合APF加ST-Transformer在五类场景中取得最短平均路径长度943.15单位、0.031的低碰撞率与0.96秒的计算时间，综合性能优于需回退机制的独立学习模型及延迟较高的传统方法。

该研究为资源受限的实时AUV导航提供了兼顾鲁棒性与效率的主干方案。</td></tr>
...
<tr><td>2026-07-24</td><td>ACME: A Multi-Cultural, Multi-Embodiment Social-Navigation Dataset<br><a href='http://arxiv.org/pdf/2607.21964'>论文</a></td><td>该论文提出了ACME数据集，旨在解决现有社交导航数据集在文化、地理和人机交互多样性方面的不足。该数据集通过跨8个地点、5个国家的大规模采集，涵盖7种机器人平台，提供29.35小时机器人机载数据和43.5小时俯视视角行人跟踪数据，是目前社交导航领域规模最大、多样性最丰富的数据集之一。

论文的核心创新点如下：

◆ 首次构建跨文化、多机器人形态的社交导航数据集，覆盖不同国家和文化背景，显著提升数据集在文化与地理多样性上的代表性。

◆ 引入基于机器人语音的显式人机交互设计，专门采集目标驱动的复杂社交场景中机器人与人群的互动行为。

◆ 提供多模态数据资源，包括3D/2D场景特征、里程计、交互信息及人工标注的行人轨迹，并同时提供人类可读与原始二进制格式。

◆ 通过定性与定量分析证明，ACME相比现有数据集包含更具挑战性的场景和分布更广的行人行为模式。</td></tr>
<tr><td>2026-07-23</td><td>DAPM: UAV Monocular Depth Estimation from Any Height, Pitch, Roll and FOV<br><a href='http://arxiv.org/pdf/2607.21438'>论文</a> | <a href='https://github.com/ThisIsLT/DAPM'>代码</a></td><td>本文针对无人机(UAV)在高度、俯仰、滚转及视场角(FOV)连续变化下的单目深度估计难题,提出了DAPM框架及UAPD数据集,系统性地解决了现有方法在多样化视角和大范围深度分布下泛化能力不足的问题。

◆ 理论层面:通过严格的理论分析,以地面为参考建立了无人机视角与观测距离之间的几何对应关系,首次给出了无人机视角的定量表征方法。

◆ IGD模块:基于上述几何关系实现密集的相机位姿监督,有效增强了深度特征,确保模型在任意视角下都能准确学习。

◆ PQB模块:采用由粗到细的渐进式量化分箱策略,结合渐进监督与分层量化分箱,大幅提升了对复杂航拍图像深度估计的鲁棒性。

◆ UAPD数据集:构建了首个覆盖连续位姿参数分布的无人机深度估计数据集,为评估模型在多视角下的性能提供了全面基准。

◆ 实验结果表明,DAPM在深度估计和相机位姿估计两项指标上均达到了当前最优性能。</td></tr>
...
<tr><td>2026-07-13</td><td>Diffusion MRI preprocessing affects ADC estimation and automatic PI-RADS v2.1 classification in bi-parametric prostate MRI<br><a href='http://arxiv.org/pdf/2607.11385'>论文</a></td><td>该研究系统评估了DWI预处理流程对前列腺MRI中ADC值估计和自动PI-RADS分类的影响,基于fastMRI前列腺队列中的268例数据,依次应用去噪、吉布斯振铃校正和微分同胚配准进行磁敏感畸变校正。ADC分析显示不同预处理流程间存在统计学显著差异,畸变校正直观地将DWI与T2w解剖结构重新对齐并显著改变ADC值,而去噪和吉布斯校正对ADC值影响较小,LLS与IWLLS两种拟合方法产生的ADC图数值上等效。

◆构建了包含去噪、吉布斯校正和畸变校正的完整DWI预处理流程,并系统比较各步骤对ADC定量分析的影响

◆基于多通道MRI输入训练3类DenseNet分类器,发现完整预处理数据集在高危PI-RADS类别上取得最佳AUROC和敏感性

◆通过假阴性分析揭示完全预处理数据集在高危类别上产生最少过度自信的误判,具有临床分诊价值

◆证实DWI预处理特别是畸变校正能同时提升ADC图质量和深度学习模型的PI-RADS分类性能,为前列腺MRI标准化预处理流程的建立提供了重要依据。</td></tr>
