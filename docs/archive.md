...
<tr><td>2026-05-26</td><td>Trinity: Unifying Class-Agnostic Terrain and Semantic Segmentation for Unstructured Outdoor Environments by Leveraging Synthetic Data<br><a href='http://arxiv.org/pdf/2605.27644'>论文</a></td><td>这篇论文针对非结构化户外环境中地形理解的难题,提出了一种统一类别特定语义分割与类别无关地形分割的方法,旨在解决现有方法依赖机器人特定标注、跨平台迁移困难以及标准语义分割无法覆盖多样化地形的问题。作者通过基于Transformer的联合架构、合成数据扩展以及真实数据集构建,实现了机器人无关的视觉地形先验学习,可服务于可通行性估计、视觉里程计和任务规划等下游任务。

◆ 提出Trinity框架,首次在统一Transformer网络中联合实现类别特定的语义分割和类别无关的地形分割,解耦了地形外观学习与机器人特定的可通行性标注。

◆ 提出仅基于视觉外观、不依赖预定义语义标签或机器人相关可通行性分数的类别无关地形分割新任务范式,使学习到的视觉地形先验具备跨机器人平台的可迁移性。

◆ 扩展了OAISYS模拟器并构建了受RUGD启发的合成数据集RUGDSynth,提供了大规模、多样化地形外观的类别无关训练样本,缓解了真实数据标注成本高的问题。

◆ 发布了EXTerra真实世界数据集,同时提供类别特定和类别无关的地形标注,为该新任务的评估和后续研究奠定了基准。

◆ 通过实验验证了联合分割策略在复杂户外环境中的可行性与有效性,证明合成数据与真实数据结合的训练范式能够有效支撑该新任务。</td></tr>
...
<tr><td>2026-05-29</td><td>Constrained Whole-Body Tracking for Humanoid Robots<br><a href='http://arxiv.org/pdf/2606.00374'>论文</a></td><td>本文提出 ConstrainedMimic,一种面向人形机器人全身运动跟踪的实时约束执行控制框架,旨在解决基于强化学习的全身控制策略在训练完成后难以保证安全性和满足新增约束的问题。该方法融合操作空间控制与控制屏障函数(CBF)的思想,利用全身运动学和动力学信息,对 RL 跟踪策略输出的动作进行实时修正,从而在运行时强制满足任意指定的运动学和动力学约束。作者在仿真 Unitree G1 上通过全身动作跟踪与遥操作实验验证了该框架在自碰撞与外部障碍避让、关节限位以及质心稳定性等多种约束下的有效性。

◆ 提出 ConstrainedMimic 框架,首次将运行时任意约束的强制执行能力引入基于 RL 的人形机器人全身跟踪策略,无需重新训练即可施加新的安全约束。
◆ 将操作空间控制与控制屏障函数原理相结合,同时在运动学参考和底层动力学两个层面进行约束处理,兼顾跟踪性能与安全保障。
◆ 设计的约束修正机制能保持与当前接触模式和跟踪目标的一致性,在约束激活时对策略原有能力的限制最小化。
◆ 框架完全可微,支持 CPU、GPU、TPU 多平台部署,运行频率可达 300-500 Hz,满足实时控制需求。
◆ 在仿真 Unitree G1 上系统验证了自碰撞与外部障碍避让、关节限位、质心稳定等多类约束的统一处理能力。</td></tr>
<tr><td>2026-05-29</td><td>GLENS: Global Search via Learning from Solver Iterates with Diffusion Models<br><a href='http://arxiv.org/pdf/2606.00366'>论文</a></td><td>本文针对多模态非凸连续优化问题,提出了GLENS方法,用于生成大量高质量且多样化的初始猜测,以加速数值求解器收敛并发现多个局部最优解。该方法的核心思想是利用求解器在迭代过程中产生的中间解作为免费的数据增强,克服了现有方法仅使用最终收敛解而导致训练数据有限、丢失局部邻域信息的问题。在基准非凸问题和双机器人避障导航问题上的实验表明,GLENS在保持多模态分布的同时显著加快了不同求解器的收敛速度。

◆ 提出利用求解器中间迭代点作为数据增强的新思路,大幅提升了数据效率,避免了昂贵的离线全局搜索成本。
◆ 设计了基于扩散模型的邻域结构模型,在问题参数条件下学习局部最优解周围的几何结构,而非仅学习最优点本身。
◆ 提出求解器行为模型,在扩散采样过程中学习并引导样本朝向附近最优解的精化方向。
◆ 将邻域几何学习与求解器行为引导相结合,在生成高质量初始猜测的同时有效保留了多个局部最优的多样性分布。
◆ 在非凸基准问题和实际多机器人导航场景中验证了方法在不同求解器和问题设置下的通用性与鲁棒性。</td></tr>
...
<tr><td>2026-05-29</td><td>Predicted-Flow Control Barrier Functions for Real-Time Safe Optimal Control<br><a href='http://arxiv.org/pdf/2606.00297'>论文</a></td><td>本文针对传统控制屏障函数(CBF)难以构造且具有短视性的问题,提出了预测流控制屏障函数(P-CBF),将CBF从当前状态的函数推广为有限预测时域内参数化控制规划下预测流的泛函,从而在整个预测时域上提供安全性认证。文章通过引入终端候选P-CBF和规划时间偏移机制,解决了控制约束下P-CBF有效性难以保证的难题,并将实时控制与控制规划参数、规划时间偏移的演化统一为单一凸优化问题,在控制约束为凸多面体时退化为二次规划(QP)。所提出的FlowBarrier方法在非完整地面机器人密集环境导航实验中,相较非线性模型预测控制和两种CBF安全滤波方法,在100次试验中取得了最高目标到达率、零安全违规和最低计算时间。

◆ 提出P-CBF概念,将CBF从状态函数推广为预测流的泛函,实现整个预测时域内的安全认证,克服了传统CBF的短视性。
◆ 引入终端候选P-CBF,通过要求预测流在终端时刻进入备份安全集,解决了控制约束下P-CBF有效性难以保证的问题。
◆ 创新性地引入规划时间偏移作为附加自由度,调节预测时域以保证优化问题的可行性。
◆ 将实时控制、控制规划参数与规划时间偏移的演化联合表述为单一凸优化问题,在多面体约束下可化为QP,统一了有限时域积分代价优化与安全认证。
◆ 实现并验证了FlowBarrier算法,在密集环境导航中相比NMPC和CBF安全滤波方法在成功率、安全性和计算效率上均显著占优。</td></tr>
...
<thead><tr><th>日期</th><th>标题</th><th>摘要</th></tr></thead>
<tbody>
<tr><td>2026-06-03</td><td>Multi-Camera AR Guidance System for Surgical Instrument Handling and Assembly: Investigating Workload and Efficiency<br><a href='http://arxiv.org/pdf/2606.04992'>论文</a></td><td>该论文针对手术器械装配过程中洗手护士面临的高认知负荷问题,提出了一种基于多相机6D姿态估计与增强现实头显的无标记术中导航系统。系统通过已知物体实现姿态估计与相机标定,在头戴显示器上叠加器械尖端定位提示和分步装配动画,并支持通过视线选择与脚踏板进行交互。技术评估表明该方法优于现有6D姿态估计方法;在29名洗手护士参与的膝关节置换模拟手术研究中,与纸质手册相比显著降低了主观工作负荷,任务完成时间缩短21.3%(约4.76分钟),且对不熟悉该器械的护士帮助尤为明显。

◆ 提出首个面向手术器械装配的无标记多相机AR引导系统,融合6D姿态估计与头显原位可视化
◆ 利用已知物体实现相机标定与姿态估计,避免了对外加标记物的依赖,更适应手术无菌环境
◆ 6D姿态估计网络完全基于合成数据训练,提升了模型的泛化能力与实际临床适用性
◆ 设计了基于视线选择与脚踏板的交互方式,满足术中无接触操作需求
◆ 通过29名洗手护士在膝关节置换模拟场景中开展系统性用户研究,实证了系统在主客观指标上的有效性,尤其对经验不足者带来显著收益...[摘要不完整，待更新]</td></tr>
