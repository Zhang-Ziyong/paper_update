...
<ol>
<li><a href='#slam'>SLAM (128篇)</a></li>
<li><a href='#sfm'>SFM (64篇)</a></li>
<li><a href='#image-matching'>Image Matching (20篇)</a></li>
<li><a href='#obstacle-avoidance'>Obstacle Avoidance (132篇)</a></li>
<li><a href='#navigation'>Navigation (143篇)</a></li>
<li><a href='#motion-planning'>Motion Planning (206篇)</a></li>
<li><a href='#sensor-calibration'>Sensor Calibration (36篇)</a></li>
<li><a href='#sensor-undistortion'>Sensor Undistortion (79篇)</a></li>
</ol>
</details>

<h2 id='slam'>SLAM (128篇)</h2>
...
<tr><td>2026-07-07</td><td>CILC: Cryptographically-secure Inter-agent Loop Closure Candidate Detection for Multi-Agent Collaborative SLAM<br><a href='http://arxiv.org/pdf/2607.06700'>论文</a></td><td>本文针对多智能体协同SLAM中全局描述符(GD)明文交换导致的隐私泄露问题展开研究。作者首先通过攻击实验证明,被攻陷的群体成员可从诚实智能体广播的GD中重建其图像与轨迹信息,揭示了现有加密通信方案的严重安全隐患。

为解决该问题,本文提出了CILC系统,首次将安全多方计算(SMPC)技术应用于智能体间回环候选检测中,通过密文相似度比较避免GD明文传输,有效防御内部恶意节点的隐私窃取。

◆仅对计算轻量的GD相似度比较环节应用SMPC,而非保护整个CSLAM流程,实现了隐私性与计算开销之间的良好权衡。

◆支持多模态GD(视觉与LiDAR)且在仿真与硬件实验中验证了实时性与通信可行性。

◆通过理论分析与实验证明,系统能够将信息泄露控制在可接受范围内,即便群体中存在被攻陷节点。</td></tr>
...
<h2 id='obstacle-avoidance'>Obstacle Avoidance (132篇)</h2>
...
<h2 id='navigation'>Navigation (143篇)</h2>
...
<h2 id='motion-planning'>Motion Planning (206篇)</h2>
...
<tr><td>2026-06-06</td><td>SynthICL: Scalable In-context Imitation Learning with Synthetic Data<br><a href='http://arxiv.org/pdf/2606.08154'>论文</a></td><td>SynthICL是一篇针对机器人上下文模仿学习（ICIL）可扩展性问题的论文，旨在用合成数据替代真实数据训练通用策略。该框架完全基于RGB合成数据进行训练，摆脱了对深度传感器、精确相机标定和真实示教数据的依赖，使系统更简单、更易扩展。作者构建了高保真的ICIL数据生成流水线，并基于此训练了流匹配Transformer策略模型。此外，模型还引入了下一阶段子目标图像预测任务，使控制更加精确和视觉化。在16个未见过的真实世界操作任务上，SynthICL仅凭测试时一条示范就取得了79%的平均成功率，优于现有方法。

创新点：
◆ 提出首个完全基于RGB合成数据训练的可扩展ICIL框架，无需深度信息和精确相机标定
◆ 设计了高保真大规模合成数据生成流水线，有效解决ICIL训练数据稀缺问题
◆ 引入子目标图像预测作为辅助任务，增强策略的视觉定位与控制精度
◆ 采用流匹配Transformer策略架构，提升对复杂操作任务的建模与泛化能力...[摘要不完整，待更新]</td></tr>
