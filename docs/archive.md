...
<tr><td>2026-09-01</td><td>Monocular Depth Estimation from a Single Image: Progress and Opportunities<br><a href='http://arxiv.org/pdf/2609.01172'>论文</a></td><td>这篇综述系统回顾了单目深度估计从早期学习方法到基础模型时代的演变历程,梳理了十年研究中的关键挑战与核心洞见,包括相对深度与度量深度的区分以及室内外合成数据集的应用。在方法论层面,创新性地将基础模型方法划分为判别式与生成式两大范式,深入探讨了大规模预训练如DINOv3和合成数据对模型性能的提升作用。主要贡献与创新点包括:◆提出判别式与生成式的基础模型分类框架,为该领域提供了新的方法论视角 ◆系统整合定量基准与定性示例的多维度模型比较方法 ◆将单图深度估计自然延伸至视频领域,扩展了研究边界 ◆从视觉SLAM、内容生成到机器人感知等多个维度展示了深度估计的实际应用价值。最后,论文指出了基础模型时代下的开放挑战和未来研究方向,为该领域的进一步发展提供了路线图。</td></tr>
<tr><td>2026-09-01</td><td>VOIM: Training-Free Open-Vocabulary 3D Instance Mapping for RGB-D and Monocular SLAM<br><a href='http://arxiv.org/pdf/2609.00775'>论文</a></td><td>VOIM提出了一种无需训练的体素级在线实例管理框架，能够从RGB-D或单目RGB构建开放词汇的3D实例地图，填补了训练免系统在单目RGB场景下的空白。其核心思想是推迟标签与实例的判定决策，待软证据在多视角下逐体素累积后再做决定，避免了在首次检测证据最弱时仓促提交。通过统一映射阶段，在ScanNet++的四种感知配置下均超越现有最强在线RGB-D系统OVO-SLAM 4.8至11.7 mIoU，并在十场景中全部胜出，同一系统无需修改即可迁移到纯单目RGB输入。

◆ 推迟决策机制：不在首次检测时即定标签和实例，而是让软证据在多视角下逐体素累积后再判定，提升地图一致性。

◆ 体素级在线实例管理：无需训练，结合现成感知模型，将标签与实例解耦到映射阶段处理。

◆ 统一框架支持RGB-D与单目RGB：同一系统无需修改即可在两种输入下工作，填补了训练免系统在单目RGB上的空白。

◆ 在ScanNet++上以44.07 mIoU显著优于基线32.37，10个场景全部胜出，池化得分33.31对比25.97。</td></tr>
...
◆ 提出跟踪失败与漂移分离评估框架,取代传统单一轨迹误差度量,能够区分系统是否彻底失效还是仅产生累积误差。

◆ 揭示学习跟踪器将灾难性失败转化为持续甚至严重漂移的现象,改变了对学习SLAM鲁棒性的传统认知。

◆ 发现合成扰动的物理保真度决定其预测真实场景能力:结构化雨雾代理可保持真实世界排序,而简单光照代理则会误导评估结论。

◆ 为SLAM鲁棒性基准测试的扰动设计提供了实证依据,强调需谨慎选择合成代理以确保工程结论可靠性。</td></tr>
<tr><td>2026-08-31</td><td>GeoFF3D: Coordinate-Anchored Feed-Forward Reconstruction for Large-Scale UAV Mapping<br><a href='http://arxiv.org/pdf/2608.28288'>论文</a> | <a href='https://github.com/yanxian-ll/GeoFF3D'>代码</a></td><td>GeoFF3D针对大范围无人机航测场景，提出一种结合坐标锚定前馈模型与空间大尺度重建框架(SLRF)的方法，直接在重力对齐的Z-up度量坐标系下预测相机位姿与稠密点图，避免了近共线轨迹下Sim(3)对齐的不稳定性问题。SLRF将图像划分为空间重叠的块，通过共享视图先验传播与分层聚合实现可扩展的多块重建，并可适配不同的有界视图模型。
...
<tr><td>2026-08-30</td><td>Everybody Tracking Every Body<br><a href='http://arxiv.org/pdf/2608.29927'>论文</a></td><td>本文研究多人交互场景下基于多视角第一人称视频的3D人体姿态估计问题，通过集中式协调融合各佩戴者相机和IMU数据实现精准追踪。系统利用VIO SLAM技术获得高质量的自运动跟踪，并将第一人称视角对他人的观察视为稀疏、不连续且可靠性变化的外视角姿态线索。◆ 创新点一：提出基于扩散模型的方法，将头部运动估计与外视角姿态观测在内容与可靠性双重条件下进行融合，有效整合异质信息源。◆ 创新点二：采用单人动作捕捉数据与多人视频混合训练策略，使模型同时学习丰富的身体运动先验和视频观测可靠性先验。◆ 创新点三：在集中式架构下统一同步来自多个佩戴者的数据流，将传统单人自我中心感知拓展为协同式群体感知框架。在多人交互数据集上的实验表明，该融合方法在绝对与相对姿态精度上均优于纯运动基线和纯视觉基线。</td></tr>
<tr><td>2026-08-29</td><td>RoSe-SLAM: Robust Semantic-Aware Gaussian Splatting SLAM from Dynamic Monocular Videos<br><a href='http://arxiv.org/pdf/2608.29003'>论文</a></td><td>RoSe-SLAM提出了一种面向动态非结构化环境的鲁棒语义感知高斯泼溅SLAM系统，通过整合2D基础模型的语义特征与几何运动线索，实现了对单目视频输入的准确相机跟踪与高质量静态场景重建。该方法突破了传统SLAM静态假设的局限，能够有效识别并解耦动态干扰物与静态背景。

核心创新点如下：

◆ 利用2D基础模型提取的语义特征替代传统手工语义标签，将丰富的语义信息蒸馏至高斯场中，实现语义感知的多视图一致性，显著提升动态环境下的几何重建与场景修复质量。

◆ 提出时空运动掩码生成模块，兼顾长期运动监测与短期瞬态动态捕捉，实现对动态对象与静态背景的鲁棒有效解耦。

◆ 在全局束调整中设计遮挡感知的关键帧选择机制，以遮挡程度作为关键帧优先级度量标准。

◆ 引入多视图语义一致性模块，结合几何运动线索与语义先验动态过滤不可靠观测，重建精确的静态场景几何结构。

该方法在TUM、Bonn和Wild-Mocap等动态基准数据集上的广泛实验表明，其在轨迹估计和静态场景建图方面均优于现有动态RGB SLAM基线方法。</td></tr>
<tr><td>2026-08-28</td><td>Generation of High-Level Concepts in 3D Scene Graphs via Autoregressive Diffusion<br><a href='http://arxiv.org/pdf/2608.28733'>论文</a></td><td>本文针对室内三维场景图(3DSG)中高层概念生成问题,提出了一种基于自回归扩散的统一图生成模型,能够从观测到的竖直平面自底向上构建任意层级深度的完整场景图。

◆创新点一:将图结构生成与节点空间特征学习统一到单一自回归扩散模型中,克服了现有方法需要分别为结构和特征训练单独模型的局限,显著提升了对新类别与复杂层级的可扩展性。

◆创新点二:实现了自底向上、跨任意层级深度的完整3DSG生成,仅以观测到的竖直平面为输入,即可联合推断出房间、楼层、建筑等高层度量-语义概念。

◆创新点三:提出基于Fused Gromov-Wasserstein距离的图级评估方法,为生成3DSG与真值之间的比较提供了原则性的度量框架,弥补了传统节点级指标的不足。

实验在涵盖合成场景、真实建筑平面图和机器人传感器数据的多种数据集上验证,所提方法一致超越所有学习方法和随机基线,并在最大层级和真实单层数据上击败了具有目标图大小先验的一次性生成模型。</td></tr>
...
<tr><td>2026-09-02</td><td>MV-dVRK: A Multi-Viewpoint Benchmark for Spatial Surgical Perception<br><a href='http://arxiv.org/pdf/2609.02717'>论文</a></td><td>本文提出MV-dVRK，首个结合多曝光同步立体视点、精确表面几何与相机位姿的离体手术数据集。
◆ 首次为真实内窥镜图像提供多视角3D重建的严格评估基准，填补临床机器人单立体相机导致多视点数据稀缺的空白。
◆ 静态子集提供经工业3D扫描仪验证的密集SfM参考几何、真值相机位姿及稀疏视图测试集。
◆ 系统比较零样本单目、立体、多立体和多视图方法，发现双内窥镜时多立体覆盖最高，三视角时优化多视图方法在1mm容差内覆盖67%真值表面点，远超前馈基础模型43%。
◆ 包含十个动态序列，覆盖多类手术任务，运动复杂性和组织变形递增，为多视角手术感知研究提供基础。
该数据集与基准已公开，推动手术场景多视点三维感知发展。</td></tr>
...
◆ 由正的ΣsSFR梯度发现LSBG呈“由内向外”增长。
◆ 证明SFMS在千秒差距尺度仍成立，恒星形成活动受局部事件调控，并指出不同激发源处理会改变斜率，仅用恒星形成区更可取。</td></tr>
...
<tr><td>2026-09-01</td><td>Linking neutral gas inflows and outflows to offsets in the star-forming main sequence and mass-metallicity relation<br><a href='http://arxiv.org/pdf/2609.01707'>论文</a></td><td>本文利用DESI DR2中约6000个有Na I D吸收的恒星形成星系，将中性气体流入/流出与恒星形成主序（SFMS）和质量-金属丰度关系（MZR）的群体偏移直接联系起来。
◆ 首次在大样本中系统比较中性气体流宿主与质量、红移匹配控制样本，揭示气体流与SFMS及MZR散射的统计关联。
◆ 区分慢流入、快流入与流出，发现慢流入宿主sSFR增强0.20-0.30 dex且无金属丰度偏移，暗示入流气体可能已富金属或长期混合。
◆ 流出宿主sSFR增强0.25-0.40 dex且中心金属丰度升高0.04-0.06 dex，位于SFMS上1σ包络，支持反馈调节后续生长。
◆ 快流入宿主SFR增强较弱且金属丰度略低，说明不同流速对应重子循环的不同阶段。
◆ 气体流宿主还呈现Dn4000系统偏移，表明中性气体流通过重子循环贡献于星系尺度关系的散射。</td></tr>
...
<tr><td>2026-07-10</td><td>What VGGT Knows About Overlap: Probing Geometric Foundation Models for Co-Visibility<br><a href='http://arxiv.org/pdf/2607.09503'>论文</a></td><td>针对3D重建中图像对共视性判断这一基础难题，本文首次揭示几何基础模型VGGT无需任何监督即可隐式编码共视性信息。

◆ 发现VGGT内部存在类大语言模型的层次化表征结构：早期层构建三维场景表征，后期层专门作为共视性推理器。

◆ 识别出第L17层作为稳定的&quot;负锚点&quot;，能一致地将非共视图像对分离路由，为几何基础模型的层专业化提供了任务驱动的实证证据。

◆ 提出Co-VGGT方法，冻结VGGT主体，仅训练轻量化的层级混合专家头（参数少于7.5M），将每层视为针对不同输入对自适应加权的专门专家。

◆ 在Co-VisiON基准上，Co-VGGT超越人工标注基线，比先前工作提升超过25%的图像对准确率和10%的多视角准确率，且预测高度校准（ECE=0.030），可直接作为边权重用于下游SfM和SLAM流程而无需后处理校正。</td></tr>
<tr><td>2026-07-10</td><td>Glob3R: Global Structure-from-Motion with 3D Foundation Models<br><a href='http://arxiv.org/pdf/2607.09225'>论文</a> | <a href='https://junyuandeng.github.io/Glob3r'>代码</a></td><td>Glob3R提出了一种基于3D基础模型（如VGGT）的全局SfM风格重建方法，旨在解决现有前馈方法在大规模或无序图像集上精度不足、容易产生漂移和不一致性的问题。该方法冻结Pi3X骨干网络，新增轻量级稠密匹配头来预测参考帧与相邻视图之间的图像warp，并将其转化为稀疏但可靠的多视图特征轨迹，作为全局优化的对应约束。

◆ 基于3D基础模型的前馈预测，并设计稠密匹配头将warps转化为多视图特征轨迹，提供全局优化的对应关系约束

◆ 提出基于关键帧的滑动窗口关联策略，通过跨窗口传播轨迹和相对位姿，实现可扩展的大规模重建

◆ 引入全局运动平均和光束法平差，对相机位姿进行全局优化以减少尺度不一致，并恢复稠密场景几何

在室内、室外、大规模驾驶和无序SfM基准上的实验表明，Glob3R在鲁棒性和精度上均优于前馈基础模型基线和近期可扩展重建方法，同时比经典SfM流水线更稳健，其精化后的位姿还提升了神经渲染质量，验证了基础模型先验与全局几何优化相结合的优势。</td></tr>
...
<tr><td>2026-08-11</td><td>Multi-Level Evidence Aggregation for Robust Facial Phenotype Retrieval in Rare Genetic Disorder Prioritization<br><a href='http://arxiv.org/pdf/2608.11037'>论文</a></td><td>本文提出推理时多级证据聚合框架，在不修改GestaltMatcher-Arc编码器的情况下提升罕见遗传病面部表型检索。
◆ 融合同一患者多图的嵌入级聚合，充分利用患者级观察证据。
◆ 构建患者加权疾病质心，整合疾病级gallery证据。
◆ 采用个体-质心混合评分，同时纳入局部最近邻证据。
在GMDB v1.1.4的已见、未见和多图子集上，该方法一致提升top-N精度，GMDB-Freq的top-1从38.52%升至48.82%，GMDB-Rare从19.38%升至23.79%，GMDB-Multi-Freq从46.12%升至60.94%，GMDB-Multi-Rare从18.54%升至26.71%。
这些结果说明推理时多级证据聚合无需重训即可增强检索，支持从孤立单图匹配转向患者与疾病证据聚合的罕见病优先排序。</td></tr>
<tr><td>2026-08-10</td><td>XFeat Revisited: Reproducibility and Evaluation of a Lightweight Image Matcher<br><a href='http://arxiv.org/pdf/2608.09519'>论文</a></td><td>◆ 本文对轻量图像匹配器XFeat进行可复现性研究，基于论文与补充材料重实现架构，并区分“重评估”原检查点与“复现”模型。
◆ 作者发现论文、补充材料和公开代码在骨干布局、融合模块及训练损失等实现细节上存在差异，影响结果解释。
◆ 在MegaDepth-1500和ScanNet-1500上，复现模型与原检查点精度接近，部分甚至更优，支持XFeat具备强精度-效率权衡。
◆ 消融显示，并行关键点分支对半稠密匹配重要，但收益弱于原文宣称；单个跳跃连接的具体位置证据仍不充分。
◆ 下游复现中，单应估计与原结果较一致，但Aachen视觉定位即使使用发布检查点也低于报告值，表明对未充分说明的评估细节敏感。
◆ 进一步在视网膜、热可见和多模态遥感等零样本OOD与跨模态场景评估，XFeat部分有效，但在严重模态偏移下性能急剧下降。</td></tr>
<tr><td>2026-08-06</td><td>A Low-Latency ASIC Architecture for Real-Time Line Segment Detection<br><a href='http://arxiv.org/pdf/2608.06439'>论文</a></td><td>本文提出一种基于步长算法的低延迟实时线段检测ASIC架构，采用全流水线结构，实现每时钟一像素处理和确定性延迟，在45nm工艺下VGA达325 FPS、Full HD达48 FPS，功耗25.54 mW、面积0.412 mm²，较90nm LHT ASIC功耗降低49%且帧率提升超1.6倍。
◆ 采用寄存器行缓冲与数据重用，减少存储访问开销。
◆ 采用基于MCM的无乘法器滤波，降低计算与功耗成本。
◆ 采用8类角度量化，简化角度表示与后续匹配。
◆ 采用类CAM关联存储器，实现单周期匹配。
◆ 优化重复消除机制，提升线段输出质量与效率。</td></tr>
...
◆ 建立兼容稀疏、半稠密和稠密匹配器的统一评估协议。
◆ 在LEVIR-GM上，LoRetta的AUC达83.3%，比最强基线RoMa v2高1.6点，1和2像素PCK分别提升6.5和8.2点，推理延迟降低47.8%。
◆ 宇航员到卫星、无人机到卫星地理定位实验表明其可作为可复用几何对齐器，具备良好迁移能力。</td></tr>
...
◆融合了基于学习的关键点匹配与基于优化的对齐策略，实现高效准确的位姿估计。

◆设计了新颖的失败检测与恢复模块，可实时监控位姿可靠性，自动识别跟踪发散或遮挡状态。

◆跟踪失败时执行全局重检测与位姿估计，并通过严格的候选验证机制确保恢复的可靠性。

◆还构建了一个包含遮挡与快速运动场景的新数据集用于评估。

实验结果表明，该方法在常规序列上达到state-of-the-art精度，运行时速达57.6 FPS，并在挑战性条件下展现出最鲁棒的跟踪性能。</td></tr>
...
◆ 突破传统阈值调参对标注验证集的依赖，使VPR系统在缺乏地面真值的真实场景中仍能保持鲁棒，对安全关键机器人应用具有重要意义。

◆ 在六个基准数据集、五种SOTA VPR方法与四种VLM上进行了系统实验，验证了方法的通用性与可迁移性。

实验结果表明，该方法将召回率@1平均提升13.6%，同时将错误接受率降至12%，精度保持在95%以上，覆盖率超过75%，显著优于现有最优验证方法。</td></tr>
...
<tr><td>2026-06-22</td><td>G-MASt3R-SfM: Graph-based View Pruning and Multi-stage Optimization for Robust SfM<br><a href='http://arxiv.org/pdf/2606.22856'>论文</a></td><td>G-MASt3R-SfM是针对多视图三维重建中运动恢复结构（SfM）任务提出的新型鲁棒性增强方法。该工作指出了现有MASt3R-SfM的关键缺陷：MASt3R匹配在非重叠图像对之间容易产生错误对应关系，直接用于优化会显著降低位姿估计精度。为此，论文提出了两个核心创新模块。

◆ 图视角剪枝（GVP）模块：基于匹配置信度构建场景图，通过几何一致性对外视（outlier view）进行剪枝，有效过滤不可靠的图像对。

◆ 多阶段优化（MSO）模块：采用由局部一致性逐步扩展到全局一致性的渐进式策略，对相机参数进行分层细化优化。

在ETH3D数据集上的实验表明，该方法在相机位姿估计和三维重建两项任务上均达到了当前最优精度，能够有效抑制异常匹配带来的噪声干扰。整体而言，该工作通过图结构剪枝与多阶段优化的结合，显著提升了基于深度匹配的SfM管线在复杂场景下的鲁棒性。</td></tr>
...
<tr><td>2026-09-20</td><td>PackLab: A Comprehensive Framework for Developing, Training, and Evaluating MLLMs in Robotic Bin Packing<br><a href='http://arxiv.org/pdf/2609.23784'>论文</a> | <a href='https://github.com/Correr-Zhou/PackLab'>代码</a></td><td>本文提出 PackLab，一个面向机器人箱装中多模态大模型开发、训练与评估的综合框架，旨在解决现有几何启发式和强化学习难以适应异构配置与闭环长程决策的问题。
◆ PackLab-Suite 提供基于物理仿真的平台，可规模化生成多样装箱轨迹并评估物理结果。
◆ PackLab-VLM 是装箱专用多模态大模型，能理解动态物体与容器状态，并在闭环中联合选择物体和预测放置。
◆ PackLab-Bench 提供多难度标准化装箱场景，支持系统化评估。
实验表明，PackLab-VLM 在多种物体集合和容器配置上平均优于传统装箱启发式、传统强化学习和通用多模态大模型。
该工作凸显了 MLLM 在长程机器人装箱中的潜力，并开源代码、模型、数据集与基准。</td></tr>
<tr><td>2026-09-20</td><td>Which Terrain Is Better? Preference Learning with VLM Prototypes for Off-Road Traversability Ranking<br><a href='http://arxiv.org/pdf/2609.23673'>论文</a></td><td>论文指出现有越野可通行性分数只回答“是什么”，无法表达机器人应偏好哪块地形，因此将问题重新定义为视觉可通行性排序，用区域比较监督偏好。
◆ 提出TravPro，把标准标注自动转换为有序区域对，在冻结VLM的patch token上训练轻量readout学习偏好。
◆ 将VLM patch token一次性聚类为固定原型库，由readout为每个原型学习偏好分数，实现可迁移的偏好表征。
...
<tr><td>2026-09-16</td><td>AdaGeoVLN: Selective Geometry Across Representation Depth and Navigation Time for Vision-Language Navigation<br><a href='http://arxiv.org/pdf/2609.18789'>论文</a> | <a href='https://humanoid-research.github.io/adageovln/'>代码</a></td><td>AdaGeoVLN提出一种流式视觉语言导航框架，核心是联合利用表示深度与导航时间上的几何信息。  
◆ 提出分层GFM-VLM融合，将GFM早期、中期、后期表示耦合到连续策略阶段，而非反复注入终端特征。  
◆ 设计导航感知GFM记忆，按指令相关性、几何置信度和转移新颖性保留历史VGGT全局注意力KV状态，并设置每层有界预算。  
◆ 保留状态在融合前为后续观测提供几何上下文，且仅用单RGB流、无需额外导航专用外部数据。  
在R2R-CE和RxR-CE上取得强性能。消融表明，多深度耦合明显优于匹配位置的终端特征重复注入，有界保留在保持性能的同时大幅减少GFM-KV内存。</td></tr>
<tr><td>2026-09-16</td><td>Calibrated Probabilistic Obstruction Reasoning with Vision-Language Models for Grasping in Clutter<br><a href='http://arxiv.org/pdf/2609.18718'>论文</a></td><td>CPOR-Grasp 提出面向杂乱抓取的校准概率阻碍推理框架，将 VLM、深度与 amodal 掩码证据校准融合，并把成对阻碍不确定性传播到抓取、移除或延迟决策。
◆ 不再固守单一阻碍图或移除策略，而是诱导有效阻碍图分布，并边缘化计算目标可达性与阻碍物移除概率。
◆ 通过保留最高概率图实现可处理推断，并用被丢弃概率质量的总变差上界支持可认证决策、自适应停止和原则性延迟。
◆ 在 Gemini Robotics 上把校准误差从 0.1416 降至 0.0185，显著缓解 VLM 预测失准。
◆ 图截断在 99.74% 决策上与精确推理一致，且使用图数量少 56 倍。
在合成和真实 UNOBench 场景中，CPOR-Grasp 优于现有方法，真实实验平均成功率达 77.8%。</td></tr>
<tr><td>2026-09-16</td><td>GroundingVLN: Reasoning and Acting with Grounding for Vision-Language Navigation<br><a href='http://arxiv.org/pdf/2609.18581'>论文</a></td><td>针对现有VLN中语义推理与空间执行脱节、推理未锚定视觉证据且高层决策缺精确空间目标的难题，论文提出GroundingVLN，以视觉 grounding 作为推理与行动的共享接口。
◆ 推理时锚定：在结构化推理中将任务相关视觉证据绑定到精确图像位置，实现 grounding reasoning。
◆ 行动时锚定：预测与进度对齐的像素目标，并由几何规划器转化为底层动作，实现 acting through grounding。
◆ 数据与训练：构建时序对齐的 GroundingCOTVLN-188K，并提出 GEAR 强化学习对齐 grounded reasoning、空间决策与下游执行。
实验表明其在 R2R-CE 和 RxR-CE 达 69.9% 与 75.1% SR，仅用最强基线 0.9% 训练数据，且仅用 R2R 训练即可在 RxR-CE 获 59.9% SR，超出最强基线 20.1%。</td></tr>
<tr><td>2026-09-16</td><td>Robot Visions: Breaking reCAPTCHA at Zero Cost and Zero Shot<br><a href='http://arxiv.org/pdf/2609.18518'>论文</a></td><td>本文系统研究Google reCAPTCHA，将其视觉挑战分为Type A独立图像块（静态与动态）和Type B单图4×4网格两类。
◆ 首次证明完全免费、本地运行的开源模型可在零样本、零成本下攻破Google reCAPTCHA，无需训练和API访问。
◆ 针对两类挑战设计零样本求解器，Type A用CLIP达58%单挑战准确率，Type B用OWLv2达43.5%。
◆ 构建端到端自动求解器，在500个真实reCAPTCHA会话中取得92.6%的每会话成功率。
◆ 展示非技术攻击者仅靠自然语言指令操作普通AI助手即可实施攻击，把攻击技能门槛降至近零。
◆ 揭示reCAPTCHA视觉挑战式回退机制已成防御链最弱环节，挑战式视觉CAPTCHA或已走到生命周期终点。</td></tr>
<tr><td>2026-09-16</td><td>VLM-MPPI: Grounding Natural Language in Behaviorally Diverse Trajectories for Aerial Navigation<br><a href='http://arxiv.org/pdf/2609.18451'>论文</a></td><td>本文提出一种面向杂乱室内环境的层次化无人机导航框架，将自然语言意图与动态可行飞行行为对齐。  
◆ 采用六个行为条件MPPI规划器并行集成，通过模式特定引导代价与采样偏置诱导不同轨迹模式，形成紧凑且有意多样的候选集，而非随机扰动。  
◆ 将三维候选轨迹投影到机载第一人称RGB视图，把语言接地转化为视觉动作选择问题，由预训练VLM根据叠加图像和语言提示异步选择候选索引。  
◆ 构建MPPI 20Hz重规划与PID底层跟踪的异步协同管线，在VLM延迟下仍保持语言对齐与飞行安全。  
◆ 在Isaac Sim和真实四旋翼平台（LiDAR、RGB）验证，实现语义有意义的行为多样性、可重复安全飞行，并在评估场景中达到100%任务成功。</td></tr>
<tr><td>2026-09-16</td><td>Decoupling Vision, Language, and Action for Efficient Multi-Task Robot Policies<br><a href='http://arxiv.org/pdf/2609.18374'>论文</a></td><td>论文研究解耦视觉、语言与动作模块能否替代庞大VLA主干，以更低成本完成多任务机器人操控。
◆ 提出DEM，用微调DINOv3视觉编码器、冻结NeoBERT语言编码器和MeanFlow动作头组成解耦策略。
◆ 通过固定演示、训练预算、任务和评测平台，仅更换视觉、语言和动作组件，与七种VLA基线进行受控对比。
◆ MeanFlow动作头能在单次前向中生成整个动作块，显著降低每个控制步的推理开销。
◆ 在18个仿真任务和3个真实机器人任务上，DEM成功率与SOTA VLM主干策略相当，但推理频率高8到17倍，单次推理能耗低6到15倍。
◆ 证明在低层操作任务范围内，现代独立视觉/语言编码器加轻量动作头能取得更优的成功率—延迟—能耗权衡。</td></tr>
<tr><td>2026-09-16</td><td>RAFAIL: Relationship-Aware Failure Detection for Robotic Manipulation<br><a href='http://arxiv.org/pdf/2609.18324'>论文</a></td><td>RAFAIL提出一种面向机器人操作执行失败的检测框架，其核心是将失败判断转化为任务相关实体关系中的异常检测。它关注夹爪与物体、物体与目标等关系，把OOD检测聚焦到与失败相关的观测部分，从而降低任务无关场景变化的干扰。离线用VLM标注成功演示的任务进度和关系重要性，学习基于点云的关系表示；运行时用关系特定OOD检测器评估表示，并无需VLM推理预测关系重要性和进度，且不需失败数据，在三个真实任务上取得73.4%平衡准确率并优于最强OOD与不确定性基线。
◆ 以任务相关实体关系为检测单元，将失败建模为关系层面的OOD异常。
◆ 利用VLM离线标注任务进度与关系重要性，学习不依赖策略内部特征的点云关系表示。
◆ 在线避免VLM推理，无需失败数据，并在真实操作任务上超越OOD和不确定性基线。</td></tr>
<tr><td>2026-09-16</td><td>Fetch My Beer: Synthetic-to-real Hierarchical Policy for Smooth Pick-and-place<br><a href='http://arxiv.org/pdf/2609.18119'>论文</a> | <a href='https://fetch-my-beer.github.io/'>代码</a></td><td>本文提出面向液体容器稳定运输的合成到真实分层策略，在拾放过程中抑制晃动并防止溢出。  
◆ 构建可扩展合成数据管线：合成抓取、用视觉语言模型过滤不稳定姿态，并通过流体仿真验证运输轨迹。  
◆ 提出分层扩散控制器：高层将语言与视觉观测映射为SE(3)控制目标，低层在紧凑潜空间规划并解码密集动作块。  
◆ 通过潜空间规划与动作块解码实现高频平滑控制，满足轨迹级动态稳定需求。  
◆ 将物理验证的数据生成与分层扩散控制耦合，形成从仿真到真实部署的完整框架。  
实验表明，该系统在运输平滑性和动态稳定性上优于现有先进操作策略。</td></tr>
<tr><td>2026-09-15</td><td>HINT-Plan: Human Intention-Aware Robot Task Planning in Context-Rich Environments using Vision Language Models<br><a href='http://arxiv.org/pdf/2609.17771'>论文</a></td><td>HINT-Plan提出了一种将人类意图预测融入机器人任务规划的新方法，以弥补现有研究多关注低层避障、忽视人类高层行为影响的问题。  
◆ 它利用视觉语言模型从第三人称图像观测中预测人类高层意图，并将意图转化为目标状态，进而求解人机联合任务规划问题。  
◆ 它采用分层场景图作为富上下文环境的高层表示，并把环境拓扑与可操作知识翻译为形式化规划语言，以保证生成计划可执行。  
◆ 在照片级仿真中，HINT-Plan的人机联合任务规划总体成功率达到69.71%，比基线最高提升35.29%，同时减少了功能冲突。  
结果表明，显式地将推断出的人类意图纳入形式化多智能体任务规划，能够有效支持主动的人类感知机器人决策。</td></tr>
...
◆创新点二：揭示了VLM点嵌入普遍存在离群点与多模式问题，在约30%点中离群比例超过0.1。  
◆创新点三：量化了户外3DSG在导航目标检索中的可行性，成功率达约70%，同时指出路径效率和可穿越性缺陷。  
◆创新点四：发现区域级理解在自然环境中F1仅约0.359，并验证了多公里轨迹下地图可保持小于600MB的紧凑规模。</td></tr>
...
◆ 引入并发动作查询机制，在单次前向传播中因果关注多视图图像、历史动作与文本指令等多模态上下文，并将隐藏状态直接投影到预训练VQ-VAE潜空间

◆ 利用冻结解码器将连续潜变量转化为动作，消除量化误差并避免耗时的自回归生成过程

在nuScenes开放循环评测中，LaPla将长时域L2误差较当前最优VLA方法降低15.52%；在NVIDIA AlpaSim闭环仿真中将成功率提升33.34个百分点，同时显著降低推理延迟，验证了其在精度、效率与物理可行性方面的综合优势。</td></tr>
<tr><td>2026-09-03</td><td>FailBench: How Reliable are VLMs at Judging Robot Task Success?<br><a href='http://arxiv.org/pdf/2609.03611'>论文</a></td><td>本文核心贡献是提出了FailBench基准,用于评估视觉语言模型在机器人任务失败检测中的可靠性。该基准涵盖14个公开数据源、2197次操作尝试,其中75%为自然失败且包含大量跨领域真实场景,弥补了现有基准在跨域泛化评估上的不足。

◆ 创新点一:构建大规模跨域失败检测基准,涵盖真实与模拟数据且多数失败为自然产生,系统评估VLM在不同场景下的泛化能力。

◆ 创新点二:发现专门微调的失败检测模型反而劣于通用VLM及其预训练基线,揭示了任务专用微调的局限性。

◆ 创新点三:揭示了视觉证据需求对性能的关键影响,接触密集型装配任务上模型性能接近随机猜测,显示其细粒度感知能力不足。
...
◆利用视觉语言模型作为评估器，对回放视频进行成对偏好比较并聚合成策略排名，捕捉二元标签难以反映的执行质量差异。

◆提出可验证的评估协议，衡量评估管道在统计稳定性、减少硬件操作以及与人类偏好一致性方面的有效性。

◆在仿真和真实环境中的实验表明，该方法能够产出可靠稳定的策略结论，并与人类判断高度一致。

该工作将机器人评估从人工成功率统计推进到自动化、统计稳定且具有质量感知的新阶段。</td></tr>
...
◆提出KPA知识保持自适应框架，包含三个组件：结构化的感知到决策提示、基于SLERP的专家合并、以及面向决策状态的RegMoE（基于LoRA的混合专家模块）。

◆RegMoE能够根据不同驾驶决策状态分配轻量化的自适应容量，同时保留预训练模型的开世界知识。

在域内驾驶数据划分和CoLT-Drive上的实验表明，KPA在CoLT-Drive上达到60.8%的动作对准确率，显著优于Qwen3-VL-2B基线（50.3%）和LoRA SFT（32.4%），同时保持有竞争力的域内准确率。</td></tr>
<tr><td>2026-08-31</td><td>LightNav-0: Eliciting VLM Spatial Intelligence for Generalist Embodied Navigation<br><a href='http://arxiv.org/pdf/2608.30935'>论文</a></td><td>LightNav-0是一个面向通用具身导航的紧凑型模型，旨在直接激发预训练视觉语言模型（VLM）的空间智能，实现跨任务、环境和机器人形态的统一导航。其核心创新点如下：

◆ 提出统一token接口：双通道指向（dual-channel pointing）表达与场景和形态无关的空间意图，残差向量量化动作tokenizer将其映射为形态特定的精确轨迹，无需任务专用预测头。

◆ 设计时序感知的视觉历史压缩机制，并结合具身推理（ER）中训练、监督微调和强化学习的完整训练流程。

◆ 单一模型同时支持指令跟随、开放词汇目标导航和视觉跟踪等多种任务，训练数据涵盖2K+场景和4K+小时导航语料。

◆ 初始化模型LightNav-ER在8个具身推理基准上取得最高均值，LightNav-0在10个公开导航仿真环境中达到单目成功率最优。

◆ 真实场景实验验证了模型在不同机器人形态、多样场景以及静态和动态目标间的零样本泛化能力，证明紧凑VLM可作为通用具身导航的统一可迁移骨干网络。</td></tr>
<tr><td>2026-08-31</td><td>Lucida: Parse, Generate, and Place for Composable Real-to-Sim Scene Modeling<br><a href='http://arxiv.org/pdf/2608.30821'>论文</a></td><td>Lucida针对真实室内场景的可组合建模任务，将传统流程（解析、生成、放置）的要求重新分配，让每一步只依赖真实采集能稳定提供的信息，把精度推迟到流程末端而非起始处实现，从而在杂乱捕获条件下仍能完成高质量的Real-to-Sim重建。系统将视频解析为带有逐实例多视角证据的场景图，基于证据为每个实例生成完整资产，并使用VLM策略进行精确放置。

核心创新点如下：

◆ 将实例解析、资产生成与场景放置的精度要求进行重新分配，使各阶段仅依赖真实采集的可靠输入，最终精度通过端到端协同达到，而非在起始阶段强求。

◆ 提出基于多视角证据的场景图表示方法，从视频中提取每个实例的观测证据，为后续资产生成提供鲁棒的多视图信息基础。

◆ 设计GizmoAct放置策略，将对象放置建模为多轮GUI交互任务，VLM在闭环中操控物体的gizmo对齐工具并自主判断对齐完成时机。

◆ 在多个基准上取得显著提升，包括R2S-Scene场景级3D检测mAP较Boxer提升69%，CA-1M上ADD-SB@0.05从57.8%提升至83.4%，场景重建F-Score从SAM3D的0.794提升至0.924。</td></tr>
<tr><td>2026-08-31</td><td>Scaffolding Foundation Models into Physical-World Agents Pushes the Frontier of Long-Horizon Navigation<br><a href='http://arxiv.org/pdf/2608.30396'>论文</a></td><td>该论文针对长视野物理世界智能体中高层推理与底层导航难以兼顾的难题,提出NavMCP代理框架,将视觉语言模型的推理能力与导航基础模型(NFM)的执行能力进行有效耦合。VLM负责决定需要寻找何种证据、在何处搜索以及何时停止,NFM则将每个语义子目标落地为闭环导航行为,从而把孤立的导航回合转化为持续的具身交互。

◆ 提出NavMCP代理化脚手架框架,无需重新训练即可将VLM与NFM整合为长视野物理世界智能体。

◆ 设计意图、观测、记忆三大协作通道:意图通道将证据需求转化为导航调用,观测通道将回放转为基于源的轨迹证据,记忆通道跨调用积累发现、负面证据与未解决目标。

◆ 在HM-EQA、MT-HM3D、EXPRESS-Bench等具身问答基准上取得最优结果,在HM-EQA上以相同骨干较回合式接口提升14.9个百分点。

◆ 在Unitree Go2真机实验中达到78.3%成功率,随任务视野延长,相对最强基线的优势从10点扩大至45点,验证了持续任务级推理在长视野导航中的关键作用。</td></tr>
<tr><td>2026-08-31</td><td>CometVLA: Co-Training on an Embodied Data Pyramid towards Physical Understanding<br><a href='http://arxiv.org/pdf/2608.30289'>论文</a></td><td>CometVLA针对视觉-语言-动作模型在需要物理常识的操作任务中表现脆弱的问题,系统性地弥合了物理理解与机器人动作生成之间的鸿沟。研究者首先指出,现有物理VQA数据通常脱离具身场景且与机器人动作域不对齐,自我中心视频仅被用作辅助预训练,且更强的VLM物理理解是否真正有助于下游动作生成尚不明确。为此,论文构建了与机器人动作数据和本体严格对齐的具身物理VQA语料库CometData和基准CometBench,并提出在具身数据金字塔(遥操作、仿真、自我中心轨迹、VQA)上进行协同训练。核心创新点如下:

◆ 构建CometData与CometBench,提出首个与机器人动作数据及本体严格对齐的具身物理VQA语料库与评测基准
◆ 设计Global Action Prior (GAP) tokens作为紧凑可学习瓶颈,隔离任务无关的运动规律,避免物理常识注入破坏预训练VLM骨干
◆ 在涵盖遥操作、仿真、自我中心轨迹与VQA的具身数据金字塔上进行协同训练,统一物理理解与动作生成
◆ 通过相关性分析实证证明,VLM在CometBench上表现越强,VLA操作成功率越高,确立物理理解预训练对下游动作的真正价值

在真实操作任务和RoboTwin仿真中,CometVLA均稳定超越强VLA基线,验证了所提方法的有效性。</td></tr>
<tr><td>2026-08-29</td><td>AdaVLA: Adaptive Step Flow Matching for Training-free Acceleration of Vision-Language-Action Models<br><a href='http://arxiv.org/pdf/2608.29208'>论文</a></td><td>AdaVLA针对视觉-语言-动作模型的计算开销大、难以在边缘设备实时部署的问题，提出了一种无需训练的在线自适应加速框架。现有加速方法通常依赖微调或访问训练数据，且主要关注VLM推理部分，未能针对流匹配中固有的迭代ODE求解过程进行优化。

◆ 提出基于流匹配轨迹曲率的新型置信度度量指标，能够在线量化动作生成的置信水平。

◆ 利用该指标动态减少推理步数，并通过高效计算的重要性评估自适应调整MLP剪枝比例。

◆ 整个框架无需任何训练数据或微调过程，完全在线运行，保护数据隐私。

◆ 在LIBERO基准和Jetson AGX Orin设备上，π_0.5和X-VLA分别实现1.87倍和2.24倍加速，成功率几乎无损，并在真实机器人任务中通过SmolVLA验证了鲁棒性。</td></tr>
<tr><td>2026-08-29</td><td>CGFM-Nav: Cognitive Graph-Field Memory for Semantic-Guided Lifelong Multimodal Embodied Navigation<br><a href='http://arxiv.org/pdf/2608.29114'>论文</a></td><td>本文针对视觉-语言导航中环境表示难以同时支持显式语义记忆和连续探索引导的挑战,提出了认知图场记忆CGFM及其导航框架CGFM-Nav。CGFM将物体、空间关系和视觉观测组织为多模态场景图,实现目标检索和长时序推理;在无可靠目标匹配时,将图结构证据投影为目标条件语义前沿场,引导智能体向语义有前景的边界和区域探索。CGFM-Nav以基础模型Qwen3-VL-8B为骨干,融合任务相关子图选择、VLM推理与验证反馈形成闭环决策。在GOAT-Bench上的实验表明,总体成功率从53.2%提升至63.0%,SPL从30.0%提升至39.6%,验证了显式语义记忆与语义引导探索结合的有效性。

◆ 提出认知图场记忆CGFM,将显式关系记忆与连续空间直觉耦合,统一支持语义记忆和探索引导。
◆ 构建多模态场景图组织物体、空间关系和视觉观测,实现跨任务目标检索与长时序推理。
◆ 设计目标条件语义前沿场,在无可靠目标匹配时引导探索朝向语义有前景区域。
◆ 提出CGFM-Nav框架,集成子图选择、VLM推理与验证反馈形成闭环决策机制。
◆ 在GOAT-Bench上以Qwen3-VL-8B为骨干实现显著性能提升,验证方法有效性。</td></tr>
<tr><td>2026-08-28</td><td>RoboPhys-3D: A Comprehensive Embodied World Model Evaluation via 3D Reconstruction<br><a href='http://arxiv.org/pdf/2608.28718'>论文</a></td><td>本文针对视频世界模型在具身智能中缺乏统一三维评估标准的问题，提出了RoboPhys-3D基准，核心贡献与创新点如下：

该基准基于RoboTwin 2.0构建，包含50个操作任务、5000个回合和25000段多视角真值视频，覆盖四种评估场景。

◆创新点一：将生成视频与真值视频通过同一三维重建管线处理，从而分离出重建误差与生成误差，实现真正的3D几何一致性评估。

◆创新点二：提出50个互补指标，涵盖像素保真度、三维几何一致性、状态级理解和任务级完备性四个层次，评估维度全面系统。
...
◆ 将目标识别与吸盘抓取点选择解耦为两个独立模块，提升系统灵活性与可扩展性
◆ 提出融合表面平整度与法向量对齐度的几何评分策略，专门针对变形物体的吸盘吸附需求
◆ 系统比较了K近邻PCA、Sobel叉积和RANSAC平面拟合三种几何方法在曲面评估中的性能表现

在真实机器人平台上对三种变形程度、35个杂乱场景的实验表明，单物体抓取成功率达88.2%，杂乱环境下的端到端回收成功率为72.6%，验证了该方案在可回收垃圾分拣任务中的有效性。</td></tr>
<tr><td>2026-08-28</td><td>Plan Along the Way: Event-Triggered Foundation-Model Planning for TAMP Execution in Partially Observable Manipulation<br><a href='http://arxiv.org/pdf/2608.28075'>论文</a></td><td>本文针对部分可观测环境下的机器人操作任务，提出了一种名为ROBUST TAMP的模块化LLM/VLM引导的反应式任务与运动规划框架。其核心思路是将基础模型规划器严格限制在当前可见的关系化场景状态范围内，并通过对可执行接口的严格校验来确保生成的任务级动作可落地执行。

◆ 将基础模型规划器约束在当前可见的场景状态上，强制遵循&quot;所见即所规划&quot;原则，避免对未观测信息的幻觉式推理。

◆ 引入严格的可执行接口验证机制，仅接受符合规范的底层动作，拒绝不合规的高层指令，从而保证规划的可执行性。

◆ 将目标对象发现事件与执行失败事件统一建模为结构化的重规划触发事件，并结合已完成动作历史进行上下文感知的再规划。

◆ 在稳定执行窗口后重建可见场景状态，实现&quot;沿路规划&quot;的事件驱动反应式闭环，而非依赖预先完整的世界模型。

在六类RLBench/CoppeliaSim厨房与烧烤场景中，系统对隐藏对象、非目标物体干扰、铰接容器和时序操作均表现出鲁棒性，并系统比较了不同规模文本LLM与VLM规划器在成功率、部分目标完成、推理代价等方面的差异。</td></tr>
...
<tr><td>2026-08-24</td><td>OptiSight: Bridging Semantic Reasoning and Geometric Control for Embodied Navigation<br><a href='http://arxiv.org/pdf/2608.23354'>论文</a> | <a href='https://github.com/avanalperen/OptiSight-Python-Multimodal-CoT-for-Visual-Reasoning'>代码</a></td><td>OptiSight提出了一种混合框架,将视觉语言模型的语义推理能力与确定性视觉伺服控制相结合,用于解决室内自主导航中语义理解与几何控制脱节的问题。该系统采用有限状态链式思维架构,仅在关键决策点调用VLM,从而显著降低了计算开销,连续导航则由几何控制模块完成。

◆基于Grounded-SAM实现开放词汇目标定位,无需依赖密集建图即可将视觉观测通过相机投影几何直接转换为导航指令。

◆创新性地将VLM语义推理与视觉伺服几何控制解耦,采用有限状态链式思维在关键节点调度VLM,兼顾推理深度与实时性能。

◆在仅8GB显存预算下实现零样本导航,在AI Habitat中验证了对障碍物规避和语义模糊等复杂场景的可靠性。</td></tr>
...
<tr><td>2026-08-23</td><td>OmniCAD: A Large-Scale Benchmark for 3D Spatial Reasoning in Robotics Assemblies<br><a href='http://arxiv.org/pdf/2608.22637'>论文</a></td><td>本文提出了OmniCAD,一个面向工业装配场景的大规模3D空间推理基准,旨在系统评估视觉语言模型在复杂机械装配任务中的能力。

该基准包含25,000个机械装配模型,平均每个装配体由12个零件组成,涵盖21种配合关系,涉及机器人机构、汽车、航空航天和农业机械等多种工业系统,并提供人工校验的真值3D模型及20个视角的渲染图像。

OmniCAD从三个维度评估模型能力:零件级空间推理要求预测部件位置和姿态;零件间关系推理要求识别配合关系和装配约束;工具增强智能体推理则要求模型迭代选择视角、检查视觉证据并优化预测结果。

实验表明,当前视觉语言模型在工业装配推理中表现不佳,常出现位姿不准、配合关系无效、零件相互穿透等问题,且性能随装配复杂度增加而显著下降。

◆构建了首个大规模工业装配3D空间推理基准,涵盖多样化机械系统与丰富配合关系类型

◆提出了三维评估框架,系统性覆盖零件定位、配合关系识别与工具增强迭代推理三个层次

◆开源了基准数据、评估代码及工具接口,为物理有效的可扩展3D装配推理研究提供基础设施

◆揭示了现有视觉语言模型在工业装配场景中的具体缺陷,为未来模型改进指明了方向...[摘要不完整，待更新]</td></tr>
<tr><td>2026-08-23</td><td>EMPIRE: Explicit Manipulation Planning as a Learnable Intermediate Representation for Egocentric Hand-Motion Forecasting<br><a href='http://arxiv.org/pdf/2608.22449'>论文</a> | <a href='https://github.com/wangwen-banban/EMPIRE'>代码</a></td><td>本文研究从自我中心视角预测灵巧双手动作的任务。现有基于VLM的方法直接将观测映射到未来动作,忽略了手物交互的潜在操作过程,且端到端优化会使动作生成梯度干扰预学习的操作表征。EMPIRE提出两阶段框架,将显式操作规划作为可学习的中间表征。第一阶段从多模态上下文中学习手物交互进程的操作规划,第二阶段的动作生成器以冻结的规划表征为条件合成未来双手动作。论文还构建了EMPIRE-651K数据集,包含65万余训练窗口,覆盖111个任务,每个样本配有显式的每手操作规划。实验表明该方法在MPJPE和手指相对误差上均达到当前最优。

◆ 提出将显式操作规划作为可学习中间表征,显式建模手物交互进程
◆ 设计两阶段解耦训练框架,冻结规划器表征以避免动作生成梯度干扰
◆ 构建大规模EMPIRE-651K数据集,提供细粒度每手操作规划标注...[摘要不完整，待更新]</td></tr>
<tr><td>2026-08-22</td><td>Ludi${}_{\scriptscriptstyle 0.1}$: An Agentic System for Socially Intelligent Robots<br><a href='http://arxiv.org/pdf/2608.22035'>论文</a></td><td>Ludi 0.1 是一个面向社交智能机器人的代理式系统，旨在实现流畅的人机协作而非仅仅执行孤立命令。该系统的核心是一个经过微调的视觉语言模型，专门基于多轮交互轨迹进行训练，涵盖模糊请求、澄清、纠正、中断、社交与任务混合对话以及多步任务等复杂场景。系统通过一个定制化的框架管理模型与工具的交互循环，并配合专门的导航和操控策略来执行物理技能。Ludi 0.1 不仅展示了当前实现自然人机协作的可行路径，还生成了高质量的多模态交互轨迹数据，为未来开发更深度集成的机器人与人交互基础模型提供了资源。

◆ 构建了集成交互式语音、多模态推理、记忆、导航和操控的代理式机器人系统框架
◆ 提出基于多样化多轮交互轨迹微调视觉语言模型的方法，覆盖模糊性、纠正、中断等真实交互场景
◆ 设计了专用框架来管理模型与工具的交互循环，实现高层决策与底层技能策略的协同
◆ 通过生成多模态交互轨迹数据，为未来人机交互基础模型的训练提供可扩展的数据资源...[摘要不完整，待更新]</td></tr>
<tr><td>2026-08-22</td><td>GuardianBench: A Same-Scene Instruction-Contrastive Benchmark for Latent Contextual Risk in Embodied AI<br><a href='http://arxiv.org/pdf/2608.21928'>论文</a></td><td>本文针对具身AI中指令与场景组合才显现的&quot;潜在上下文风险&quot;展开研究,指出当前安全评估多关注视觉场景变化或执行过程动态,却忽略了固定场景下仅改变指令所带来的风险。

◆提出GuardianBench基准,基于国际安全标准构建3024条指令-场景样本,采用同场景Safe/Unsafe对比配对设计,系统隔离潜在上下文风险。

◆发现主流视觉语言模型存在&quot;指令不敏感判决&quot;缺陷,在同一场景下对安全与不安全指令均倾向于批准,平均配对准确率仅24.1%。

◆通过系统化推理审计定位主导失败原因,揭示模型无法有效绑定区分安全与不安全组合的关键指令线索。

◆提出VLOS(判决对数几率监督)轻量级后训练方法,在开源模型上显著提升安全推理性能,为基于判决的校准提供了可行路径。

该工作从任务形式化、对抗式基准构建、配对与推理级失败诊断到判决校准,形成了完整的潜在风险评估与改进框架。</td></tr>
...
<tr><td>2026-08-13</td><td>MLLM-Routed Heterogeneous Ensembles for Robust Cross-Dataset Image Classification<br><a href='http://arxiv.org/pdf/2608.13463'>论文</a></td><td>论文针对图像分类模型跨域跨难度泛化能力不足的问题，提出了ARMDIL自适应路由框架。核心思路是使用多模态大语言模型作为智能代理，动态将每张图像分发到最合适的视觉骨干网络，从而融合不同架构的互补优势。

◆ 构建异构视觉模型集成，融合ResNet卷积网络、SSL自监督学习器和VLM视觉语言模型三类架构
◆ 设计多数据集统一标签空间，使异构模型能够在共享语义框架下协同完成跨域分类任务
◆ 实现基于提示词的灵活更新机制，仅通过修改提示即可集成新信息而无需重新训练模型
◆ 利用MLLM的自然语言推理链提供路由决策依据，增强系统决策过程的可解释性

实验结果表明ARMDIL在跨数据集分类任务上与专用训练的路由器性能相当，同时在适应性、鲁棒性和可解释性方面展现出明显优势，为通用视觉系统提供了更可靠的解决方案。</td></tr>
...
<tr><td>2026-07-17</td><td>DPNeXt: A Lightweight Multi-Scale Feature Fusion Framework for Efficient ViT-Based Multi-Task Dense Prediction<br><a href='http://arxiv.org/pdf/2607.16012'>论文</a> | <a href='https://github.com/kangjehun/DPNeXt'>代码</a></td><td>本文针对机器人感知系统中多任务密集预测(语义分割与深度估计)的解码瓶颈,提出轻量化框架DPNeXt,在多个基准上取得领先性能并显著降低参数量。

◆ 提出DPNeXt解码器,采用双路深度可分离倒置瓶颈结构,实现以融合为中心的解码与独立任务模块化,高效利用冻结的视觉基础模型特征。

◆ 设计多任务边界引导(MTBG)策略,通过对称的边界聚焦监督缓解任务间负迁移,无需额外标注或推理开销即可增强几何一致性。

实验表明,DPNeXt-S在Cityscapes上超越此前最优多任务模型,DPNeXt-B在NYUv2上同时取得最佳分割与深度估计结果。

相比标准DPT,DPNeXt-S可训练参数量减少78.6%,并在资源受限的笔记本硬件上实现最快的推理速度。</td></tr>
...
<tr><td>2026-05-31</td><td>One Channel to Rule Them All: Rethinking Input Representation for Visual Place Recognition<br><a href='http://arxiv.org/pdf/2606.00936'>论文</a></td><td>本文挑战了视觉地点识别过度依赖RGB输入的默认假设，系统考察色彩信息在训练方式、模型架构与标准基准中面对真实外观变化时的作用。
◆ 发现灰度输入总体可匹配RGB性能，并在严重光照、天气、季节与场景变化下常优于RGB，而颜色仅在存在持久且具判别性的色度线索时带来明显增益。
◆ 在所选基准上，全灰度训练的MixVPR平均Recall@1达82.4%，略高于RGB版本的81.2%。
◆ 某些轻量灰度变体参数少60%仍可超过更重的RGB模型，显示去除颜色可降低模型冗余。
◆ 灰度还带来存储、带宽与资源受限系统部署上的实际优势。
结论是：在跨光照、天气、季节和场景变化的全球VPR中，颜色贡献有限，灰度单独已足以实现可靠地点识别。</td></tr>
