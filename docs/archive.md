...
<tr><td>2026-07-10</td><td>Glob3R: Global Structure-from-Motion with 3D Foundation Models<br><a href='http://arxiv.org/pdf/2607.09225'>论文</a> | <a href='https://junyuandeng.github.io/Glob3r'>代码</a></td><td>Glob3R提出了一种基于3D基础模型的全局SfM重建方法,旨在解决现有feed-forward方法(如VGGT)在长序列或大规模无序图像集上精度不足、容易产生漂移和不一致的问题。其核心思想是显式优化前馈几何预测结果,冻结Pi3X骨干网络并附加轻量级密集匹配头,将参考帧与邻域视图间的图像warp转化为稀疏可靠的多视图特征轨迹,作为全局优化的对应约束。

◆ 在冻结的Pi3X骨干网络上增加轻量级密集匹配头,预测参考帧与邻域视图间的图像warp,并转化为稀疏多视图特征轨迹提供全局对应约束
◆ 提出基于关键帧的滑动窗口关联策略,通过重叠窗口传播轨迹与相对位姿,实现可扩展的大规模重建
◆ 引入全局运动平均与束调整对相机位姿进行全局精修,消除尺度不一致并恢复稠密场景几何
◆ 在室内、室外、驾驶场景及无序SfM基准上均优于前馈基线与近期可扩展方法,同时比经典SfM更鲁棒,并能提升下游神经渲染质量,验证了基础模型先验与全局几何优化相结合的有效性...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-06-22</td><td>G-MASt3R-SfM: Graph-based View Pruning and Multi-stage Optimization for Robust SfM<br><a href='http://arxiv.org/pdf/2606.22856'>论文</a></td><td>本文针对基于MASt3R的Structure from Motion (SfM)方法在处理非重叠图像对时因错误匹配导致位姿估计精度下降的问题,提出了G-MASt3R-SfM方法。该方法在ETH3D数据集上取得了相机位姿估计和三维重建的先进精度,有效抑制了异常值引起的噪声。

◆ Graph-based View Pruning (GVP)模块:基于匹配置信度构建场景图,通过几何方法对离群视图进行剪枝,过滤掉非重叠或匹配不可靠的图像对,从源头减少错误匹配的干扰。

◆ Multi-Stage Optimization (MSO)模块:采用多阶段渐进式优化策略,将相机参数优化从局部一致性逐步扩展到全局一致性,有效避免错误匹配直接进入全局优化导致的误差累积和精度退化。

整体而言,本文通过&quot;图结构剪枝+多阶段优化&quot;的协同设计,在保留MASt3R强匹配能力优势的同时,显著提升了SfM管线对噪声匹配的鲁棒性,为复杂场景下的多视图三维重建提供了更可靠的解决方案。</td></tr>
