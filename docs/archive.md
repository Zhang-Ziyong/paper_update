...
<tr><td>2026-06-08</td><td>Submodular Optimization with Applications to Decision and Control<br><a href='http://arxiv.org/pdf/2606.10192'>论文</a></td><td>这篇论文是一篇关于子模优化及其在决策与控制中应用的综述,系统梳理了该领域的理论、算法与应用进展。论文聚焦于具有&quot;边际收益递减&quot;性质的子模集函数框架,阐述了虽然精确最大化是NP难问题,但贪心算法可为单调目标提供常数因子近似保证,随机化贪心变体则可将此类保证扩展至非单调情形。作者从结构性质、约束类型、近似算法到具体应用场景进行了全面回顾,并特别关注控制论中典型目标函数的子模性判定。

核心贡献和创新点如下:

◆ 系统整合了子模优化的最新理论成果,包括曲率和子模性比率等实例相关的精细化分析工具,并给出了最新的近似比和困难性结果。

◆ 全面综述了在传感器调度、多智能体协调、鲁棒子模优化、领导者-跟随者系统、分布式优化、博弈论、资源分配、社交网络及信息路径规划等控制领域的应用,体系较为完整。

◆ 强调实际可实现的基于贪心的算法,并通过曲率与子模性比率提供针对具体问题实例的精细化改进方案。

◆ 系统梳理了多种约束族(拟阵、背包约束、p-系统)下的优化方法,为实际应用中的约束建模提供了参考。

◆ 对控制论中典型目标函数的子模性进行了清晰判定:可控性Gramian的对数行列式和秩、Kalman滤波器信息矩阵的对数行列式具有子模性,而稳态Kalman误差协方差和基于逆Gramian的平均控制能量则不具备子模或超模性,该结论澄清了应用边界。

◆ 提炼出贯穿多个方向的开放性问题,为后续研究指明了方向。</td></tr>
<tr><td>2026-06-08</td><td>FlexPath: Learned Semantic Path Priors for Image-Based Planning<br><a href='http://arxiv.org/pdf/2606.10167'>论文</a> | <a href='https://github.com/FraunhoferIVI/FlexPath'>代码</a></td><td>论文核心贡献总结:

FlexPath提出了一个两阶段的图像化路径规划框架,通过解耦可行性学习与偏好适应,解决了现有基于学习的路径规划方法被最短路径目标所束缚、难以灵活适配不同规划准则的问题。该方法在TMP数据集上相比当前最优方法TransPath减少了14.3%的搜索开销,同时在多个未见过的领域展现出零样本泛化能力,并能扩展至障碍物避让、语义感知避让以及路径点引导等多种任务。

◆ 提出两阶段解耦框架,第一阶段通过模仿学习从视觉地图中获得任务无关的可行路径空间先验,第二阶段再针对具体任务进行目标级适配,避免重新学习路径结构。

◆ 设计了可微的路径形状目标函数(PSOs),使单一预训练模型可灵活适应多种规划准则,无需为每个新目标重新训练网络。

◆ 在最短路径规划中不仅降低搜索成本,还能找到平均代价更低的路径,并在三个未见领域实现强零样本泛化。

◆ 在障碍物避让任务中,以最小净空距离2实现了96.8%的完全避障率,同时保持较低的搜索代价。

◆ 框架在推理阶段可与经典搜索算法兼容,实现了学习方法与传统规划器的有效融合。</td></tr>
<tr><td>2026-06-08</td><td>Uncertainty-Aware Motion Planning for Autonomous Driving in Mixed Traffic Environment<br><a href='http://arxiv.org/pdf/2606.09958'>论文</a></td><td>这篇论文针对混合交通环境下自动驾驶的运动规划问题,指出现有基于强化学习的方法将预测的人类驾驶意图直接作为确定性状态纳入观测,忽略了由行为多样性、感知噪声和部分可观测性带来的固有不确定性,从而导致决策不安全。为此,作者提出了不确定性感知运动规划框架UAMP,通过量化人类意图预测的不确定性并将其融入价值学习过程,实现更安全、舒适的自动驾驶决策。在多种混合交通场景下的实验表明,UAMP在保持交通效率的同时显著提升了安全性和驾驶舒适度。

核心创新点如下:

◆ 提出了UAMP框架,首次将人类意图预测的不确定性显式建模并融入自动驾驶强化学习决策过程,解决了将不确定预测当作确定性状态所导致的不安全决策问题。

◆ 设计了邻近感知的不确定性估计器,根据车辆间的交互关系量化意图预测的不确定性,并构建了周围人类驾驶车辆的联合意图分布,捕捉交互条件下的不确定性集合。

◆ 提出了不确定性校准的价值学习方法UCVL,在不确定性集合内修正因直接使用不确定意图预测而产生的价值函数学习偏差,使策略学习对预测噪声更具鲁棒性。

◆ 在多种混合交通场景中的实验验证表明,该方法在安全性与舒适度上显著优于现有方法,同时不牺牲交通效率。</td></tr>
