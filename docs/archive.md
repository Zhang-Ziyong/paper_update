...
<tr><td>2026-07-01</td><td>SE(2) Navigation Mesh<br><a href='http://arxiv.org/pdf/2607.01454'>论文</a></td><td>地面机器人在复杂多层环境中的全局导航要求可达区域表示准确且路径规划高效，但传统点云与体素图缺乏显式表面结构，稠密网格寻路代价高，而经典导航网格假设偏航不变性，难以适用于受限空间中的非圆形机器人。本文提出SE(2) Navigation Mesh，关键创新如下：

◆ 提出SE(2) NavMesh表示，基于足部轮廓掩码评估可达性，构建跨偏航层级的图结构并显式编码平移与旋转连接性，突破传统NavMesh偏航不变性的限制。

◆ 设计A*-String Pulling-A*（ASA）分层路径规划策略，分阶段优化位置与朝向。

◆ 提出从流式点云在线增量构建SE(2) NavMesh的方法，可与几何重建过程并发执行。

实验显示，该方法比传统NavMesh多捕获50%以上可达区域，在受限环境中优于基于采样的基线方法，并在真实机器人上得到实时导航验证。</td></tr>
...
<tr><td>2026-06-03</td><td>A Geometry-Informed Computer Vision Method for Detecting and Examining Overtaking Vehicles From A Bicycle<br><a href='http://arxiv.org/pdf/2606.23699'>论文</a></td><td>本文针对自行车仪器化研究中人工逐帧标注的瓶颈，提出了一种基于几何信息约束的计算机视觉流水线，仅依靠单台自行车后置摄像头即可自动检测超车事件，无需多传感器配置或相机标定。该方法将RT-DETR目标检测与ByteTrack多目标跟踪相结合，通过基于透视投影原理的方位角趋势、目标表观尺寸增长和空间确认三阶段几何验证模块，有效过滤误检。

◆核心创新：基于透视投影几何的轻量级验证模块，无需相机内外参即可从单目视频自动判定超车事件。
◆检测性能优异：在密歇根州安娜堡城市道路的315个真实超车事件上达到97.8%召回率且零误报。
◆预警时效显著：平均在车辆通过前2.44秒识别超车意图，84.1%的事件超过1.5秒人类反应阈值，具备主动预警可行性。
◆横向距离无需标定即可估计：通过包围盒几何特征实现13至14厘米的平均绝对误差，足以区分近距离与正常通过事件用于安全分级，揭示了33.3%的车辆未达到5英尺安全横向距离阈值。</td></tr>
