...
<ol>
<li><a href='#slam'>SLAM (159篇)</a></li>
<li><a href='#sfm'>SFM (77篇)</a></li>
<li><a href='#image-matching'>Image Matching (32篇)</a></li>
<li><a href='#obstacle-avoidance'>Obstacle Avoidance (187篇)</a></li>
<li><a href='#navigation'>Navigation (205篇)</a></li>
<li><a href='#motion-planning'>Motion Planning (304篇)</a></li>
<li><a href='#sensor-calibration'>Sensor Calibration (49篇)</a></li>
<li><a href='#sensor-undistortion'>Sensor Undistortion (103篇)</a></li>
</ol>
</details>

<h2 id='slam'>SLAM (159篇)</h2>
...
<tr><td>2026-07-10</td><td>Empirical Pedestrian Safety Assessment in a Mobile Robot Using a Predictive Social Force Model<br><a href='http://arxiv.org/pdf/2607.09192'>论文</a></td><td>本文针对移动机器人在人行道与行人共享场景下的安全导航问题，提出了预测型社会力模型PSFM和PTSFM，通过在有限时间范围内集成预测的社会力向量，扩展了传统SFM和基于PTTC的TSFM。研究在非完整约束移动机器人上实现了四种模型（SFM、TSFM、PSFM、PTSFM），并组织志愿者进行面对式交互实验，从客观指标（最小PTTC、平均速度、最小距离、横向距离、最大轨迹曲率）和主观指标（Likert量表舒适度、平滑度等）进行系统评估。实验结果证实PTTC的融入显著提升了安全指标，而预测机制的贡献较为有限，在单行人场景中未带来显著额外收益，主观评分也无统计学显著差异。

◆ 提出预测型社会力模型PSFM和预测型PTTC社会力模型PTSFM，在有限时间范围内融合预测社会力向量
◆ 首次在非完整约束移动机器人上对SFM、TSFM、PSFM、PTSFM四种模型进行系统性实机对比实验
◆ 建立了融合客观安全指标与Likert主观评分的综合评估框架
◆ 实证验证PTTC增强可提升安全性，而预测扩展在单行人场景下收益有限...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-06-22</td><td>G-MASt3R-SfM: Graph-based View Pruning and Multi-stage Optimization for Robust SfM<br><a href='http://arxiv.org/pdf/2606.22856'>论文</a></td><td>该论文针对现有MASt3R-SfM方法在处理非重叠图像对时易产生错误匹配,进而导致位姿估计精度显著下降的问题,提出了名为G-MASt3R-SfM的鲁棒性增强型SfM方案。该方法通过构建基于匹配置信度的场景图,对离群视图进行几何剪枝,从源头过滤不可靠的匹配。在此基础上,设计多阶段优化流程,将相机参数优化从局部一致性逐步推进到全局一致性,实现相机位姿的渐进式精炼。在ETH3D数据集上的实验结果表明,该方法在相机位姿估计和三维重建精度上均达到当前最优水平,有效抑制了离群噪声对重建结果的干扰。

创新点:
◆ 图基视图剪枝模块(GVP):依据匹配置信度构建场景图,通过几何方式识别并剔除离群视图
◆ 多阶段优化模块(MSO):采用从局部一致性到全局一致性的渐进式策略,逐阶段精炼相机参数...[摘要不完整，待更新]</td></tr>
...
<h2 id='obstacle-avoidance'>Obstacle Avoidance (187篇)</h2>
...
<h2 id='navigation'>Navigation (205篇)</h2>
...
<h2 id='motion-planning'>Motion Planning (304篇)</h2>
...
<tr><td>2026-08-06</td><td>Plan-and-Avoid: Real-Time Aircraft Trajectory Coordination in a Multi-Agent Environment<br><a href='http://arxiv.org/pdf/2608.06648'>论文</a></td><td>本文提出了一种实时Plan-and-Avoid（PAA）框架，用于在多智能体空域环境中围绕优先航迹进行协作式飞行协调。PAA框架首先预测与周围交通的不确定性感知间隔违例，当优先航迹自身无法维持安全间隔时，会向附近航空器发出考虑飞行器性能约束的单向避让建议以修改其航迹，从而保证所有航空器保持well-clear间隔。

◆ 提出了统一的多智能体空域协调框架，可应用于任何声明的优先航迹（如应急着陆、任务关键或受限机动性场景），具有通用性。

◆ 集成了不确定性感知的安全间隔违例预测和考虑飞行器性能约束的单向避让建议生成机制，实现实时协作式冲突解决。

◆ 框架在个人计算机上实现最坏端到端响应时间仅5.7秒，满足RTCA DO-365标准中35秒时间阈值的建议占比达93.5%。

◆ 使用华盛顿特区真实ADS-B交通数据，验证了900余次迫降场景和575个唯一冲突的可行性，具有实际运行价值。</td></tr>
...
<h2 id='sensor-undistortion'>Sensor Undistortion (103篇)</h2>
...
<tr><td>2026-07-20</td><td>Does Robust VIO Need More Learning? Geometry-Verified Visual Measurements under Distribution Shift<br><a href='http://arxiv.org/pdf/2607.17956'>论文</a></td><td>本文针对分布偏移下视觉惯性里程计(VIO)的鲁棒性问题,提出了一种&quot;最小化学习&quot;的立体VIO框架,将学习严格限制在视觉测量生成阶段,而时间跟踪、几何验证与状态估计均保持显式方法。

◆ 创新性地质疑&quot;学习越多越鲁棒&quot;的传统思路,仅用SEA-RAFT生成稠密立体对应与不确定性,保留显式几何与估计模块,实现学习与几何的轻量化融合

◆ 设计基于预测不确定性筛选与立体极线一致性双重校验的稠密光流过滤机制,通过不确定性加权重投影因子嵌入滑窗立体惯性估计器

◆ 将学习得到的不确定性贯穿传播至立体三角化,生成各向异性3D高斯地图,实现测量、估计与建图全链路的不确定性一致性

在EuRoC、VIODE和4Seasons数据集上,面对运动模糊、动态场景、光照变化及室内外大跨度分布偏移均取得稳定准确估计;消融实验表明,单独使用学习光流并不充分,真正的鲁棒性提升来源于学习对应、几何验证与不确定性加权三者的协同。</td></tr>
<tr><td>2026-07-20</td><td>SLAM in Low-Light Environments: Project Report<br><a href='http://arxiv.org/pdf/2607.17699'>论文</a></td><td>本文系统性地评估了六种代表性SLAM系统（ORB-SLAM3、DSO、Kimera-VIO、OpenVINS、DPVO、DPV-SLAM）在低光照场景下的表现，填补了该领域缺乏标准化基准评测的空白。研究采用LaMARia数据集中五条光照难度递进的序列，从绝对轨迹误差、相对位姿误差和控制点召回率三个维度进行全面比较，揭示了不同范式系统在暗光环境下的性能分化。

◆ 构建了一个覆盖特征点法、直接法、滤波法和学习法四大范式的低光SLAM统一评测框架

◆ 首次在LaMARia低光照序列上对六类主流系统进行横向对比，量化分析了惯性融合与全局优化对暗光跟踪的关键作用

◆ 通过实验得出明确结论：纯视觉SLAM在低光下保持稳定跟踪必须同时具备IMU融合和回环/全局优化能力，传统单目和滤波方法在困难低光序列上普遍失败或发散

◆ 指出了未来改进方向，包括面向低光场景的专用学习型前端或引入多模态互补传感，为后续研究提供了清晰的技术路线指引...[摘要不完整，待更新]</td></tr>
