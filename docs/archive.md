...
<tr><td>2026-07-07</td><td>CILC: Cryptographically-secure Inter-agent Loop Closure Candidate Detection for Multi-Agent Collaborative SLAM<br><a href='http://arxiv.org/pdf/2607.06700'>论文</a></td><td>该论文针对多智能体协作SLAM中全局描述符明文交换导致的隐私泄露问题，提出CILC系统。论文首先揭示了被攻陷的智能体可从公开的全局描述符中重构其他智能体图像与轨迹的具体威胁。在此基础上提出首个基于安全多方计算（SMPC）的智能体间回环候选检测方案，无需明文交换GD即可完成相似度比较。该方案仅加密隐私敏感但计算轻量的GD比较环节，规避了对整个CSLAM管道的复杂保护，实现了隐私性与系统开销的良好权衡。仿真与硬件实验证明系统在视觉和LiDAR多模态描述符下仍保持实时性与通信可行性，并能有效抑制恶意智能体的信息窃取。

◆ 首次将SMPC引入多智能体CSLAM回环检测，填补隐私保护研究空白
◆ 选择性保护策略：仅加密GD相似度比较这一关键轻量步骤，兼顾隐私与效率
◆ 多模态兼容，支持视觉与LiDAR描述符，并通过仿真与硬件双重实验验证实时可行性...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-06-06</td><td>SynthICL: Scalable In-context Imitation Learning with Synthetic Data<br><a href='http://arxiv.org/pdf/2606.08154'>论文</a></td><td>SynthICL提出了一种可扩展的上下文模仿学习框架,仅使用RGB合成数据训练机器人策略,无需深度传感、相机标定或真实世界数据。该方法通过构建高保真数据生成流水线,合成了适用于上下文学习的演示数据,并基于此训练了一个流匹配Transformer策略。模型在训练时还加入了下一子目标图像预测任务,使控制更加精确且具有视觉依据。在16个未见过的真实操作任务上,SynthICL仅凭测试时提供的一条示范就达到了79%的平均成功率,优于现有方法。

◆ 提出纯RGB合成数据驱动的上下文模仿学习框架,摆脱对深度信息和真实数据的依赖
◆ 构建高保真合成数据生成流水线,支持大规模可扩展训练
◆ 采用流匹配Transformer作为策略骨干,提升动作生成的精度与质量
◆ 引入子目标图像预测作为辅助任务,增强视觉引导与控制粒度
◆ 在16个真实未见任务上仅用一条示范即达到79%成功率,显著超越现有方法...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-06-30</td><td>Distortion-Corrected Diffusion MRI Using Rotated-View EPI and Joint Field-Map/Image Estimation with Gaussian Primitives<br><a href='http://arxiv.org/pdf/2606.31521'>论文</a></td><td>本文针对扩散MRI中EPI因B0场不均匀性产生的几何畸变问题，提出了一种物理驱动的联合重建框架，直接从k空间数据同时估计B0场和无畸变图像，无需依赖并行成像的中间重建结果。
◆ 提出基于高斯基元的联合B0场与图像估计方法，将两者均建模为连续高斯叠加并嵌入MRI物理前向模型，自然支持旋转视角EPI采集且无需插值。
◆ 引入旋转视角EPI策略，将畸变分散到多个相位编码方向，提升点扩散函数各向同性并增强B0场估计的约束。
◆ 采用实数非负的扩散加权图像建模，将图像相位吸收到每shot的相位因子中，提升对噪声和伪影的鲁棒性。
在体脑扩散EPI实验上，该方法在脑边界与无畸变结构参考的一致性上最接近，高b值和高加速条件下改善尤为显著，视觉对比也显示出更优的细节保真度与噪声抑制。</td></tr>
<tr><td>2026-06-28</td><td>Again-Pose: Anchor-Guided Adaptive Inter-Frame Motion Cues Propagating for High-quality Human Pose Reconstruction<br><a href='http://arxiv.org/pdf/2606.29230'>论文</a></td><td>Again-Pose针对极端运动场景下视频人体姿态重建中输入特征坍塌、隐式时序聚合失效的问题，将退化帧的姿态估计重新建模为运动引导的恢复任务，显著提升了模型的鲁棒性和稳定性。

◆ 提出Anchor-guided框架，通过特征显著性显式识别高质量Anchor帧，避免隐式注意力在退化特征上的错误聚合。

◆ 设计Dual-path Motion-aware Module，捕获细粒度跨帧动态运动线索。

◆ 设计Difference-weighted Fusion Module，自适应传播锚帧的运动线索以抑制漂移。

◆ 在Human3.6M、3DPW、PoseTrack及极端运动数据集FineDiving上验证，在严重模糊和遮挡下显著优于现有最优方法。</td></tr>
