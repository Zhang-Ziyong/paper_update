...
<tr><td>2026-06-08</td><td>Submodular Optimization with Applications to Decision and Control<br><a href='http://arxiv.org/pdf/2606.10192'>论文</a></td><td>这篇综述论文系统性地梳理了次模优化理论及其在决策与控制领域的应用。论文聚焦于次模集合函数的&quot;边际效用递减&quot;特性,阐述了尽管精确最大化是NP难问题,但贪心算法可对单调目标实现常数因子的近似保证,而基于随机贪心的变体则将此类保证扩展到非单调情形。论文覆盖了次模函数的结构性质、实际约束族(拟阵、背包、p-系统)以及主要近似算法,并广泛调研了在传感器调度、多智能体协调、鲁棒次模优化、领导者-跟随者系统、分布式优化、博弈论、资源分配、社交网络和信息路径规划等领域的应用。最后,论文还分析了控制论中典型目标函数的次模性,指出哪些目标具有次模性、哪些则不具备。

◆ 首次将次模优化理论与系统控制领域应用进行系统化整合,填补了该交叉方向综述的空白
◆ 强调通过曲率和次模比进行实例相关的精细化分析,超越了传统的最坏情况近似界
◆ 提供了最新的近似比和难度结果,涵盖单调与非单调次模最大化的最新进展
◆ 特别澄清了控制论中典型目标的次模性边界:可控性Gramian矩阵的对数行列式和秩、Kalman滤波信息矩阵的对数行列式具有次模性,而稳态Kalman滤波误差协方差和基于逆Gramian的平均控制能量则既非次模也非超模
◆ 突出了实用可实现的贪心类算法,为工程应用提供了直接指导
◆ 提出了跨领域的开放研究方向,为后续研究指明了路径...[摘要不完整，待更新]</td></tr>
...
FlexPath提出了一种基于图像的路径规划两阶段学习框架,旨在解决现有学习型规划器因受最短路径监督约束而难以灵活适配多样化任务目标的问题。该方法将路径可行性学习与任务偏好优化解耦,先通过模仿学习获取任务无关的空间先验,再利用可微分的路径形状目标(PSO)对预训练模型进行目标级适配,使单一模型能够灵活服务于多种规划需求,并可与经典搜索算法在推理阶段无缝结合。

◆ 提出可行性与偏好解耦的两阶段规划范式,使路径结构学习与任务目标优化相互独立,避免针对每种新目标重新训练。

◆ 设计可微分路径形状目标(PSOs),支持在不重新学习路径结构的前提下,通过高效的目标级适配将先验调整至特定任务标准。

◆ 在最短路径规划中,相比当前最优的TransPath在TMP数据集上降低14.3%的搜索开销,并获得平均更低的路径代价。

◆ 在三个未见领域展现出强大的零样本泛化能力,验证了空间先验的任务独立性。

◆ 框架可扩展至障碍物间距控制(最小间距2时实现96.8%完全避障)、语义感知避让以及路径点引导等多种规划场景。

◆ 推理阶段保持与经典规划器的兼容性,兼顾学习方法的效率与经典算法的可靠性。</td></tr>
