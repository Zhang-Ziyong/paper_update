...
<tr><td>2026-07-15</td><td>SeeSE3: Emergence of 3D Space in Vision Features<br><a href='http://arxiv.org/pdf/2607.14228'>论文</a></td><td>该论文研究视觉基础模型的特征表示是否蕴含三维欧几里得空间的内在结构。研究者跳出以往通过回归深度或法向量等图像中心量来探测3D感知的范式,转而探究视觉特征空间结构与欧几里得变换群SE(3)之间的关系。通过拓扑与几何两个视角设计探测工具,发现自监督视觉模型在静态场景中潜藏着与三维欧几里得空间高度对齐的子空间。基于这一洞察,作者提出在潜空间直接进行视觉里程计与定位的新方法,无需显式三维重建。

◆ 提出基于SE(3)变换群结构的新型探测框架,取代传统的图像中心量回归方法
◆ 设计互邻域度量指标,从拓扑角度衡量特征邻域与空间拓扑的对齐程度
◆ 提出Poincaré Adapter,测试相机运动几何在潜空间位移中的线性可达性
◆ 发现自监督视觉模型无需3D监督或主动代理即可涌现出强3D感知的潜空间子结构
◆ 开创&quot;潜空间导航&quot;技术,在特征空间中直接完成视觉里程计与定位任务,绕过显式三维重建...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-07</td><td>CILC: Cryptographically-secure Inter-agent Loop Closure Candidate Detection for Multi-Agent Collaborative SLAM<br><a href='http://arxiv.org/pdf/2607.06700'>论文</a></td><td>本文针对多智能体协作SLAM中全局描述符交换存在的隐私泄露问题，提出CILC系统，其核心贡献和创新点如下：

在多智能体协作SLAM场景中，机器人需要持续交换全局描述符以检测智能体间回环，而现有加密无线电方案无法防范被攻陷的群组成员。文章首先揭示了这一具体威胁，证明恶意代理可从诚实代理广播的全局描述符中重建其图像和轨迹的近似信息。

◆ 首次将安全多方计算(SMPC)引入多智能体协作SLAM的智能体间回环候选检测流程，实现在不交换明文全局描述符的前提下完成相似性比较。

◆ 采用选择性保护策略，仅对隐私敏感且计算轻量的GD相似度比较步骤应用SMPC，而非加密整个CSLAM流程，从而获得隐私性与计算开销之间的良好权衡。

◆ 通过仿真和硬件实验验证系统在多模态(视觉和LiDAR)全局描述符下仍具备实时性和通信可行性，并能有效缓解向被攻陷代理的信息泄露。</td></tr>
...
<tr><td>2026-06-26</td><td>RayPE: Ray-Space Positional Encoding for 3D-Aware Video Generation<br><a href='http://arxiv.org/pdf/2606.27345'>论文</a></td><td>该论文针对现代视频扩散Transformer仅在(u,v,t)采样网格上做RoPE位置编码、缺乏3D场景结构感知的问题，提出RayPE位置编码模块。其核心思想基于Plucker互易积与注意力点积的双线性代数同构，将每条射线的6D Plucker坐标加性注入自注意力的Q/K，并通过Q/K翻转设计使对称恒等配置与互易积严格对应。注意力得分可分解为内容项、几何项及两个交叉项，实验表明四项缺一不可。

◆将Plucker互易积的双线性结构与Transformer点积注意力建立代数同构，把6D射线坐标以加性方式注入Q/K，并通过Q/K翻转排列让对称配置与互易积精确吻合。

◆注意力得分自然分解为内容项、几何项和两个内容-几何交叉项，四项共同作用且不可缺一，赋予模型显式3D几何感知能力。

◆针对相机平移尺度异质的数据（ SfM、深度SLAM、度量级），解耦射线方向与矩幅大小，用对数幅值的可学习门控和RMSNorm与QKNorm内容分支对齐，保证数值稳定。

◆模块加性注入、零初始化启动，仅增加不到0.1%参数量，在四数据集混合训练下显著提升相机可控性、跨帧3D一致性与整体视频质量。</td></tr>
...
<tr><td>2026-07-05</td><td>GPU-Accelerated Polygonal Signed Distance Functions for Real-Time Collision Avoidance<br><a href='http://arxiv.org/pdf/2607.04310'>论文</a></td><td>本文针对障碍密集环境下基于优化的局部规划控制面临的计算瓶颈问题，提出了多边形符号距离函数（PSDF）及其在模型预测控制中的嵌入方法（PSDF-MPC）。该方法采用CPU/GPU分工架构，GPU负责批量计算PSDF值与梯度，CPU则求解维度仅取决于系统状态和预测时域的稀疏二次规划，使计算复杂度与障碍特征数量解耦。微基准测试、闭环仿真与真实环境实验均验证了PSDF-MPC在密集多边形环境中能够保持实时可行性与鲁棒避碰性能。

◆提出几何精确的PSDF，精确计算凸多边形机器人轮廓与以边界边表示的障碍物之间的有符号距离。
◆采用无权重、无分支的张量化几何流水线设计，支持批处理GPU执行与自动微分。
◆将PSDF嵌入基于序列二次规划（SQP）实时迭代的MPC框架，通过阶段安全约束的局部线性化实现高效求解。</td></tr>
...
<tr><td>2026-07-16</td><td>Catch, Throw, Repeat: Planning for Human-Robot Partner Juggling<br><a href='http://arxiv.org/pdf/2607.15129'>论文</a></td><td>本文针对人机动态物体交换这一难题，提出了一种面向人机协作杂耍的实时规划与控制架构，使机器人能可靠地与人类伙伴完成多球同步抛接。该系统集成了预测性球体追踪、基于多重打靶法的自适应在线轨迹优化，以及基于状态机的协调逻辑，在不确定性环境下实现了精确的实时协同与反馈控制。在8名不同技能水平参与者（从初学者到专家）的用户研究中，该系统成功实现了人与机器人共享的三球瀑布式杂耍。实验结果表明，所有受试者在10分钟测试中均超越了此前报告的最佳成绩，其中一人将共享三球瀑布式杂耍的连续机器人接球纪录提升至20次（原纪录的5倍），另一人在单球接返场景中实现了40次连续接球的100%成功率。

◆ 提出了首个面向人机协作杂耍的实时规划与控制架构，实现了多球同步抛接中的鲁棒协调
◆ 将预测性球体追踪与基于多重打靶法的在线自适应轨迹优化相结合，应对感知与时序不确定性
◆ 设计了基于状态机的协调逻辑，支持多球模式下的同步切换与稳定节拍控制
◆ 通过系统的用户研究验证了方法在不同技能水平人类伙伴间的泛化能力，并刷新了多项共享杂耍纪录...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-15</td><td>Merging Reaction to Cognition: A Hybrid Cognitive Strategy for Odour Source Localisation in Natural Environments<br><a href='http://arxiv.org/pdf/2607.13853'>论文</a></td><td>这篇论文针对湍流羽流间歇性结构导致气味源定位困难的问题，提出了一种融合反应式行为与认知概率规划的混合定位策略。现有两类方法各有局限：生物启发式反应策略效率高但依赖场景调参，认知概率策略鲁棒但探索过度且对信念精度敏感。基于马尔可夫链分析发现检测后源指向运动频率约为非检测时的两倍，本文据此将反应行为显式嵌入信念驱动的运动规划中。

◆提出检测触发的切换机制，形式化横流探索与源指向运动之间的过渡，使决策优先源接近而非信息获取
◆行为参数直接由信念度量推导，无需人工调参即可实现自适应反应行为
◆在三种湍流仿真和葡萄牙蒙德戈河自主水面船实地实验中验证，行程距离降低50%，成功率86%，平均定位误差3.2米...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-14</td><td>Worlds in One Demo: A Synthetic Data Engine for Learning Open-World Mobile Manipulation<br><a href='http://arxiv.org/pdf/2607.13154'>论文</a></td><td>本文针对开放世界移动操作策略学习面临的数据规模瓶颈问题，提出WANDA框架，通过单一人类演示即可生成大规模多样化合成数据。该方法从源RGBD观测中重建高斯溅射背景和机器人-物体交互轨迹作为世界基底，再利用全身运动规划将接触丰富的交互片段重组到广泛的空间配置中。实验证明，基于WANDA训练的策略在长时程鲁棒性、空间泛化和跨环境泛化方面均表现优异，并支持跨形态机器人的零样本部署。

◆ 基于单一演示重建高斯溅射世界基底，最大化每条演示的数据价值
◆ 全身运动规划重组接触丰富片段，实现空间配置多样化扩展
◆ 校正状态扩展机制增加机器人物体状态多样性，增强长时程鲁棒性
◆ 利用日常照片生成多样化3D场景，支撑跨环境泛化
◆ 网格与高斯背景合成实现照片级真实观测，支持跨形态零样本部署...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-06-06</td><td>SynthICL: Scalable In-context Imitation Learning with Synthetic Data<br><a href='http://arxiv.org/pdf/2606.08154'>论文</a></td><td>SynthICL是一个用于上下文模仿学习的可扩展框架，核心思路是完全依赖RGB合成数据训练策略，避免对深度传感、相机标定和真实数据的依赖。作者设计了一套高保真合成数据生成流水线，并基于此训练了一个流匹配(flow-matching)Transformer策略，使其在测试时仅需少量演示即可泛化到新任务。

创新点总结如下：

◆ 提出首个完全基于RGB合成数据训练上下文模仿学习策略的框架SynthICL，摆脱了对深度信息和真实数据的依赖，大幅提升了可扩展性。

◆ 构建了面向ICIL的高保真合成数据生成流水线，能够大规模产出多样化、可控的训练样本。

◆ 引入子目标图像预测机制，让模型显式预测下一子目标图像，从而实现更精确、视觉锚定的动作控制。

◆ 在16个未见过的真实世界操作任务上仅凭一次演示即达到79%的平均成功率，显著优于现有方法。</td></tr>
...
<tr><td>2026-06-30</td><td>Distortion-Corrected Diffusion MRI Using Rotated-View EPI and Joint Field-Map/Image Estimation with Gaussian Primitives<br><a href='http://arxiv.org/pdf/2606.31521'>论文</a></td><td>这篇论文针对扩散MRI中EPI成像因B0场不均匀导致的几何失真问题,提出了一种物理信息驱动的联合估计框架。传统方法采用先并行成像重建再估计B0场校正的串行流程,在高加速因子和高b值下重建伪影和低信噪比会严重制约校正质量。论文创新点如下:

◆ 直接从k空间数据联合估计B0场和无失真图像,摆脱对中间并行成像重建的依赖
◆ 图像与B0场均采用高斯基元的叠加进行连续参数化表示,兼顾平滑区域与组织边界
◆ 支持旋转视角EPI采集且无需插值,失真被分散到多个相位编码方向,改善点扩散函数各向同性
◆ 将扩散加权图像建模为实非负信号,把图像相位吸收为逐采集相位因子

在体内脑扩散EPI数据上,该方法在脑边界与无失真结构参考的一致性上表现最佳,且在高b值与高加速条件下相对串行方法的提升最为显著,同时在细节保真和噪声抑制方面也展现出明显优势。</td></tr>
<tr><td>2026-06-28</td><td>Again-Pose: Anchor-Guided Adaptive Inter-Frame Motion Cues Propagating for High-quality Human Pose Reconstruction<br><a href='http://arxiv.org/pdf/2606.29230'>论文</a></td><td>Again-Pose 论文针对极端运动场景下视频3D人体姿态重建的难题展开研究，指出传统隐式时序注意力方法在严重运动模糊和遮挡时容易因特征崩溃而失效。为此提出将退化帧的姿态估计重新建模为基于运动引导的恢复任务，核心思想是显式筛选高质量锚定帧并传播其可靠的运动学线索来&quot;修复&quot;中间退化帧的姿态。

◆ 提出基于特征显著性的锚定帧自动识别机制，以可靠的参考帧替代盲目平滑特征聚合，避免退化特征污染。

◆ 设计双路径运动感知模块，捕捉帧间细粒度动态变化，提取精确的运动学线索。

◆ 引入差异加权融合模块，自适应地传播运动线索以抑制误差漂移，提升重建稳定性。

◆ 在Human3.6M、3DPW、PoseTrack及极具挑战性的FineDiving数据集上全面超越现有方法，在其他方法失效的极端场景下仍能恢复合理姿态。</td></tr>
