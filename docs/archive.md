...
◆ 提出一种模块化的真实感3D声呐仿真架构，将GPU加速图形引擎与物理声学传播原理结合起来。

◆ 在NVIDIA Isaac Sim中实现了参考Water Linked 3D-15传感器的体积式3D声呐模型，并集成到完整水下仿真框架中。

◆ 通过硬件在环实验验证系统可用性，在Jetson Orin Nano上运行改造后的FastLIO2 SLAM，实现合成3D声呐、DVL、IMU和压力数据融合。

◆ 论文还将仿真结果与港口板桩检测的真实数据进行定性对比，分析当前sim-to-real差距，并为面向完全声学驱动的体积感知仿真提出发展路线。</td></tr>
...
<tr><td>2026-06-12</td><td>Sensitivity Shaping for Latent Modeling<br><a href='http://arxiv.org/pdf/2606.14585'>论文</a></td><td>本文聚焦于生成式动力学模型在机器人规划中的安全性，指出现有事后式OOD支持度检测在模型对关键动作不敏感时可能失效。
◆ 揭示了一类新的失效机制：不受支持的控制动作可能产生看似正常的潜变量预测，从而掩盖真实预测误差和OOD风险。
◆ 提出“支持条件控制敏感性正则化”，在高支持训练区域增强动力学模型对控制输入变化的局部响应。
◆ 该方法既保留了控制动作导致的必要变化，又通过支持度约束避免在弱数据区域产生不稳定外推。
◆ 在视觉避障、操作任务和真实机器人导航中验证了方法能提升OOD检测能力。
◆ 实验表明，该敏感性塑形机制可带来更安全的闭环规划，减少策略执行中的危险转移。</td></tr>
<tr><td>2026-06-12</td><td>ForestBack: Breadcrumb-Based Pedestrian Dead Reckoning for Infrastructure-Free Return Navigation<br><a href='http://arxiv.org/pdf/2606.14421'>论文</a></td><td>◆ ForestBack提出了一种无需GPS、Wi-Fi、蓝牙信标或预部署设施的行人返航导航框架，面向GPS受限环境中的可靠原路返回问题。
◆ 其核心创新是将用户行走轨迹记录为可逆“面包屑”节点序列，并据此进行双向路径重建和反向导航指引。
◆ 系统融合加速度步态检测、自适应步长估计、磁力计辅助航向估计和气压高度修正，提高了PDR轨迹重建精度。
◆ 在含障碍物绕行的室内路线中，ForestBack相较传统PDR将平均RMSE从1.129米降至0.965米，提升15.76%。
◆ 论文还发布了包含36次行走试验和42,474条时序样本的数据集及分析笔记本，为无基础设施PDR返航导航研究提供了可复现实验基准。</td></tr>
...
<tr><td>2026-06-10</td><td>SAFER-Nav: Enhancing Safety for Visual Robot Navigation via Segmentation-Aware Fine-Tuning<br><a href='http://arxiv.org/pdf/2606.11636'>论文</a></td><td>◆ 提出SAFER-Nav，通过分割感知微调将障碍物边界与可通行自由空间结构直接融入视觉导航策略，提升安全性。

◆ 方法不依赖外部轨迹修正，而是在策略内部学习与安全相关的场景结构，使模型更能应对未知障碍和环境分布变化。

◆ 该框架兼容多种仅基于RGB输入的导航骨干模型，包括Transformer和扩散式策略，具备较强通用性。

◆ 在多种机器人平台、室内环境以及静态/动态障碍场景中，SAFER-Nav显著降低碰撞频率。

◆ 与ViNT、NoMaD及其CARE增强版本相比，该方法在提升安全性的同时保持了良好的目标到达性能。</td></tr>
<tr><td>2026-06-09</td><td>AgniNav: Configuration-Driven Cross-Embodiment Local Planning for Robot Navigation<br><a href='http://arxiv.org/pdf/2606.10903'>论文</a></td><td>◆ AgniNav提出一种配置驱动的单目局部导航框架，使同一套感知与规划系统可在轮式、四足和人形机器人间零重训练迁移。

◆ 其核心是用四个可测量参数描述机器人安全碰撞包络：碰撞相关高度、前向长度、后向长度和半宽。

◆ 高度参数用于条件化图像到伪激光扫描网络，从单目彩色图像预测与碰撞安全相关的一维扫描表示。

◆ 其余足迹参数用于配置尺寸感知局部规划器，实现针对不同机器人外形的碰撞检测与路径选择。

◆ 训练方法利用彩色-深度配对数据生成高度条件化的列最小扫描标签，无需采集特定机器人数据即可监督多种安全包络。

◆ 实机结果显示，该方法在Turtlebot2、Unitree Go2和K1平台上以Jetson Orin 30Hz运行，并取得较高成功率和低碰撞率。</td></tr>
...
<tr><td>2026-06-12</td><td>ParkingTransformer: LLM-Enhanced End-to-End Trajectory Planning for Autonomous Parking<br><a href='http://arxiv.org/pdf/2606.17082'>论文</a></td><td>◆论文提出ParkingTransformer，将大语言模型的场景理解能力引入端到端自主泊车，实现从道路到目标车位的长距离轨迹规划。

◆该方法融合多视角感知、历史信息和原始传感器数据，通过轨迹查询与LLM隐式状态特征交互，避免依赖稠密BEV表示。

◆针对LLM空间推理不足，作者引入3D位置编码，显式增强模型对几何结构和空间关系的理解。

◆论文设计固定窗口流式历史处理机制，提升长时序信息建模效率，并显著加快推理速度。

◆通过粗到细解码策略逐步优化轨迹精度，并在CARLA和真实车辆实验中取得较高驾驶得分与88.70%的平均成功率，验证了方法有效性。</td></tr>
...
<tr><td>2026-05-10</td><td>VFM-SDM: A vision foundation model-based framework for training-free, marker-free, and calibration-free structural displacement measurement<br><a href='http://arxiv.org/pdf/2605.09677'>论文</a></td><td>◆提出VFM-SDM框架，将视觉基础模型用于结构位移测量，实现无需专项训练、无需标记物、无需现场相机标定的非接触式部署。
◆通过VFM推断相机参数并结合点跟踪与三角测量，重建结构多方向位移，降低传统视觉测量对人工准备的依赖。
◆引入结构几何约束，抑制不符合物理规律的位移偏差，提升估计结果的一致性和鲁棒性。
◆构建了服役人行桥多模态现场数据集，并提出统一基准评测流程，增强方法验证的可复现性。
◆实验表明该方法在竖向和横向位移上具有较低幅值误差、较高时间相关性和较小峰峰值误差，展示了面向数字孪生和数据驱动建造的应用潜力。</td></tr>
