...
<tr><td>2026-06-18</td><td>Gaussian Process Prior Variational Autoencoder for Endoscopic Videos<br><a href='http://arxiv.org/pdf/2606.19908'>论文</a></td><td>◆提出GPVAE用于内镜视频修复，将传统VAE的独立潜变量先验替换为时间高斯过程先验，从而显式利用帧间连续性。
◆该方法支持缺失帧的不确定性感知插值与重建，并输出逐帧置信度，反映恢复结果的可靠性。
◆框架结合EndoVAE卷积骨干和GastroNet-5M预训练ViT编码器，更好适配内镜图像特征。
◆引入HPA和SPA两种可扩展高斯过程近似，并利用DUCKNet掩膜排除镜面反射像素对训练的干扰。
◆在C3VDv2上，GPVAE相对VAE平均降低重建RMSE 21.9%、最高26.1%，并使下游轨迹RMSE平均降低12.7%。</td></tr>
...
<tr><td>2026-06-15</td><td>VISTA: Scale-Aware Visual Navigation via Action History Conditioning<br><a href='http://arxiv.org/pdf/2606.17294'>论文</a></td><td>◆ VISTA针对视觉导航基础模型中归一化动作带来的尺度敏感问题，指出同一归一化轨迹在不同物理缩放下会改变真实几何路径并增加碰撞风险。

◆ 其核心创新是将归一化动作历史与图像观测共同作为条件输入，使模型显式理解预测动作与机器人实际位移之间的尺度关系。

◆ 该方法提升了跨机器人、跨环境部署时的鲁棒性，缓解了端到端导航策略在零样本应用中的尺度适配难题。

◆ VISTA还引入DINOv3视觉编码器，增强对空间结构和几何关系的表征能力，从而改善在视觉重复、特征稀疏场景中的导航表现。

◆ 实验显示，VISTA在户外、森林和办公室等未见真实环境中实现100%零样本目标预测准确率，并平均通过95%的路径检查点，证明其具备稳定的路径跟随与泛化能力。</td></tr>
...
<tr><td>2026-06-11</td><td>Observation of intertwined charge density wave order and superconductivity in Janus monolayer<br><a href='http://arxiv.org/pdf/2606.13828'>论文</a></td><td>◆本文首次系统预测1T-ZrSeTe Janus单层中CDW与超导两类不稳定性的共存与耦合关系。
◆研究发现M点显著声子软化源于增强的电子-声子相互作用及带间、带内散射共同驱动。
◆2×2×1晶格畸变会重构能带并打开小的间接带隙，使体系由半金属转变为半导体。
◆与ZrTe2单层相比，Se替代一侧Te显著降低畸变能量收益，表明Janus结构削弱CDW倾向。
◆论文指出电子关联和双轴应变可有效调控CDW与超导不稳定性。
◆高温未畸变相呈现声子介导的双能隙超导，主要来自M点软模与Zr d、Te p费米面能带的耦合，SOC会降低转变温度。</td></tr>
