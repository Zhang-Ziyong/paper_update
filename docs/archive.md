...
<tr><td>2026-07-02</td><td>LuxSQA: Ask Me in Luxembourgish with TTS-Augmented Spoken Question Answering<br><a href='http://arxiv.org/pdf/2607.02763'>论文</a></td><td>本文针对低资源语言Luxembourgish的语音问答任务，探索了利用TTS合成数据替代人工录制语料的可行性。研究从现有文本QA资源出发，将问题翻译为Luxembourgish，并通过多种TTS系统合成语音问题，再与文本答案配对，构成了约4.8万单源和约23万4TTS多源的训练集。在模型架构上，采用了参数高效的SLAM风格设计，冻结Whisper编码器与多语言LLM，通过可学习的投影器与LoRA适配器连接。

◆ 首次系统验证了TTS合成语音可直接作为低资源SQA任务的有效训练数据，无需大规模人工录制的QA语料
◆ 提出了结合冻结Whisper编码器、可学习投影器与LoRA适配器的参数高效SLAM架构，有效降低训练成本
◆ 引入4TTS多源混合数据策略，结合语音设计型合成配置，显著提升Luxembourgish SQA性能
◆ 揭示了无参考TTS音质评分与下游QA性能之间不存在单调对应关系，主张应将合成语音作为任务相关训练数据来评估，而不仅关注其自然度

该研究为低资源语言SQA提供了新的数据构建与模型训练范式，强调了任务导向评估的重要性。</td></tr>
...
<tr><td>2026-06-03</td><td>A Geometry-Informed Computer Vision Method for Detecting and Examining Overtaking Vehicles From A Bicycle<br><a href='http://arxiv.org/pdf/2606.23699'>论文</a></td><td>本文提出一种基于几何信息的计算机视觉流水线，用于从自行车后置摄像头自动检测超车事件，突破了仪器化研究中手动逐帧标注的瓶颈。

◆融合RT-DETR目标检测与ByteTrack多目标跟踪，通过基于透视投影原理的三阶段几何验证模块（方位角趋势、表观尺寸增长、空间确认），无需多传感器或显式标定即可自动识别超车事件。

◆在315个真实城市道路超车事件上达到97.8%召回率且零误报，平均提前2.44秒识别超车意图，84.1%的事件超过1.5秒人类反应时间阈值，具备主动安全告警潜力。

◆提出基于边界框几何特征的无标定横向距离估计方法，留一交叉验证下平均绝对误差仅13-14厘米，足以区分近距离与标准距离通过。

实测发现33.3%的车辆通过距离低于5英尺安全阈值，与既有研究一致，验证了该系统对车辆-自行车交互大规模分析的可行性与可扩展性。</td></tr>
