...
<tr><td>2026-07-22</td><td>DINS-IO: Learned Inertial Odometry via Differentiable INS Consistency<br><a href='http://arxiv.org/pdf/2607.20232'>论文</a></td><td>该论文提出DINS-IO，一种无需位置标签即可从原始IMU数据学习惯性里程计的自监督框架。核心思想是将捷联惯导系统的速度递推视为强可微一致性先验，要求预测速度旋转换算后与积分比力在仅含未知初速度和常值加速度计偏差下保持一致。论文将该约束构建为滑动窗口最小二乘问题，采用全局共享偏差并推导闭式解，以求解残差作为自监督损失，使梯度通过解析解回传至网络。为实现高频约束设计了在IMU采样率下输出稠密机体速度的网络。针对自监督阶段速度尚未度量校准的问题，论文利用少量标注轨迹直接监督机体速度并以低秩适配(LoRA)方式微调。在标准基准上，DINS-IO以极少量标签即达到或超越全监督基线水平。

◆ 利用捷联INS速度递推作为可微一致性先验，免去位置标签需求
◆ 将约束建模为滑动窗口最小二乘并推导闭式解，残差作为自监督损失使梯度回传网络
◆ 设计IMU采样率的高频网络，输出稠密机体速度以提供逐样本约束
◆ 通过少量标注轨迹对机体速度直接监督并采用LoRA低秩适配完成度量校准
◆ 以自监督预训练加少量标签微调的范式，匹配或超越全监督方法性能...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-12</td><td>Mapping Pamir: Multi-Session Visual-Inertial SLAM and 3D Reconstruction of an Underwater Shipwreck<br><a href='http://arxiv.org/pdf/2607.10925'>论文</a></td><td>本文提出了一种基于低成本运动相机的水下多会话建图框架，通过将视觉惯性数据与潜水电脑的水深记录相结合来增强定位精度。研究采用开源的SVIn2框架生成各会话的轨迹与稀疏重建，再利用COLMAP进行全局优化和稠密三维重建，并通过固定位置的标定目标将不同会话的数据统一到同一坐标系下。作者将该流程应用于巴巴多斯海岸附近的一艘沉船，首次同时完成了沉船外部与可进入内部的两会话完整建图，并使用两台不同视场角的相机进行了第三轮采集以验证方案的可扩展性。整体工作在低成本硬件条件下实现了水下多源数据的有效融合与多视角三维重建。

创新点：
◆ 提出了一种融合低成本运动相机、视觉惯性SLAM与潜水电脑水深信息的水下多会话建图流程
◆ 利用固定标定目标实现跨会话的坐标变换，将多次独立采集数据统一到同一参考系
◆ 首次实现了沉船外部与可进入内部区域的完整三维建图，并通过双相机不同视场角的组合采集验证了方法的灵活性与实用性...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-01</td><td>EPO: Boosting 3D Foundation Models with Edge-based Pose Optimization<br><a href='http://arxiv.org/pdf/2607.00579'>论文</a></td><td>EPO（Edge-based Pose Optimization）是一种基于边缘的无轨迹几何优化框架，专门用于提升3D基础模型生成的运动恢复结构重建结果。3D基础模型通过跳过传统流程中耗时的特征提取与匹配步骤实现快速推断，但几何精度不及传统管线；现有类似Bundle Adjustment的后处理方法需要重建特征轨迹，丧失了速度优势。

◆ 提出完全可微的边缘对齐优化框架，以边缘图对齐作为几何优化的代理信号，无需任何特征提取或轨迹构建，从根本上保留3D基础模型的推理速度优势。
◆ 摒弃传统Bundle Adjustment对显式特征轨迹的依赖，大幅降低内存占用，使其能够在消费级硬件上运行，而传统精调方法因内存需求过高难以部署。
◆ 在多个数据集和任务上的广泛实验表明，EPO在精度上匹配甚至超越Bundle Adjustment类方法，同时运行时间和内存开销显著更低。</td></tr>
...
<tr><td>2026-05-01</td><td>TrueEBSD in MTEX: automatic image matching for correlative microscopy applications<br><a href='http://arxiv.org/pdf/2605.00703'>论文</a></td><td>TrueEBSD是一款开源的MATLAB程序，用于图像配准和电子背散射衍射（EBSD）图的空间畸变校正。本论文将TrueEBSD重新实现为MTEX（成熟的EBSD数据分析工具箱）的附加模块，使EBSD图能够与其它成像模式的数据进行空间配准和关联分析。

创新点：
◆ 将TrueEBSD重新实现为MTEX插件，实现了两个工具的无缝集成与扩展
◆ 提供自动图像配准和空间畸变校正功能，无需手动操作即可对齐多模态数据
◆ 支持EBSD图与其它成像数据（如SEM）的关联分析，扩展了定量晶体学分析能力

论文通过两个应用案例验证了工具的实用性：一是测量WC-Co复合材料中的Co相比例和WC连续性；二是分析铜多晶体中晶界对空洞形成的相对敏感性。在两个案例中，EBSD图通过与SEM图像数据融合增强，使研究者能够获得单独分析EBSD图和SEM图像时无法实现的定量晶体学测量结果，从而为相关显微分析提供了更强大的研究手段。</td></tr>
</tbody>
</table>
</div>
...
<tr><td>2026-07-22</td><td>Towards Capability-Aware Traversability Navigation for Unstructured Environments<br><a href='http://arxiv.org/pdf/2607.20679'>论文</a></td><td>本文针对非结构化环境中不同机器人平台的可通行性估计难题,提出能力感知可通行性(CAT)框架。现有方法依赖后期轨迹过滤实现跨形态迁移,未能将平台约束编码进学习表示。CAT通过交互式标注将物理轨迹与密集监督掩码对齐,利用SPADE模块把机器人特定可行性向量调制进语义地形图,实现物理能力与空间特征的深度融合。模型在物理轨迹AUROC上提升11.0%,人类标注AUPRC提升15.8%,显著优于基线。消融实验证明空间调制和形态原型是能力敏感性的关键。该方法已在四足和轮式机器人嵌入式平台以4.8Hz实时运行,验证了跨形态泛化能力。

◆ 提出CAT框架,将机器人物理能力约束直接嵌入空间特征表示,突破了传统后处理轨迹过滤的范式
◆ 设计交互式标注流水线,将物理轨迹与密集监督掩码对齐,保证监督信号源自真实可执行路径
◆ 引入SPADE空间自适应归一化机制,实现机器人特定向量对语义地形...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-22</td><td>Self-Supervised Bio-Inspired Robotic Trajectory Planning with Obstacle Avoidance<br><a href='http://arxiv.org/pdf/2607.20743'>论文</a></td><td>本文提出了一种受神经科学启发的自监督学习框架，将前向模型与逆向模型作为内部监督信号，用于在含障碍物环境中生成无碰撞的机器人轨迹。该工作是其前期研究的延伸，专门验证了所提框架在避障场景下的可行性与泛化能力。相较于传统的基于采样的规划器，该方法只需少量神经网络前向传播即可生成轨迹，显著提升了规划效率。实验结果表明该方法有效，但揭示出规划器倾向于利用前向-逆向模型学习信号这一隐患。针对该问题，作者系统分析了其成因并提出了改进的训练机制与多种缓解策略以提升规划性能与鲁棒性。

创新点：
◆ 提出基于前向与逆向模型内部监督的自监督轨迹规划框架，无需专家演示或大规模探索
◆ 将神经科学启发的学习机制扩展至含障碍物的避障规划场景
◆ 发现并分析规划器过度利用学习信号的倾向性偏差现象
◆ 提出针对性训练策略与缓解方案以增强规划质量与泛化能力...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-06</td><td>A Reliable Context-Aware and Temporal Planning Framework for Autonomous Driving<br><a href='http://arxiv.org/pdf/2607.04689'>论文</a></td><td>针对自动驾驶中相机观测退化（如遮挡、运动模糊、光照变化）导致规划不稳定的问题，本文提出RCT-AD框架，其核心在于显式建模特征质量与时序一致性，从而支持更安全、更一致的轨迹规划。

◆ 提出可靠上下文感知模块，对每帧特征进行可靠性评分，并通过质量门控的先进后出记忆机制选择性保留可信特征，从可靠历史上下文中重建退化观测，避免被污染的输入破坏场景表示。

◆ 设计时序轨迹规划器，捕捉长期依赖与多智能体交互关系，生成更平滑、安全感知的轨迹。

◆ 引入联合检测与分割头，将语义与运动线索注入共享BEV空间，强化场景理解能力。

◆ 在nuScenes基准上取得61.5 NDS、52.9 mAP和52.3 mIoU，感知精度、运动预测与规划鲁棒性均优于近期端到端基线，同时保持适合实时部署的计算效率。</td></tr>
