...
<ol>
<li><a href='#slam'>SLAM (43篇)</a></li>
<li><a href='#sfm'>SFM (31篇)</a></li>
<li><a href='#image-matching'>Image Matching (10篇)</a></li>
<li><a href='#obstacle-avoidance'>Obstacle Avoidance (47篇)</a></li>
<li><a href='#navigation'>Navigation (49篇)</a></li>
<li><a href='#motion-planning'>Motion Planning (62篇)</a></li>
<li><a href='#sensor-calibration'>Sensor Calibration (15篇)</a></li>
<li><a href='#sensor-undistortion'>Sensor Undistortion (30篇)</a></li>
</ol>
</details>

<h2 id='slam'>SLAM (43篇)</h2>
...
<tr><td>2026-05-29</td><td>ScaRF-SLAM: Scale-Consistent Reconstruction with Feed-Forward Models and Classical Visual SLAM<br><a href='http://arxiv.org/pdf/2606.00307'>论文</a></td><td>本论文提出ScaRF-SLAM,一种将经典基于特征的视觉SLAM与几何基础模型(GFM)解耦融合的框架,旨在解决直接使用GFM预测进行位姿跟踪时因几何预测误差导致的位姿估计不稳定问题。该方法将跟踪与建图分离:经典SLAM负责鲁棒低延迟的位姿跟踪,GFM仅用于稠密建图,从而避免GFM预测误差传播至位姿估计,同时利用几何约束提升重建质量。实验表明,该方法在建筑尺度数据集上每10米误差约2厘米,在大尺度户外场景每30米误差约10厘米,相比现有方法重建精度提升10%-20%。

◆ 提出解耦式架构,将经典特征SLAM用于跟踪、GFM专用于建图,避免几何预测不准确性污染位姿估计
◆ 设计基于多关键帧的子地图构建机制,通过轻量级的帧级与子图级尺度优化实现尺度一致性
◆ 采用基于投影的子图内点云融合策略,并支持根据SLAM轨迹更新在线动态调整子图
◆ 构建了一个富含回环、建筑尺度的室内数据集,提供精确的传感器轨迹和LiDAR真值,用于评估跟踪与重建精度
◆ 在建筑尺度室内和大尺度户外场景中均实现了高精度的稠密重建,显著优于现有统一SLAM-GFM方法...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-05-26</td><td>Trinity: Unifying Class-Agnostic Terrain and Semantic Segmentation for Unstructured Outdoor Environments by Leveraging Synthetic Data<br><a href='http://arxiv.org/pdf/2605.27644'>论文</a></td><td>本论文提出了一个名为Trinity的统一Transformer架构，能够同时进行类别的语义分割和类别无关的地形分割，用于无人地面车辆在非结构化户外环境中的地形理解。通过仅基于视觉外观分割地形区域，而不依赖预定义语义标签或机器人特定的可穿越性评分，学习到机器人无关的视觉地形先验，可结合机器人经验用于可穿越性估计、视觉里程计和任务规划等下游任务。该方法解决了现有方法需机器人特定标注或语义类映射、跨平台迁移困难的问题。

◆ 提出联合学习类语义分割与类无关地形分割的统一网络Trinity，使得地形分割不再依赖机器人特定的可穿越性标签，提升了跨平台泛化能力。

◆ 扩展了OAISYS模拟器并创建RUGDSynth合成数据集，包含大量类无关地形样本，支持大规模训练，同时发布真实场景的EXTerra数据集，带有双重标注（类语义和类无关地形标签）。

实验验证了该联合分割方法在复杂户外环境中的有效性，证明任务设定的可行性和网络性能。</td></tr>
...
<h2 id='image-matching'>Image Matching (10篇)</h2>
...
<h2 id='obstacle-avoidance'>Obstacle Avoidance (47篇)</h2>
...
<tr><td>2026-05-29</td><td>Constrained Whole-Body Tracking for Humanoid Robots<br><a href='http://arxiv.org/pdf/2606.00374'>论文</a></td><td>这篇论文提出了ConstrainedMimic,一个针对人形机器人全身运动跟踪的约束控制框架,旨在解决强化学习策略在训练后难以满足运行时安全约束的问题。该方法结合操作空间控制与控制屏障函数(CBF)原理,利用全身运动学和动力学信息,在RL跟踪策略之上实现实时约束执行。作者在仿真的Unitree G1人形机器人上通过全身运动跟踪和遥操作实验,验证了碰撞避免、关节限位以及质心稳定性等多种约束的有效性。该框架在保持当前接触模式和跟踪目标一致性的前提下,最大程度地保留了策略原有能力,且具备全可微、跨硬件平台运行(CPU/GPU/TPU)以及高频部署(300-500 Hz)等优势。

◆ 提出了首个将操作空间控制与控制屏障函数相结合的人形机器人RL跟踪策略约束执行框架,可处理训练后才指定的任意运行时约束。
◆ 同时在运动学参考与底层动力学层面施加约束,支持自碰撞、外部障碍避让、关节限位、质心稳定性等多种约束类型的统一处理。
◆ 通过保持与当前接触模式及跟踪目标的一致性,实现约束激活时对策略能力的最小干预,避免过度保守。
◆ 框架完全可微,支持CPU、GPU、TPU多平台部署,运行频率高达300-500 Hz,具备良好的实用性与扩展性。</td></tr>
<tr><td>2026-05-29</td><td>GLENS: Global Search via Learning from Solver Iterates with Diffusion Models<br><a href='http://arxiv.org/pdf/2606.00366'>论文</a></td><td>本论文针对多模态非凸连续优化问题,提出了GLENS方法,用于生成大量高质量且多样化的初始猜测点,以加速数值求解器收敛并发现多个局部最优解。该方法的核心思想是利用求解器的中间迭代过程作为免费的数据增强来源,克服了现有数据驱动方法仅使用最终收敛解、训练数据有限的不足。实验表明,GLENS在非凸基准问题和双机器人避障导航问题上均能生成高质量初始猜测,保持多样化局部最优分布,并在不同求解器和问题设置下实现更快的收敛。

◆ 首次提出将求解器中间迭代点作为免费数据增强来源,显著提升数据效率,解决了离线求解数据稀缺的瓶颈。

◆ 设计了基于扩散模型的邻域结构模型,在问题参数条件下学习最优解周围的局部几何结构,而非仅学习离散的最优点。

◆ 提出求解器行为模型,学习迭代过程中的精化方向,在扩散采样过程中引导样本向附近最优解收敛。

◆ 实现了高质量与多样性的双重目标,既保证生成的初始猜测能让求解器快速收敛,又保留了多模态最优解的分布特性,支持灵活的下游决策。

◆ 在修改后的非凸基准问题及实际的双机器人避障导航问题上验证了方法的通用性与跨求解器适应能力。</td></tr>
...
<h2 id='navigation'>Navigation (49篇)</h2>
...
<tr><td>2026-05-29</td><td>Predicted-Flow Control Barrier Functions for Real-Time Safe Optimal Control<br><a href='http://arxiv.org/pdf/2606.00297'>论文</a></td><td>本文针对传统控制屏障函数(CBF)难以构造且具有短视性的问题,提出了预测流控制屏障函数(P-CBF)框架,将CBF从当前状态的函数推广为有限预测时域内参数化控制规划下预测流的泛函,从而在整个预测时域内提供安全性认证。该方法通过引入终端候选P-CBF和规划时间偏移两项机制,解决了控制约束下P-CBF可行性难以保证的难题,并将实时控制、控制规划参数演化和规划时间偏移统一在单一凸优化中求解,实现了有限时域积分代价优化与安全认证的统一。在非完整地面机器人密集环境导航实验中,所提方法(FlowBarrier)相比非线性模型预测控制和两种CBF安全过滤方法,在100次试验中取得了最高目标到达率、零安全违规和最低计算时间。

◆ 提出预测流控制屏障函数P-CBF,将CBF从状态函数推广为预测流的泛函,实现整个预测时域的安全认证,克服了传统CBF的短视性。

◆ 设计终端候选P-CBF机制,要求预测流在终端时刻进入备份安全集,解决了控制约束下P-CBF有效性难以保证的核心难题。

◆ 引入规划时间偏移作为额外自由度调节预测时域,确保优化问题的可行性,并保证关联安全集的前向不变性。

◆ 将实时控制、规划参数演化与时间偏移联合纳入单一凸优化(在多面体约束下退化为二次规划QP),首次统一了有限时域积分代价优化与安全认证。

◆ 实现了FlowBarrier算法并在非完整地面机器人上验证,相比NMPC和两类CBF安全过滤器,在目标达成率、安全性和计算效率上均显著占优。</td></tr>
...
<tr><td>2026-05-28</td><td>Learning-Based Navigation for Indoor Mobile Robots<br><a href='http://arxiv.org/pdf/2605.30468'>论文</a> | <a href='https://ntdathp.github.io/rl_robot_web/'>代码</a></td><td>本论文针对室内移动机器人导航问题,提出了一种融合监督学习与强化学习的两层导航框架,将全局规划与局部控制分别由神经网络驱动并协同工作,在仿真和真实室内环境中均验证了方法的有效性,能够在存在障碍物的情况下生成可行的全局路径并输出可靠的局部运动指令,实现安全的目标导向导航。

核心贡献与创新点如下:

◆ 提出基于监督学习的神经全局规划器,利用代价感知的 A* 专家轨迹进行训练,使全局路径兼顾最短性与代价最优性。

◆ 创新性地将动态窗口法(DWA)重构为离散候选动作选择问题,提出 Learning-Based DWA 局部规划器,在保留 DWA 运动可行性的同时引入学习能力。

◆ 采用&quot;行为克隆预训练 + PPO 强化学习微调&quot;的两阶段训练策略,先模仿专家再自主优化,提升了策略收敛速度与最终性能。

◆ 在 PPO 训练中引入可行性感知的动作掩码机制(feasibility-aware masking),有效约束策略输出在安全可执行的动作空间内,提升导航安全性。

◆ 构建并验证了学习式全局规划与强化学习式局部控制相结合的端到端导航系统,并在仿真与真实环境中完成部署测试,源代码公开发布以促进复现研究。</td></tr>
...
<h2 id='motion-planning'>Motion Planning (62篇)</h2>
...
<tr><td>2026-05-30</td><td>From Cues to Horizons: Dynamic Risk Horizon Profiling for Trajectory Prediction<br><a href='http://arxiv.org/pdf/2606.00857'>论文</a> | <a href='https://github.com/bilab-nyu/RHP'>代码</a></td><td>论文核心贡献总结:

该论文针对自动驾驶中车辆轨迹预测的安全性问题,提出了一种风险时域剖析(Risk Horizon Profiling, RHP)模块。不同于以往仅将历史风险作为辅助信号的方法,该研究将风险的未来演化和不确定性纳入预测框架,通过连续可学习的势场模型刻画周围交通参与者的时空邻近性,从而生成跨未来时域的风险分布,自适应识别人类驾驶员所感知的关键时刻。实验在涵盖高速公路(highD)和城市街道(SHRP2)两类不同驾驶场景的数据集上进行,结果显示在highD上5秒RMSE降低25.0%,在SHRP2上5秒minFDE降低29.1%,验证了方法在短时与长时预测中的优越性和跨场景泛化能力。

主要创新点:

◆ 提出风险时域剖析(RHP)模块,首次将风险的未来演化作为主动建模对象,而非仅利用历史风险作为辅助信号。

◆ 设计了连续可学习的势场模型,通过计算周围对象的时空邻近性来量化并刻画跨未来时域的动态风险分布。

◆ 引入&quot;关键时刻&quot;自适应识别机制,使模型能够模拟人类驾驶员对危险时段的感知与判断。

◆ 在高速公路与城市街道两类异构场景及涵盖安全、近碰撞、碰撞事件的多样化风险数据上验证了方法的鲁棒泛化能力,为更安全的自动驾驶路径规划与决策提供支持。</td></tr>
...
<h2 id='sensor-calibration'>Sensor Calibration (15篇)</h2>

<div class="table-container">
<table>
<thead><tr><th>日期</th><th>标题</th><th>摘要</th></tr></thead>
<tbody>
<tr><td>2026-06-03</td><td>Multi-Camera AR Guidance System for Surgical Instrument Handling and Assembly: Investigating Workload and Efficiency<br><a href='http://arxiv.org/pdf/2606.04992'>论文</a></td><td>这篇论文针对手术器械装配与传递过程中洗手护士面临的高认知负荷问题,提出了一种基于多相机6D姿态估计与头戴式显示器增强现实的术中器械引导系统。该系统无需额外标记物,通过已知物体实现姿态估计和相机标定,姿态估计网络完全使用合成数据训练以提升泛化能力。AR引导界面提供器械尖端定位提示和分步装配动画,用户可通过注视选择和脚踏板在装配步骤间切换。在膝关节置换术仿真场景中对29名洗手护士进行的用户研究表明,该系统相比传统纸质手册显著降低了主观工作负荷,任务完成时间缩短21.3%(4.76分钟),对器械不熟悉的护士受益最为明显。

◆ 提出了一种无需标记物的多相机6D姿态估计方法,在技术指标上优于现有最先进方法
◆ 将多相机姿态估计与头戴式AR原位可视化相结合,专门面向手术器械装配场景
◆ 姿态估计网络仅基于合成数据训练,提升了真实场景下的泛化性和适用性
◆ 设计了基于注视选择与脚踏板的术中交互方式,符合无菌手术环境的操作需求
◆ 在膝关节置换术仿真中开展了29名洗手护士参与的大规模用户研究,从主观与客观双维度验证系统效果
◆ 证实该系统对经验不足的洗手护士辅助效果尤为显著,具有实际临床应用价值...[摘要不完整，待更新]</td></tr>
...
<h2 id='sensor-undistortion'>Sensor Undistortion (30篇)</h2>
