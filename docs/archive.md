...
<ol>
<li><a href='#slam'>SLAM (60篇)</a></li>
<li><a href='#sfm'>SFM (40篇)</a></li>
<li><a href='#image-matching'>Image Matching (12篇)</a></li>
<li><a href='#obstacle-avoidance'>Obstacle Avoidance (89篇)</a></li>
<li><a href='#navigation'>Navigation (79篇)</a></li>
<li><a href='#motion-planning'>Motion Planning (114篇)</a></li>
<li><a href='#sensor-calibration'>Sensor Calibration (23篇)</a></li>
<li><a href='#sensor-undistortion'>Sensor Undistortion (46篇)</a></li>
</ol>
</details>

<h2 id='slam'>SLAM (60篇)</h2>
...
<tr><td>2026-06-04</td><td>Towards Realistic 3D Sonar Simulation<br><a href='http://arxiv.org/pdf/2606.06130'>论文</a></td><td>◆论文指出现有3D声呐仿真过度依赖几何渲染、近似为水下LiDAR，难以反映折射、多路径干涉和相位相关成像等真实声学现象。

◆提出一种模块化3D声呐仿真架构，将GPU加速图形引擎与物理声传播模型结合，以提升水下感知仿真的真实性。

◆在NVIDIA Isaac Sim中实现了参考Water Linked 3D-15传感器的体积式3D声呐模型，并集成到完整水下仿真框架。

◆通过硬件在环实验验证系统可用性，在Jetson Orin Nano上运行改造的FastLIO2，实现合成声呐、DVL、IMU和压力数据融合。

◆论文还将仿真结果与港口板桩检测的真实数据进行定性对比，分析sim-to-real差距，并为未来全声学驱动的体积感知仿真提供路线图。</td></tr>
...
<h2 id='image-matching'>Image Matching (12篇)</h2>
...
<h2 id='obstacle-avoidance'>Obstacle Avoidance (89篇)</h2>
...
<tr><td>2026-06-18</td><td>Stable Transformer-Actor-Critic Model Predictive Control: A Contraction Analysis Approach<br><a href='http://arxiv.org/pdf/2606.20197'>论文</a></td><td>◆提出了一种Transformer-Actor-Critic MPC架构，用于处理复杂非凸控制任务，并同时关注闭环稳定性与鲁棒性保证。
...
<tr><td>2026-06-12</td><td>Sensitivity Shaping for Latent Modeling<br><a href='http://arxiv.org/pdf/2606.14585'>论文</a></td><td>◆论文指出，现有在固定潜在动力学模型上附加的事后OOD支持度估计，可能因模型对关键动作变化不敏感而失效。
◆作者揭示了一个重要风险：未被数据支持的控制动作也可能产生看似正常的潜在预测，从而掩盖真实预测误差和安全隐患。
◆论文提出“支持度条件化的控制敏感性正则化”，鼓励模型在高支持训练区域内对动作扰动保持足够敏感。
◆该方法在保留控制引起的有效动态差异的同时，避免在低支持区域过度放大不可靠外推。
◆在视觉避障、操作任务和真实机器人导航实验中，该方法提升了OOD检测能力，并带来更安全的闭环规划表现。</td></tr>
<tr><td>2026-06-12</td><td>ForestBack: Breadcrumb-Based Pedestrian Dead Reckoning for Infrastructure-Free Return Navigation<br><a href='http://arxiv.org/pdf/2606.14421'>论文</a></td><td>◆提出ForestBack，一种无需GPS、Wi-Fi、蓝牙信标或预部署设施的行人返回导航框架，面向GPS受限环境中的可靠返程需求。
◆将用户行走轨迹记录为可逆“面包屑”节点序列，并基于双向路径重构生成反向导航指引，突出路线保持能力。
◆融合步态检测、自适应步长估计、磁力计航向估计和气压高度修正，提升PDR轨迹重建精度。
◆在含障碍物和五个检查点的室内路线中，构建36次试验、42474条时间序列样本的数据集进行验证。
◆实验显示其相较传统PDR将平均RMSE降低15.76%，最终位置误差也明显下降，转弯事件检测一致性约99.90%。
◆公开数据集与分析笔记本，支持基础设施无关PDR返回导航的复现和后续基准研究。</td></tr>
...
<h2 id='navigation'>Navigation (79篇)</h2>
...
<tr><td>2026-06-10</td><td>SAFER-Nav: Enhancing Safety for Visual Robot Navigation via Segmentation-Aware Fine-Tuning<br><a href='http://arxiv.org/pdf/2606.11636'>论文</a></td><td>◆ SAFER-Nav针对视觉导航基础模型在陌生环境、未见障碍和分布偏移下仍能朝目标前进但容易碰撞的问题，提出了更安全的训练方案。
◆ 核心创新是将障碍物边界与可通行自由空间等分割感知结构直接融入策略微调，而非依赖外部轨迹修正或隐式几何先验。
◆ 该方法设计为可适配多种仅使用RGB输入的导航骨干模型，提升了在不同视觉策略上的通用性。
◆ 论文在多种机器人平台、室内环境以及静态和动态障碍场景中进行了验证，显示出较强的跨场景鲁棒性。
◆ 实验表明，相比ViNT、NoMaD及其CARE增强版本，SAFER-Nav显著降低碰撞频率，同时基本保持目标到达能力。</td></tr>
<tr><td>2026-06-09</td><td>AgniNav: Configuration-Driven Cross-Embodiment Local Planning for Robot Navigation<br><a href='http://arxiv.org/pdf/2606.10903'>论文</a></td><td>◆ AgniNav提出了一种配置驱动的单目局部导航框架，使不同机器人形态之间能够在无需重新训练的情况下迁移。

◆ 其核心创新是用四个可测量参数描述机器人安全碰撞包络：碰撞相关高度、前长、后长和半宽。

◆ 高度参数用于条件化图像到伪激光扫描网络，从单目RGB图像预测与碰撞相关的一维扫描信息。

◆ 其余足迹参数直接配置维度感知局部规划器，用于针对不同机器人尺寸进行碰撞检测和路径规划。

◆ 训练阶段利用彩色-深度数据生成高度条件化标签，使同一图像可监督多种安全包络，避免采集特定机器人数据。

◆ 实验表明该方法可在Turtlebot2、Unitree Go2和K1等轮式、四足和类人平台上以30 Hz运行，并取得高成功率和低碰撞率。</td></tr>
...
<h2 id='motion-planning'>Motion Planning (114篇)</h2>
...
<tr><td>2026-06-12</td><td>ParkingTransformer: LLM-Enhanced End-to-End Trajectory Planning for Autonomous Parking<br><a href='http://arxiv.org/pdf/2606.17082'>论文</a></td><td>◆提出ParkingTransformer，将多视角感知与LLM的场景理解能力结合，用于端到端自主泊车轨迹规划，提升语义理解与可解释性。
◆方法将轨迹查询与LLM隐式状态特征融合，直接利用历史信息和原始传感器数据输出轨迹，避免依赖稠密BEV表示。
◆针对LLM空间推理不足，引入3D位置编码，显式注入几何空间信息以增强泊车场景建模能力。
◆设计固定窗口流式历史信息处理机制，提高长时序建模效率，并显著加快推理速度。
◆采用粗到细解码策略逐步优化轨迹精度，并在CARLA和真实车辆实验中取得61.32驾驶分数和88.70%平均成功率，验证了方法有效性。</td></tr>
...
<h2 id='sensor-calibration'>Sensor Calibration (23篇)</h2>
...
<tr><td>2026-05-10</td><td>VFM-SDM: A vision foundation model-based framework for training-free, marker-free, and calibration-free structural displacement measurement<br><a href='http://arxiv.org/pdf/2605.09677'>论文</a></td><td>◆提出VFM-SDM框架，将视觉基础模型用于结构位移测量，实现无需任务专门训练、无需布设标记、无需人工相机标定的非接触式部署。

◆框架结合VFM推断的相机参数估计与点跟踪，通过三角测量重建结构多方向位移，提升真实场景中的自动化测量能力。

◆引入结构几何约束，抑制物理上不合理的位移偏差，从而提高位移估计的一致性与可靠性。

◆构建并发布在役人行桥多模态现场数据集，同时提出统一基准测试流程，支持可复现的性能评估。

◆实验结果显示该方法在竖向和横向位移上具有较低幅值误差、较强时间相关性和较小峰峰值误差，证明其适用于数字孪生和数据驱动建造中的结构响应监测。</td></tr>
...
<h2 id='sensor-undistortion'>Sensor Undistortion (46篇)</h2>
