...
<tr><td>2026-07-10</td><td>Glob3R: Global Structure-from-Motion with 3D Foundation Models<br><a href='http://arxiv.org/pdf/2607.09225'>论文</a> | <a href='https://junyuandeng.github.io/Glob3r'>代码</a></td><td>Glob3R提出基于3D基础模型的全局SfM重建框架，解决现有前馈方法在长序列或大规模无序图像上精度不足、分块处理易漂移的问题。其核心思想是显式优化前馈几何预测结果，通过全局运动平均与光束法平差精化位姿和几何。

◆在冻结的Pi3X骨干上增加轻量级密集匹配头，预测参考帧与相邻视图间的图像形变，并将其转化为稀疏可靠的多视图特征轨迹，为全局优化提供对应约束。

◆提出基于关键帧的滑动窗口关联策略，跨窗口传播轨迹与相对位姿，实现可扩展的全局重建。

◆结合全局运动平均和光束法平差优化相机位姿、消除尺度不一致并恢复稠密场景几何。

实验在室内、室外、大规模驾驶及无序SfM基准上均验证了该方法相对于前馈基线和经典SfM的鲁棒精度优势，并提升了下游神经渲染质量。</td></tr>
<tr><td>2026-07-10</td><td>Empirical Pedestrian Safety Assessment in a Mobile Robot Using a Predictive Social Force Model<br><a href='http://arxiv.org/pdf/2607.09192'>论文</a></td><td>本文针对移动机器人在人行道与行人共存的场景，提出在社会力模型中引入预测机制以提升安全性。研究者将经典社会力模型(SFM)与基于投影碰撞时间的安全变体(TSFM)分别扩展为预测版本PSFM和PTSFM，在有限时间范围内对未来社会力向量进行积分，并部署到非完整约束移动机器人上与真实志愿者进行面对场景实验。

◆ 提出预测型社会力模型PSFM，将社会力在有限时间域内进行预测积分。
◆ 提出预测型TSFM即PTSFM，将预测机制与PTTC安全度量相结合。
◆ 构建兼顾客观指标(最小PTTC、平均速度、轨迹曲率等)与主观Likert问卷的综合评估体系。
◆ 通过真人实验对比四种变体，发现PTTC的引入显著改善安全指标，而预测机制的贡献有限且未带来显著的主观差异。</td></tr>
...
<tr><td>2026-06-22</td><td>G-MASt3R-SfM: Graph-based View Pruning and Multi-stage Optimization for Robust SfM<br><a href='http://arxiv.org/pdf/2606.22856'>论文</a></td><td>G-MASt3R-SfM针对基于MASt3R的SfM方法在处理非重叠图像对时容易产生错误对应、导致位姿估计精度下降的问题，提出了一种新的鲁棒SfM流程。该方法在ETH3D数据集上的实验表明，其在相机位姿估计和三维重建两方面均达到了当前最优精度，能够有效抑制异常匹配带来的噪声。

核心创新点如下：

◆ 图视角剪枝模块（GVP）：基于匹配置信度构建场景图，通过几何关系剔除异常视角，从源头减少不可靠的对应关系进入后续优化。

◆ 多阶段优化模块（MSO）：采用渐进式优化策略，将优化范围从局部一致性逐步扩展到全局一致性，逐层精化相机参数，提升整体位姿估计的鲁棒性与精度。</td></tr>
...
<tr><td>2026-08-03</td><td>Biconvex Optimization for Smooth Minimum-Time Trajectories around Convex Obstacles<br><a href='http://arxiv.org/pdf/2608.02834'>论文</a> | <a href='https://wernerpe.github.io/bmtp-website/'>代码</a></td><td>本文提出了一种用于凸障碍物环境下最小时间运动规划的双凸优化方法,具有收敛保证、任意时刻可停止的特性,并支持任意阶导数约束。该方法通过变量替换将最小时间目标和所有导数约束联合凸化,并使用时变分离超平面处理避碰问题,将原问题转化为双凸规划,通过交替求解最大间隔分离超平面和优化轨迹来求解。

◆ 通过变量替换同时凸化最小时间目标和高阶导数约束,大幅扩展了问题的可解范围。

◆ 采用时变分离超平面处理凸障碍物避碰,仅对当前违反约束的障碍添加平面,使轨迹能跳出局部最小。

◆ 提出双凸交替求解框架,保证从简单无碰折线出发即可全局收敛,且具备任意时刻返回可行解的能力。

◆ 在无人机导航和双臂卸料实验中,该方法在计算效率与现有分解式规划器相当的同时,对更广泛问题类型和较差初始化均表现出显著更强的鲁棒性。</td></tr>
...
<tr><td>2026-07-20</td><td>Does Robust VIO Need More Learning? Geometry-Verified Visual Measurements under Distribution Shift<br><a href='http://arxiv.org/pdf/2607.17956'>论文</a></td><td>这篇论文提出了一种极简学习化的立体视觉惯性里程计框架，挑战了&quot;学习模块越多越鲁棒&quot;的传统观点，主张将学习仅限制在视觉测量生成环节，而时序跟踪、几何验证和状态估计仍保留显式几何方法。该工作利用SEA-RAFT网络生成密集立体匹配并预测像素级不确定性，再通过不确定性阈值与立体极线一致性进行双重过滤，将筛选后的对应关系以不确定性加权重投影因子的形式融入滑窗立体惯性估计器。实验在EuRoC、VIODE和4Seasons数据集上验证了其在运动模糊、动态干扰、光照剧变以及室内外分布偏移下的稳定精度。消融实验进一步证明，学习光流本身并不足够，性能增益来源于学习匹配提议与几何验证、不确定性加权的紧密结合。

◆提出极简学习化立体VIO框架，仅在视觉测量生成环节引入学习，其余环节保留显式几何方法
◆结合预测不确定性与极线一致性对密集光流进行稀疏采样和双重过滤，剔除不可靠匹配
◆在滑窗立体惯性估计器中设计不确定性加权重投影因子，提升对分布外场景的鲁棒性
◆将不确定性传播至立体三角化，支撑下游各向异性3D高斯建图
◆通过系统消融实验揭示学习光流单独使用并不充分，必须与几何验证和不确定性加权结合才能带来性能提升...[摘要不完整，待更新]</td></tr>
