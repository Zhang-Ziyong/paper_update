...
<tr><td>2026-07-02</td><td>LuxSQA: Ask Me in Luxembourgish with TTS-Augmented Spoken Question Answering<br><a href='http://arxiv.org/pdf/2607.02763'>论文</a></td><td>本文针对低资源语言卢森堡语的语音问答任务,探索了利用文本转语音合成数据替代人工录制语料的可行性。研究从现有文本问答资源出发,翻译问题并通过多种TTS系统合成语音问题,与文本答案配对形成训练数据。模型采用参数高效的SLAM架构,将冻结的Whisper编码器通过可学习投影层和LoRA适配器连接到冻结的多语言大语言模型后端。

◆ 提出在低资源卢森堡语场景下,使用TTS合成数据替代大规模人工录制问答语料的完整流程,大幅降低对真实语音标注的依赖。

◆ 构建了包含MMS-TTS、Qwen3-TTS和OmniVoice多源混合的大规模合成语料,涵盖约48k单源及230k多源问题。

◆ 设计了结合冻结Whisper编码器、可学习投影层与LoRA适配器的参数高效架构,有效适配多语言LLM后端。

◆ 实验发现多源混合与基于语音设计的合成配置带来最佳SQA性能,推翻TTS质量分数可预测下游任务表现的假设,强调合成语音应作为任务特定训练数据而非仅追求自然度来评估。</td></tr>
...
<tr><td>2026-06-13</td><td>PEARLS: NuSTAR and XMM-Newton Extragalactic Survey of the JWST North Ecliptic Pole Time Domain Field VI: Multiwavelength SED Analysis<br><a href='http://arxiv.org/pdf/2606.15365'>论文</a></td><td>本文对北黄极时域场中261个X射线源(红移达z~5)进行了多波段能谱分布建模，结合SED拟合与NuSTAR、XMM-Newton的X射线数据，系统分析其恒星形成率与黑洞吸积率的关联。研究发现样本中多数源SFR低于恒星形成主序，而BHAR普遍高于群体平均值，X射线选样本天然偏向活动阶段。通过引入相对SFMS的SFR与比AGN光度L_AGN/M_*的相关性(ρ=+0.73)，识别出X射线明亮且剧烈恒星形成的&quot;冷类星体&quot;群体，揭示短时标剧烈吸积事件的存在。低质量星系BHAR远超其质量对应的群体均值，呈&quot;生长爆发&quot;模式；高质量星系则处于&quot;维持模式&quot;吸积，呈现质量依赖的双模吸积图景。

◆创新点：
◆将X射线观测(SED+X-ray)与SFMS、BHAR双框架结合，首次在统计样本上同时量化SFR偏离主序程度与AGN比光度的相关关系。
◆发现X射线亮且富恒星形成的&quot;冷类星体&quot;种群，表征短时标剧烈吸积阶段，弥补传统分类的盲区。
◆揭示低质量与高质量星系呈现&quot;growth spurt&quot;与&quot;maintenance mode&quot;两种截然不同的黑洞吸积模式。
◆论证X射线视角能识别传统遮蔽/未遮蔽/射电噪分类无法捕捉的稀有AGN演化相位，凸显X射线巡天对瞬时星系-超大质量黑洞耦合研究的关键价值。</td></tr>
...
<thead><tr><th>日期</th><th>标题</th><th>摘要</th></tr></thead>
<tbody>
<tr><td>2026-07-09</td><td>Behavior Foundations for Quadruped Robots: ABot-C0 Technical Report<br><a href='http://arxiv.org/pdf/2607.07370'>论文</a></td><td>本文提出ABot-C0，一个面向四足机器人的通用运动控制系统，建立了三层互补的行为基础框架。研究的核心贡献在于通过条件视频生成合成、动捕标注、遥操作和人工设计构建了数据金字塔，产出16,074条物理可行的运动片段，为多样化运动学习需求提供了数据基础。论文首次在四足机器人上验证了基于Flow-Matching的通用策略存在运动跟踪的扩展规律，并展现出对未见动作的零样本跟踪能力。论文进一步采用三阶段特权信息到感知的框架，结合时序LiDAR记忆与地形预测监督，实现鲁棒的全地形运动能力。最终在城市场景自主导航和陪伴式多模态交互中验证了四足机器人从功能演示迈向产品级行为智能的可行性。

◆ 构建多源数据金字塔，通过条件视频生成、动捕、遥操作和人工设计四种方式产出16,074条物理可行运动片段，解决四足动物运动数据稀缺难题

◆ 首次在四足机器人上发现Flow-Matching通用策略的运动跟踪扩展规律，具备对未见动作的零样本泛化能力

◆ 提出三阶段特权到感知框架，融合时序LiDAR记忆与地形预测监督，实现全地形鲁棒运动控制

◆ 统一的运动通用体系统一协调多策略执行、行为平滑切换、能量高效控制与安全机制，支持真实场景部署...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-02</td><td>Path planning for unmanned naval surface vehicles<br><a href='http://arxiv.org/pdf/2607.01631'>论文</a></td><td>该论文针对无人水面艇（USV）的路径规划问题，分别提出了全局路径规划器和局部路径规划器的新颖组合方案。全局规划器融合了三种方法，包括经典Grassfire算法、其改进版本以及一种新的、更为直观且基于概率路线图（PRM）的方法，用于避开已知固定障碍物。局部规划器则基于对障碍物运动方向的观测，引入了高层决策逻辑，使USV能够系统性地选择从障碍物后方绕行的避障策略，而非被动地平行航行等待通过机会。

◆ 提出三种路径规划器组合的全局规划方案，融合Grassfire算法及其改进版本与新型概率路线图方法，提升已知固定障碍物的规避能力。

◆ 改进Grassfire算法，增强其在全局路径搜索中的效率和适应性。

◆ 重新设计概率路线图方法，使其更直观易用，适用于USV全局路径规划场景。

◆ 局部规划器引入基于障碍物运动方向观测的高层决策逻辑，使USV能主动选择从障碍物后方绕行的避障策略。

◆ 通过仿真验证了所提方法的有效性，并与D*算法及其他动态规划系统进行了对比分析。</td></tr>
...
<tr><td>2026-06-03</td><td>A Geometry-Informed Computer Vision Method for Detecting and Examining Overtaking Vehicles From A Bicycle<br><a href='http://arxiv.org/pdf/2606.23699'>论文</a></td><td>本文针对车辆超车事件依赖人工逐帧标注的瓶颈，提出基于几何信息的计算机视觉自动化检测流程。该方法将RT-DETR目标检测与ByteTrack多目标跟踪相结合，并通过基于透视投影原理的三阶段几何验证模块（方位角趋势、表观尺寸增长、空间确认）过滤候选事件，无需多传感器或相机标定。在密歇根州安娜堡315个真实超车事件验证中，召回率达97.8%且零误报。系统平均提前2.44秒识别超车意图，84.1%超过1.5秒人类反应阈值，具备支持主动骑行者警告的潜力。提出的无标定横向距离估算方法误差仅13-14厘米，可有效区分近距离与正常通过事件。

◆ 基于透视投影原理的三阶段几何验证机制，无需相机标定...[摘要不完整，待更新]</td></tr>
