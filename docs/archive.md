...
<ol>
<li><a href='#slam'>SLAM (47篇)</a></li>
<li><a href='#sfm'>SFM (35篇)</a></li>
<li><a href='#image-matching'>Image Matching (11篇)</a></li>
<li><a href='#obstacle-avoidance'>Obstacle Avoidance (68篇)</a></li>
<li><a href='#navigation'>Navigation (62篇)</a></li>
<li><a href='#motion-planning'>Motion Planning (93篇)</a></li>
<li><a href='#sensor-calibration'>Sensor Calibration (20篇)</a></li>
<li><a href='#sensor-undistortion'>Sensor Undistortion (39篇)</a></li>
</ol>
</details>

<h2 id='slam'>SLAM (47篇)</h2>
...
<h2 id='obstacle-avoidance'>Obstacle Avoidance (68篇)</h2>
...
<h2 id='navigation'>Navigation (62篇)</h2>

<div class="table-container">
<table>
<thead><tr><th>日期</th><th>标题</th><th>摘要</th></tr></thead>
<tbody>
<tr><td>2026-06-11</td><td>NavWAM: A Navigation World Action Model for Goal-Conditioned Visual Navigation<br><a href='http://arxiv.org/pdf/2606.13494'>论文</a> | <a href='https://dachii-azm.github.io/navwam/'>代码</a></td><td>这篇论文提出了NavWAM(导航世界动作模型),一种面向目标条件视觉导航任务的扩散Transformer策略。传统的导航世界模型只能进行视觉预测,需要外部规划器(如CEM动作搜索)才能将预测转化为闭环控制,而NavWAM将世界模型的预测能力与动作生成统一到同一框架中,使视觉前瞻能力可直接用于机器人控制。该模型通过仿真预训练加真实机器人适配的两阶段方式构建,在离线基准测试和真实机器人闭环部署中均超越了基于规划的世界模型基线方法,且无需CEM风格的动作搜索。

◆首次将导航世界模型的未来预测、目标进展价值估计与动作块生成统一在共享的潜在序列中,通过扩散Transformer策略联合建模
◆摆脱了传统世界模型对外部规划器的依赖,直接以默认策略模式输出可执行动作,避免了CEM动作搜索的高计算开销
◆通过将未来预测与决定闭环行为的动作目标、价值目标联合学习,使视觉前瞻能力真正服务于控制任务而非仅作为预测模块
◆采用仿真预训练加真实机器人适配的训练流程,有效弥合了仿真与现实的差距,实现了从离线评测到真机闭环部署的一致性能提升...[摘要不完整，待更新]</td></tr>
...
<h2 id='motion-planning'>Motion Planning (93篇)</h2>
...
<tr><td>2026-06-11</td><td>Distribution-Agnostic Robust Trajectory Optimization via Chance-Constrained Reinforcement Learning<br><a href='http://arxiv.org/pdf/2606.13605'>论文</a></td><td>本文提出了一种基于机会约束强化学习的分布无关鲁棒轨迹优化框架,通过初始条件和过程噪声建模不确定性,仅要求其可采样,无需已知概率分布。该方法先离线计算确定性标称轨迹,再利用强化学习训练结构化仿射闭环修正律(包含前馈控制调整与时变反馈增益)对基线进行鲁棒化处理,通过基于蒙特卡洛仿真的上尾分位数经验性地施加概率可行性,并通过协方差可行性惩罚项调控终端散布。该框架在三维多脉冲地火转移和大气层内火箭定点着陆两个异构问题上得到验证,在高斯及未见过的均匀不确定性和过程扰动下均表现良好。

◆ 提出分布无关的鲁棒轨迹优化框架,仅要求不确定性可采样,摆脱了对高斯等特定分布假设的依赖
◆ 设计了&quot;确定性标称轨迹+强化学习鲁棒化修正&quot;的两阶段架构,将传统优化与RL优势相结合
◆ 采用结构化仿射闭环修正律,包含前馈控制调整和时变反馈增益,具有明确的随机控制结构
◆ 通过基于轨迹采样的上尾分位数经验性地强制机会约束,并以协方差可行性惩罚调控终端散布
◆ 同一鲁棒化框架可无缝迁移到脉冲式行星际转移和连续推力大气层着陆等异构问题,无需重新设计核心结构
◆ 在高斯训练后能泛化到训练中未见的均匀不确定性和过程扰动场景,展示出跨分布的鲁棒性...[摘要不完整，待更新]</td></tr>
...
该论文针对非结构化动态环境中的反应式运动生成问题,提出了G-MAPP框架,通过GPU加速实现感知与规划模块的紧密耦合,解决了高保真环境下规划运行时性能瓶颈以及感知-规划时序集成难题。作者在7自由度Franka Emika机械臂上进行了真实世界实验验证,定量比较了CPU与GPU版本规划器的计算时间和成功率差异,结果表明GPU框架最高可获得5倍加速,并能在普通和具有挑战性的场景中成功避障。

◆ 提出了基于GPU加速的多智能体规划与感知一体化框架,在不牺牲世界表征精度和运行时性能的前提下实现实时反应式运动生成
◆ 利用GPU并行能力加速世界建模和基于向量场的规划方法,实现了更快的并行状态探索,支持准全局轨迹规划
◆ 实现了感知-动作闭环的紧耦合,使系统能够使用普通深度传感器在动态杂乱环境中实时响应
◆ 相较于CPU版本实现了最高5倍的运行速度提升,显著改善了高保真环境下的规划效率
◆ 通过真实机器人实验验证了框架在简单及复杂物理场景中的避障可行性,弥补了传统方法仅适用于静态场景或过度保守假设的不足...[摘要不完整，待更新]</td></tr>
...
<h2 id='sensor-calibration'>Sensor Calibration (20篇)</h2>
...
<h2 id='sensor-undistortion'>Sensor Undistortion (39篇)</h2>
