...
<ol>
<li><a href='#slam'>SLAM (160篇)</a></li>
<li><a href='#sfm'>SFM (77篇)</a></li>
<li><a href='#image-matching'>Image Matching (32篇)</a></li>
<li><a href='#obstacle-avoidance'>Obstacle Avoidance (189篇)</a></li>
<li><a href='#navigation'>Navigation (207篇)</a></li>
<li><a href='#motion-planning'>Motion Planning (308篇)</a></li>
<li><a href='#sensor-calibration'>Sensor Calibration (49篇)</a></li>
<li><a href='#sensor-undistortion'>Sensor Undistortion (104篇)</a></li>
</ol>
</details>

<h2 id='slam'>SLAM (160篇)</h2>
...
<tr><td>2026-07-10</td><td>Glob3R: Global Structure-from-Motion with 3D Foundation Models<br><a href='http://arxiv.org/pdf/2607.09225'>论文</a> | <a href='https://junyuandeng.github.io/Glob3r'>代码</a></td><td>Glob3R是一种基于3D基础模型的全局式SfM重建框架，旨在解决现有前馈重建方法精度不足以及分块处理导致的漂移和不一致问题。其核心思想是显式优化前馈几何预测结果，将基础模型的强大先验与全局几何优化相结合。该方法在多个室内、室外、大规模驾驶和无序图像基准上均取得了鲁棒且精确的重建效果，并优于经典SfM流程，精细化后的位姿还能提升神经渲染质量。

◆在冻结的Pi3X骨干网络上增加轻量级稠密匹配头，预测参考帧与相邻视角间的图像warp，转化为稀疏可靠的多视图特征轨迹以提供对应约束。

◆提出基于关键帧的滑动窗口关联策略，通过重叠窗口传播轨迹和相对位姿，实现可扩展的大规模重建。

◆采用全局运动平均和束调整联合优化，有效精化相机位姿、消除尺度不一致性并恢复稠密场景几何。</td></tr>
...
<tr><td>2026-06-22</td><td>G-MASt3R-SfM: Graph-based View Pruning and Multi-stage Optimization for Robust SfM<br><a href='http://arxiv.org/pdf/2606.22856'>论文</a></td><td>G-MASt3R-SfM针对基于MASt3R的SfM方法在非重叠图像对上产生错误对应关系导致位姿估计退化的问题，提出了一种新的鲁棒SfM流程。该方法通过构建场景图并基于匹配置信度对异常视图进行几何剪枝，有效剔除了不可靠的匹配对。优化过程采用由局部一致性逐步扩展到全局一致性的多阶段策略，对相机参数进行渐进式精炼。在ETH3D数据集上的实验表明，该方法在相机位姿估计和三维重建精度方面均达到了当前最优水平，并有效抑制了异常值带来的噪声。

◆ 图引导视图剪枝(GVP)模块：基于匹配置信度构建场景图，通过几何方式剪除离群视图，从源头过滤不可靠的对应关系。

◆ 多阶段优化(MSO)模块：采用由局部到全局的渐进式优化策略，分阶段精炼相机参数，兼顾优化效率与全局一致性。</td></tr>
...
<h2 id='obstacle-avoidance'>Obstacle Avoidance (189篇)</h2>
...
<h2 id='navigation'>Navigation (207篇)</h2>
...
<h2 id='motion-planning'>Motion Planning (308篇)</h2>
...
<h2 id='sensor-undistortion'>Sensor Undistortion (104篇)</h2>
