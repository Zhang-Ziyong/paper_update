...
<li><a href='#obstacle-avoidance'>Obstacle Avoidance (185篇)</a></li>
<li><a href='#navigation'>Navigation (204篇)</a></li>
<li><a href='#motion-planning'>Motion Planning (303篇)</a></li>
...
<tr><td>2026-07-10</td><td>Empirical Pedestrian Safety Assessment in a Mobile Robot Using a Predictive Social Force Model<br><a href='http://arxiv.org/pdf/2607.09192'>论文</a></td><td>该论文针对移动机器人在人行道上与行人共享空间时的安全性问题展开研究，核心关注客观安全与行人主观舒适感的平衡。研究者在传统社会力模型(SFM)和基于投影碰撞时间的社会力模型(TSFM)基础上，创新性地提出预测型SFM(PSFM)和预测型TSFM(PTSFM)，通过在有限时间范围内对预测的社会力向量进行积分来提升预测能力。文章在非完整约束移动机器人平台上实现了四种模型，并与志愿者开展面对面对抗场景的实测试验，从客观指标(最小PTTC、平均速度、最小距离、横向距离、最大轨迹曲率)和主观Likert问卷两个维度进行系统评估。

主要创新点如下：

◆ 提出预测型社会力模型PSFM和PTSFM，在有限时间窗口内集成预测社会力向量

◆ 首次在真实非完整约束移动机器人上系统对比SFM、TSFM、PSFM、PTSFM四种模型

◆ 构建融合客观安全指标与主观舒适度问卷的综合评估框架

◆ 通过实验验证PTTC集成能显著提升安全性能，而预测机制的额外收益在单行人场景中有限...[摘要不完整，待更新]</td></tr>
...
<h2 id='obstacle-avoidance'>Obstacle Avoidance (185篇)</h2>
...
<h2 id='navigation'>Navigation (204篇)</h2>
...
<h2 id='motion-planning'>Motion Planning (303篇)</h2>
...
<tr><td>2026-08-06</td><td>Plan-and-Avoid: Real-Time Aircraft Trajectory Coordination in a Multi-Agent Environment<br><a href='http://arxiv.org/pdf/2608.06648'>论文</a></td><td>该论文提出了一种名为Plan-and-Avoid（PAA）的实时协同多智能体空域运行框架，用于在已声明的优先航迹周围协调多架飞机的飞行冲突。研究团队设计了一套综合方法：首先预测与周围交通的不确定性感知间隔冲突，当优先航迹单独无法维持安全间隔时，生成受飞行器自身机动能力约束的单向避让建议。该框架的Plan组件以应急着陆规划器为应用案例，Avoid组件则对靠近优先航迹的飞机发出避让指令。基于华盛顿特区空域超过900个强制着陆案例和140多小时的真实ADS-B仿真数据验证，PAA在最坏情况下端到端响应时间仅为5.7秒，并使93.5%的避让建议满足RTCA DO-365标准规定的35秒时间阈值。

◆ 提出Plan-and-Avoid一体化框架，将优先航迹规划与冲突规避紧密结合，适用于任何已声明的优先航迹场景。

◆ 引入不确定性感知机制，对间隔冲突预测进行风险量化，提升避让决策的鲁棒性。

◆ 生成受飞行器机动能力约束的单向避让建议，确保建议在物理上可执行，避免不可行的调度方案。

◆ 基于真实ADS-B数据进行大规模仿真验证，覆盖900余案例，证明了框架的实时性与工程实用性。</td></tr>
