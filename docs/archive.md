...
<ol>
<li><a href='#slam'>SLAM (77篇)</a></li>
<li><a href='#sfm'>SFM (46篇)</a></li>
<li><a href='#image-matching'>Image Matching (13篇)</a></li>
<li><a href='#obstacle-avoidance'>Obstacle Avoidance (100篇)</a></li>
<li><a href='#navigation'>Navigation (92篇)</a></li>
<li><a href='#motion-planning'>Motion Planning (135篇)</a></li>
<li><a href='#sensor-calibration'>Sensor Calibration (27篇)</a></li>
<li><a href='#sensor-undistortion'>Sensor Undistortion (59篇)</a></li>
</ol>
</details>

<h2 id='slam'>SLAM (77篇)</h2>
...
<tr><td>2026-06-18</td><td>Gaussian Process Prior Variational Autoencoder for Endoscopic Videos<br><a href='http://arxiv.org/pdf/2606.19908'>论文</a></td><td>◆ 论文提出GPVAE用于内窥镜视频修复，将传统VAE的独立潜变量先验替换为时间高斯过程先验，以利用帧间连续性处理缺帧和瞬时退化。

◆ 方法支持带不确定性的缺失帧插值与重建，使恢复结果不仅给出图像，还能提供逐帧置信度估计。

◆ 框架结合EndoVAE卷积编码器和GastroNet-5M预训练ViT编码器，并引入HPA与SPA两种可扩展GP近似以提升实用性。

◆ 对镜面反射，论文使用DUCKNet掩膜管线在训练目标中排除受污染像素，从而增强对内窥镜特有伪影的鲁棒性。

◆ 在C3VDv2数据集上，最佳GPVAE相比匹配VAE平均降低重建RMSE 21.9%，最高26.1%，并使下游轨迹RMSE平均降低12.7%，代价是每轮训练时间平均增加27.3%。</td></tr>
...
◆ 方法设计了点-线融合位姿优化策略，将3D线段作为额外几何约束，增强相机跟踪的鲁棒性和精度。
◆ 论文提出Multi-Meta Gaussian表示，通过 dominant directions 编码结构信息，使高斯地图更符合真实场景几何布局。
◆ 其Gaussian evolution策略可根据场景几何自适应更新高斯，并在全局优化中融入结构线索，减少地图漂移和不一致。
◆ 实验表明，MMD-SLAM在跟踪和建图上达到SOTA，相比MonoGS在ScanNet上ATE RMSE降低48.56%，在Replica上PSNR提升5.71%。</td></tr>
<tr><td>2026-06-18</td><td>UNSEEN: Uncertainty-aware Navigation via Sparse Estimation in Unknown Environments<br><a href='http://arxiv.org/pdf/2606.20755'>论文</a></td><td>◆ 提出UNSEEN，一个仅依赖前视相机的统一导航框架，在未知环境中紧密耦合定位、建图与规划，降低对多传感器和强环境假设的依赖。
◆ 框架显式传播并利用位姿与稀疏地图的不确定性，使导航决策不再只追求到达目标，也兼顾感知与估计质量。
◆ 设计UNSEEN-SLAM，以6Hz实时估计稀疏地图、机器人位姿及其不确定性，在视觉退化条件下提升定位鲁棒性。
◆ 提出UNSEEN-Plan，在滚动时域内联合优化任务进展和估计精度，主动选择更有利于感知的运动轨迹。
◆ 仿真和真实实验表明，该方法相比先进方法将平移绝对误差降低9.8%，规划带来的估计精度最高提升45%，并实现100%任务成功率。</td></tr>
...
<h2 id='image-matching'>Image Matching (13篇)</h2>
...
<h2 id='obstacle-avoidance'>Obstacle Avoidance (100篇)</h2>
...
<h2 id='navigation'>Navigation (92篇)</h2>
...
<tr><td>2026-06-15</td><td>VISTA: Scale-Aware Visual Navigation via Action History Conditioning<br><a href='http://arxiv.org/pdf/2606.17294'>论文</a></td><td>◆ 论文指出，视觉导航基础模型使用归一化动作会带来尺度部署风险：同一归一化轨迹在不同缩放下会改变真实几何形状，导致性能下降和碰撞增加。
◆ VISTA的核心创新是将归一化动作历史与图像观测共同作为条件输入，使模型显式理解预测动作与机器人真实位移之间的关系。
◆ 该方法提升了模型对不同机器人尺度、动作执行幅度和部署环境的适应能力，从而增强零样本导航鲁棒性。
◆ 为应对视觉重复、缺少显著特征的场景，VISTA引入DINOv3编码器，以获得更丰富的空间与几何表征。
◆ 实验显示，VISTA在户外、森林和办公室等真实未见环境中实现100%零样本目标预测准确率，并平均通过95%的路径检查点，证明其具有稳定的路径跟随能力。</td></tr>
...
<h2 id='motion-planning'>Motion Planning (135篇)</h2>
...
<h2 id='sensor-calibration'>Sensor Calibration (27篇)</h2>
...
<h2 id='sensor-undistortion'>Sensor Undistortion (59篇)</h2>
...
<tr><td>2026-06-11</td><td>Observation of intertwined charge density wave order and superconductivity in Janus monolayer<br><a href='http://arxiv.org/pdf/2606.13828'>论文</a></td><td>◆ 该论文首次系统研究了Janus单层1T-ZrSeTe中电荷密度波与超导不稳定性的共存和相互关联。

◆ 第一性原理计算发现，声子谱在M点出现显著软化，源于增强的电子-声子耦合以及带间、带内散射共同导致的电子不稳定性。

◆ CDW畸变会重构能带并打开小的间接带隙，使体系由半金属转变为半导体。

◆ 与单层ZrTe2相比，ZrSeTe的畸变能量增益更小，说明用Se替代一层Te会削弱CDW不稳定性。

◆ 研究还指出电子关联和双轴应变可有效调控CDW与超导，而未畸变高温相中存在由M点软声子主导的声子介导双能隙超导，SOC会降低其超导转变温度。</td></tr>
