...
<tr><td>2026-07-14</td><td>DiffRadar: Differentiable Physics-Aware Radar SLAM with Gaussian Fields<br><a href='http://arxiv.org/pdf/2607.12265'>论文</a></td><td>现有雷达SLAM系统依赖离散化雷达热图的扫描匹配，破坏了几何连续性，在特征退化或动态环境下姿态估计易失效。为此本文提出DiffRadar，将雷达观测建模为可微且物理感知的连续高斯场，在信号域直接进行SLAM。

◆将场景表示为各向异性高斯原语，通过可微雷达前向模型在距离-方位与多普勒-方位空间渲染雷达测量，实现机器人位姿与场景结构的端到端联合优化。

◆基于商品化FMCW雷达硬件...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-13</td><td>GeoGS-SLAM: Online Monocular Reconstruction Using Gaussian Splatting with Geometric Priors<br><a href='http://arxiv.org/pdf/2607.11184'>论文</a> | <a href='https://rlgao.github.io/geogs_slam'>代码</a></td><td>GeoGS-SLAM提出了一种基于3D高斯泼溅与几何先验融合的在线单目稠密重建系统,旨在解决现有方法对深度传感器的依赖或丢弃RGB信息导致重建质量下降的问题。该系统首先利用预训练的前馈视觉几何模型从无标定RGB输入中预测相机位姿和场景几何先验,再通过直接采样RGB与几何先验生成高斯基元来扩展场景地图。相机位姿与场景地图通过由粗到细的策略进行联合优化,结合光度损失与几何损失以提升精度。为保证全局一致性,系统还引入了在线回环检测与位姿图优化。大量室内外基准实验表明,该方法在保持在线实时性的同时,渲染质量与跟踪精度均优于当前最先进方法。

◆ 提出融合3DGS表征与前馈几何先验的在线单目稠密SLAM框架,无需深度传感器即可实现高质量重建
◆ 通过直接从RGB与几何先验中采样高斯基元构建场景地图,有效融合外观与几何信息
◆ 设计由粗到细的联合优化策略,联合最小化光度损失与几何损失以提升跟踪与建图精度
◆ 集成在线回环检测与位姿图优化,保障大规模场景下的全局一致性...[摘要不完整，待更新]</td></tr>
...
<thead><tr><th>日期</th><th>标题</th><th>摘要</th></tr></thead>
<tbody>
<tr><td>2026-07-15</td><td>Discriminative Barrier Functions for Safe Adversarial Imitation Learning from Observation<br><a href='http://arxiv.org/pdf/2607.13938'>论文</a></td><td>本文针对逆强化学习（IRL）依赖无约束探索而难以安全部署的问题，将控制屏障函数（CBFs）引入IRL框架，提出一种判别性屏障函数方法。该方法将奖励函数候选空间约束于CBFs空间，使智能体在学习专家行为的同时获得安全保障，并支持在线持续改进。研究表明，所恢复的屏障函数能够从无标签专家观测中以数据驱动方式获得，且对专家数据中未出现的危险状态具有鲁棒性。通过仿真导航和真实避障实验，验证了该方法相比标准IRL基线在安全性上的优势，并探讨了基于规划与基于策略的IRL方法在安全任务中的权衡。

◆ 将IRL奖励函数候选空间约束于CBFs空间，统一安全性与模仿学习
◆ 从无标签专家观测中以数据驱动方式自动恢复屏障函数
◆ 恢复的屏障函数对专家数据中未出现的危险状态具有鲁棒性
◆ 在仿真与真实机器人避障任务中均验证了安全性能提升
◆ 系统比较了基于规划与基于策略两类IRL方法在安全场景下的权衡...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-13</td><td>More than a Manipulator: Planning Propellant-Free Attitude Maneuvers for Free-Floating Spacecraft<br><a href='http://arxiv.org/pdf/2607.12130'>论文</a></td><td>本文针对自由漂浮航天器姿态控制问题，创新性地将机械臂从传统干扰源重新定位为主动姿态控制执行机构。研究通过构建包含关节约束和碰撞避免约束的轨迹优化非线性规划问题，并采用内点法求解，实现了无推进剂消耗的复杂姿态机动与消旋轨迹。

◆ 将机械臂运动作为无推进剂姿态控制手段，突破了机械臂仅作为干扰源的传统认知框架
◆ 提出融合关节限位与碰撞规避的轨迹优化方法，可适配不同运动学复杂度与质量特性的航天器-机械臂系统
◆ 通过动量与力矩包络对比分析，定量验证了机械臂作为反作用轮冗余甚至主用姿态控制系统的能力
◆ 该方法在高相对质量比载荷的太空在轨装配与制造任务中具有重要应用价值...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-10</td><td>SEAMLiS: Visibility-Aware Safety for Perception-Limited Multi-Robot Exploration<br><a href='http://arxiv.org/pdf/2607.09959'>论文</a></td><td>SEAMLiS针对有限感知范围和视场下多机器人自主探索中规划与避障分离可能导致碰撞的问题，提出了一个模块化的执行层安全框架。该框架保留上游探索栈，在执行层通过感知感知的姿态和位置滤波器强制保证安全性。

◆ 门控器式姿态滤波器在可见性优先的偏航策略与速度跟踪备用策略间自适应切换，以保持对已知自由/未知边界的可见性并预留足够制动余量
◆ 基于控制屏障函数的位置滤波器可同时规避已知障碍、新检测障碍以及其他机器人
◆ 给出了充分的避碰条件，并在随机仿真、Isaac Sim和Crazyflie硬件实验中验证了单/多机器人场景下的无碰撞探索，同时保留了可见性偏航控制的高效率...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-15</td><td>From Language to Navigation Goals: A Vision-Language Approach for Semantic Navigation of Mobile Robots Using RGB-D Perception<br><a href='http://arxiv.org/pdf/2607.13624'>论文</a></td><td>该论文提出了一种基于视觉-语言驱动的移动机器人语义导航框架,旨在让非专业用户通过自然语言指令(如&quot;去邮箱&quot;)实现直观的人机交互。系统采用模块化的ROS 2架构,将语言理解、RGB-D环境感知和自主导航无缝集成,能够从自然语言请求中识别目标物体,利用深度相机估计其位置,并通过Nav2导航栈生成并执行导航目标。该框架在TurtleBot3 Waffle和Unitree Go2机器人平台上分别完成了仿真和真实场景验证,证明了其良好的可移植性。实验结果表明,系统不仅能正确处理直接指令,还能理解上下文相关请求,并向用户提供自然语言反馈。

◆ 提出基于ROS 2模块化组件的端到端语言到导航目标转换框架,集成视觉-语言理解与自主导航
◆ 利用RGB-D感知实现目标物体识别与三维定位,无需预先建图即可生成导航目标
◆ 具备跨机器人平台的可移植性,仅需配置对应话题和服务即可在不同平台上部署
◆ 支持上下文间接指令的语义解析,并能生成自然语言形式的执行反馈
◆ 代码将开源发布,为后续自然语言机器人导航研究提供可复现的基准...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-15</td><td>WNOJ-LIO: A White-Noise-on-Jerk Motion-Prior EKF for High-Dynamic LiDAR-IMU Fusion<br><a href='http://arxiv.org/pdf/2607.13405'>论文</a> | <a href='https://github.com/LvJohny/wnoj-ekf-lio.git'>代码</a></td><td>WNOJ-LIO是针对高动态驾驶场景提出的LiDAR-IMU紧耦合里程计算法，旨在同时解决扫描内运动畸变与惯性测量受振动污染两个耦合难题。传统方法通过积分原始IMU来传播系统状态并用于点云去畸变，会将惯性噪声传递到校正后的点云与配准过程中。WNOJ-LIO的核心思想是将IMU从状态传播的驱动者重新定位为高频测量源，并采用白噪声加加速度(WNOJ)运动先验在R^3×SO(3)上进行解耦的状态预测。该方法通过闭式协方差传播弥合了批量WNOJ高斯过程轨迹先验与递推滤波之间的差距。论文在仿真和真实赛车(最高208 km/h)上进行了系统验证，结果表明该方法在加速度与角速度去噪、扫描去畸变以及定位精度方面均优于FAST-LIO基线。

创新点：
◆ 提出基于WNOJ先验的EKF框架，将IMU从传播驱动改为测量源，从源头避免惯性噪声对去畸变点云与扫描配准的污染
◆ 设计R^3×SO(3)上的解耦过程模型，实现闭式协方差传播
◆ 首次在最高208 km/h的赛车场景下对加速度、角速度、线速度、姿态和位置估计进行综合性能评估...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-15</td><td>Breaking Déjà Vu: Independent Auditing of Visual Place Recognition through Vision-Language Reasoning<br><a href='http://arxiv.org/pdf/2607.12818'>论文</a></td><td>本文提出了一种名为&quot;视觉地点识别审计&quot;（Visual Place Recognition Auditing）的独立后检索验证框架，旨在解决传统视觉地点识别（VPR）方法依赖固定阈值而难以应对环境变化的问题。该方法利用视觉语言模型（VLM）对查询图像和候选图像进行联合推理，独立于底层VPR架构进行实例级真伪判断，无需架构特定的置信度度量、依赖数据集的阈值或部署环境的先验知识。作者在六个基准数据集上对五种先进的VPR方法和四种VLM进行了全面实验，结果表明该审计框架平均将recall@1提升了13.6%，同时将错误接受率降低至12%，并保持精度高于95%、覆盖率高于75%，显著优于现有方法。

◆ 首创VPR独立审计范式，将VLM作为外部验证器进行后检索审查，而非修改VPR模型本身
◆ 摆脱对架构特定置信度、预设阈值和环境先验知识的依赖，实现真正即插即用的实例级真伪判定
◆ 在六个数据集、五种VPR方法与四种VLM上取得一致提升，平均recall@1提高13.6%，兼顾精度与覆盖率...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-15</td><td>Merging Reaction to Cognition: A Hybrid Cognitive Strategy for Odour Source Localisation in Natural Environments<br><a href='http://arxiv.org/pdf/2607.13853'>论文</a></td><td>本文针对湍流自然环境中化学污染源定位难题,提出一种融合反应式行为与认知概率推理的混合策略,旨在克服纯反应式方法依赖场景调参与纯认知方法过度探索的双重局限。

◆ 核心创新在于设计检测触发的切换机制,显式地将生物启发反应行为融入信念依赖的运动规划,实现横流探索与趋源运动之间的形式化自动转换。

◆ 进一步将行为...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-15</td><td>Unifying Decision-Making and Trajectory-Planning in Unsignalized Intersections Using Time-Varying Potential Fields<br><a href='http://arxiv.org/pdf/2607.13626'>论文</a></td><td>本文针对无信号交叉口场景下自动驾驶车辆的决策与轨迹规划问题，提出了一种基于时变人工势场（TV-APF）的有限时域最优控制（FHOCP）一体化框架。该方法将传统上分开处理的决策与轨迹规划统一在同一个优化问题中求解，避免了两阶段方法可能产生的次优解与不一致问题。框架通过短时域运动预测和专用的冲突区域占用系数评估碰撞风险，使势场能随时间动态反映其他交通参与者的状态变化。仿真结果在多车交通场景中验证了所提方法能生成可行且安全的参考轨迹。

创新点：
◆ 提出基于时变人工势场的FHOCP框架，将决策与轨迹规划统一建模与求解
◆ 设计冲突区域占用系数，将潜在碰撞风险显式嵌入势场函数
◆ 利用短时域运动预测驱动势场的时间演化，提升对动态冲突的预判能力...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-14</td><td>Worlds in One Demo: A Synthetic Data Engine for Learning Open-World Mobile Manipulation<br><a href='http://arxiv.org/pdf/2607.13154'>论文</a></td><td>WANDA是一个从单次演示中学习开放世界移动操作的合成数据引擎，旨在通过最大化...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-13</td><td>CR-Solver: GPU-Accelerated Kinematics Solver for Tendon-driven Continuum Robots<br><a href='http://arxiv.org/pdf/2607.11340'>论文</a></td><td>针对连续体机器人运动规划工具匮乏的问题，本文提出CR-Solver，一种基于GPU加速的两阶段优化求解器。该方法将逆运动学、路径跟随和轨迹规划统一在同一个约束非线性优化框架中，突破了现有规划库基于刚体假设的局限。在三个任务上的验证表明，CR-Solver相比传统CPU求解器获得显著加速，同时保持毫米级精度和95%以上的成功率。

◆ 利用GPU并行计算显著加速求解过程，相比传统CPU求解器实现大幅速度提升
◆ 统一优化框架融合多种运动生成任务，以单一约束非线性优化问题一体化解决
◆ 纯Python实现降低了使用门槛，为连续体机器人高性能运动规划提供可扩展基础...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-13</td><td>Diffusion MRI preprocessing affects ADC estimation and automatic PI-RADS v2.1 classification in bi-parametric prostate MRI<br><a href='http://arxiv.org/pdf/2607.11385'>论文</a></td><td>本论文系统研究了DWI预处理策略对前列腺双参数MRI中ADC估计和PI-RADS v2.1自动分类的影响。研究基于fastMRI数据集的268例样本,依次施加去噪、振铃校正和微分同胚畸变校正三种处理,并比较了LLS与IWLLS两种ADC拟合方法。分类任务采用3类DenseNet模型预测PI-RADS评分。结果显示,不同预处理流程导致ADC值存在统计学显著差异,LLS与IWLLS结果数值等价,而畸变校正对ADC值影响最大。完全预处理数据集取得最佳AUROC和敏感性,且对高风险类别产生最少的过度自信错误预测,有助于临床分诊应用。

◆ 首次系统评估DWI多步骤预处理对ADC估计和深度学习PI-RADS分类的联合影响
◆ 揭示畸变校正是影响ADC定量与分类性能的关键预处理环节
◆ 证明完整预处理流程可减少高风险病灶的过度自信误判,具临床分诊价值...[摘要不完整，待更新]</td></tr>
...
<tbody>
<tr><td><a href='https://github.com/hku-mars/FAST_LIO'>FAST_LIO</a></td><td>4947</td><td>A computationally efficient and robust LiDAR-inert</td></tr>
<tr><td><a href='https://github.com/hku-mars/FAST-LIVO2'>FAST-LIVO2</a></td><td>4364</td><td>FAST-LIVO2: Fast, Direct LiDAR-Inertial-Visual Odo</td></tr>
...
<tr><td><a href='https://github.com/hku-mars/livox_camera_calib'>livox_camera_calib</a></td><td>1272</td><td>This repository is used for automatic calibration </td></tr>
<tr><td><a href='https://github.com/hku-mars/FAST-Calib'>FAST-Calib</a></td><td>987</td><td>A Handy Extrinsic Calibration Tool for LiDAR-camer</td></tr>
<tr><td><a href='https://github.com/hku-mars/SUPER'>SUPER</a></td><td>966</td><td>SUPER</td></tr>
...
<tr><td><a href='https://github.com/hku-mars/Voxel-SLAM'>Voxel-SLAM</a></td><td>644</td><td>Voxel-SLAM</td></tr>
...
<tr><td><a href='https://github.com/hku-mars/ROG-Map'>ROG-Map</a></td><td>583</td><td>ROG-Map</td></tr>
...
<tr><td><a href='https://github.com/hku-mars/UMI-3D'>UMI-3D</a></td><td>256</td><td>UMI-3D SLAM and Data Processing Pipeline: https://</td></tr>
...
<tr><td><a href='https://github.com/ethz-asl/voxblox'>voxblox</a></td><td>1654</td><td>A library for flexible voxel-based mapping, mainly</td></tr>
<tr><td><a href='https://github.com/ethz-asl/okvis'>okvis</a></td><td>1360</td><td>OKVIS: Open Keyframe-based Visual-Inertial SLAM.</td></tr>
...
<tr><td><a href='https://github.com/ethz-asl/polygon_coverage_planning'>polygon_coverage_planning</a></td><td>651</td><td>Coverage planning in general polygons with holes.</td></tr>
...
<tr><td><a href='https://github.com/ethz-asl/COIN-LIO'>COIN-LIO</a></td><td>496</td><td>🪙 COIN-LIO: Complementary Intensity-Augmented LiDA</td></tr>
...
<tr><td><a href='https://github.com/ethz-asl/BIEVR-LIO'>BIEVR-LIO</a></td><td>162</td><td>[RSS 2026] 🦫 BIEVR-LIO: Robust LiDAR-Inertial Odom</td></tr>
