...
<tr><td>2026-07-06</td><td>Geometry-Aware Visual Odometry for Bronchoscopic Navigation via High-Gain Observer Fusion<br><a href='http://arxiv.org/pdf/2607.05162'>论文</a></td><td>针对现有导航支气管镜依赖术前CT或外部传感器、在重症监护等场景难以推广的局限，本文提出一种面向气道管腔几何的纯视觉里程计框架。该方法将检测到的管腔消失点反向投影为三维射线并加权融合以获得稳定前进方向，配合基于looming效应的速度估计，通过定制的高增益观测器与视觉里程计输出融合，强制执行气道跟随先验并抑制漂移。

◆ 显式建模管腔消失点几何，将二维检测反向投影为三维射线并加权融合，在视差缺失时仍提供稳定方向估计
◆ 引入基于looming效应的速度估计，与消失点方向互补，应对纹理缺失与镜面反射
◆ 设计高...[摘要不完整，待更新]</td></tr>
<tr><td>2026-07-07</td><td>Amplitude-Independent Robust Snapshot 6-D Radio SLAM via a Uniffed Angle-Delay Formulation<br><a href='http://arxiv.org/pdf/2607.04847'>论文</a></td><td>本文针对双基站快照式无线电SLAM问题，提出了一种不依赖路径幅度的鲁棒方法，可在单次多径信道快照中同时定位用户设备(UE)的6-D位姿与时钟偏差，并重构环境散射点。现有方法多依赖路径幅度或路径损耗进行视距/非视距判别，易受校准误差和传播模型失配影响，本文通过统一的角度-延迟公式将视距与单次反射非视距路径纳入同一几何框架，从根本上摆脱对幅度的依赖。粗估计阶段直接从角度-延迟测量中选择几何一致的内部点，避免幅度预分类和路径级潜变量；进一步通过扭转-摆动两阶段遍历初始化和SO(3)上的局部细化，将方法推广到完整的3-D/6-D位姿估计。

◆ 提出不依赖路径幅度的鲁棒快照式6-D无线电SLAM方法，基于统一的角度-延迟公式处理视距与单次反射非视距路径，增强对校准误差和传播模型失配的鲁棒性
◆ 粗估计阶段直接从几何一致性出发选择内点，无需幅度预分类或路径级潜变量；通过扭转-摆动两阶段初始化与SO(3)局部细化推广至6-D位姿
◆ 提出雅可比行均衡的迭代重加权最小二乘细化结合准赤池信息量准则模型比较，实现视距路径检测与UE状态、散射点的联合优化
◆ 分析了公式特有的局部秩性质及其在未知路径身份下的最小集合理论依据...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-05</td><td>Real-Time LiDAR Gaussian Splatting SLAM<br><a href='http://arxiv.org/pdf/2607.04127'>论文</a></td><td>本文提出了一种基于LiDAR的实时高斯泼溅SLAM框架，在Newer College数据集上以纯在线轨迹达到86.78% F-score和超过20 FPS的实时性能，并通过多数据集实验验证了其稳定性与可扩展性。

◆紧耦合G-ICP配准与球面光栅化稠密建图，实现大规模序列的实时稠密建图
◆利用LiDAR几何特性而非外观信息，将跟踪阶段估计的局部协方差复用为高斯的距离感知初始尺度，并据此推导表面法向量以支持几何感知的地图优化
◆提出协方差驱动的几何评分机制，根据局部结构复杂度在平面区域执行剪枝、在结构丰富区域选择性加密，提升地图表达效率
◆将优化后的高斯与LiDAR特有的置信度线索反馈至跟踪模块，形成跟踪与建图的闭环耦合，增强跟踪鲁棒性...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-04</td><td>PRISM3D: Probabilistic Refinement and Robust Initialization for Physically Consistent Scene Modeling under Extreme Motion Blur<br><a href='http://arxiv.org/pdf/2607.03855'>论文</a></td><td>本文针对极端运动模糊图像下的盲3D场景重建问题,提出了统一框架PRISM3D,解决了传统SfM在严重退化输入上失效的难题。核心贡献包括:利用深度稠密跟踪方法(VGGSfM)实现鲁棒初始化,在特征匹配失败的场景中恢复全局拓扑,是首个将该范式成功用于从极端运动模糊引导3D高斯溅射的工作。针对初始化稀疏且带噪导致确定性优化发散的问题,提出概率与物理耦合的解决方案,采用MCMC进行几何稠密化,同时用连续Bezier轨迹建模物理图像形成过程。此外,论文还扩展了多模态版本PRISM3D-E,融合事件相机的高时间分辨率信号作为结构先验,并贡献了首个配对事件流基准数据集PRISM3D-E Benchmark。大量实验表明,纯RGB框架及其多模态扩展均取得了当前最优性能。

◆ 基于VGGSfM的鲁棒初始化策略,首次实现从极端运动模糊直接引导3D高斯溅射的拓扑恢复
◆ 采用MCMC概率稠密化方法,对稀疏带噪先验进行鲁棒的几何填充
◆ 利用连续Bezier轨迹建模物理图像形成过程,引入物理一致性约束
◆ 提出多模态扩展PRISM3D-E,融合RGB与事件流以提升重建保真度
◆ 发布PRISM3D-E Benchmark数据集,填补该领域缺乏配对事件流基准的空白...[摘要不完整，待更新]</td></tr>
...
<thead><tr><th>日期</th><th>标题</th><th>摘要</th></tr></thead>
<tbody>
<tr><td>2026-07-06</td><td>Dynamic Airspace Management for UAVs in Evolving Urban Environments: Collaborative Coordination and Human Safety<br><a href='http://arxiv.org/pdf/2607.04825'>论文</a> | <a href='https://github.com/pharos-anonymized/source-code.git'>代码</a></td><td>该论文针对城市低空经济中无人机空域管理的安全问题,提出了Pharos系统,旨在复杂城市环境中实现多无人机的协同空域管理与人类安全保护。Pharos定位于分布式局部感知与集中式精细控制之间,既保证了扩展性,又实现了共享空域中的安全并行调度。研究创新性地将人类恐惧感纳入决策考量,这是以往空域管理系统中较少涉及的人文因素,体现了以人为中心的设计理念。系统基于MAPPO多智能体强化学习算法实现,相较HAPPO和HATRPO具有更快的收敛速度和更高的奖励值。论文还设计了空间熵作为系统评估指标,以量化空域利用效率。仿真结果表明,Pharos在人类恐惧降低方面较Ipopt基准减少了52.72%,空间利用率较Ipopt和A-star分别提升70.82%和2.03%,验证了其有效性与优越性。

◆首次将人类恐惧感作为核心因素纳入无人机空域管理决策框架,体现人本安全理念

◆提出Pharos协同管理系统,平衡分布式感知与集中式控制两种范式

◆基于MAPPO算法实现多无人机协同调度,收敛速度与奖励均优于同类MARL算法

◆创新设计空间熵评估指标,量化空域利用效率,弥补传统评价方法不足...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-02</td><td>RCOA Extension and Applications<br><a href='http://arxiv.org/pdf/2607.02797'>论文</a></td><td>本文对松弛凸障碍物规避(RCOA)方法进行了扩展与应用研究。RCOA是首个实现完全凸优化的障碍物规避控制方法,其收敛分析表明即便障碍物超出预测时域,仍能保持有效的规避能力。本文将RCOA扩展至三维环境并应用于无人机导航,同时引入车辆几何形状建模,实现了三维物体间的碰撞检测而不再局限于质点模型。

◆ 将RCOA从二维扩展到三维环境,并成功应用于无人机(UAV)导航任务

◆ 引入车辆几何形状建模,实现三维物体间的精确碰撞规避,突破传统质点假设限制

◆ 证明RCOA在障碍物超出预测时域时仍保持规避有效性,具有独特的外推能力

◆ 数值仿真表明计算性能达到或超越现有最优方法,实时频率超过30Hz

◆ 通过缩短预测时域使NMPC能够执行穿越窄通道的激进机动,确保实时可行性...[摘要不完整，待更新]</td></tr>
<tr><td>2026-07-01</td><td>Neuro-Symbolic Safety Guidance for Vision-Language-Action Models via Constrained Flow Matching<br><a href='http://arxiv.org/pdf/2607.01378'>论文</a></td><td>本文针对视觉-语言-动作(VLA)模型在真实部署中缺乏有效安全保障的问题,提出了一种基于约束流匹配的神经-符号安全引导机制。该方法将安全约束建模为最小范数约束优化问题,在流匹配去噪过程的中间轨迹预测阶段进行符号化约束校正,实现了对碰撞的预测性规避而非被动反应式干预。通过将神经轨迹生成与符号约束满足交替进行,系统能够在碰撞不可避免之前提前检测并修正危险轨迹。在SafeLIBERO基准上的实验表明,该方法取得了82.8%的避碰率和81.6%的任务成功率,相比单步方法分别提升了6.3%和19.8%,在长时序任务中因复合分布漂移问题而获益最为显著。

◆ 提出基于流匹配的VLA模型的神经-符号安全引导框架,实现预测性碰撞规避
◆ 将安全约束形式化为最小范数约束优化,在去噪过程中迭代校正中间轨迹
◆ 神经轨迹生成与符号约束满足交替执行,具备前瞻性安全保障能力
◆ 在长时序任务上取得显著性能提升,有效缓解复合分布漂移问题...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-01</td><td>Robots Ask the Way: Communication-Enabled Social Navigation<br><a href='http://arxiv.org/pdf/2607.01044'>论文</a></td><td>当前社交导航方法侧重于被动避障和轨迹调整，缺少主动通过人机交流获取信息的机制。本文提出通信使能的社交导航任务CommNav，让机器人主动向周围居民询问目标人物的位置、动向等线索，以提升多人物环境下的定位效率。为此，作者将Habitat 3.0扩展为支持多智能体通信的Habitat 3.0c仿真环境。实验显示，在先进社交导航模型中加入通信模块COMM可使Episode Success提升10个百分点，且该策略对自然口语化指令具有接近完美结构化数据的鲁棒性。

◆ 提出CommNav新任务范式，机器人主动通过人机对话收集导航线索
◆ 构建Habitat 3.0c仿真平台，引入多人物通信协议支持信息交换
◆ 设计COMM通信模块及预训练策略，缓解稀疏交互信号带来的训练难题
◆ 验证导航策略对LLM生成指令和人类口语化指令均保持强鲁棒性...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-01</td><td>BIFROST: Bridging Invariant Feature Representation for Observation-space Sim2Real Transfer<br><a href='http://arxiv.org/pdf/2607.01410'>论文</a></td><td>BIFROST针对机器人策略学习中仿真到现实(sim2real)迁移的难题，提出了一种基于观察空间共享结构挖掘的零样本迁移框架。现有方法通常孤立处理视觉或动力学差异，而BIFROST通过跨域双模拟(cross-domain bisimulation)目标，在配对的跨域数据上学习共享的历史编码器，使产生等价长期行为轨迹的观察-动作序列在潜在空间中彼此接近，而与具体域无关。基于该编码器在仿真中训练的策略可直接零样本迁移到真实环境。实验验证在视觉导航sim2sim以及接触丰富的操作和视觉伺服sim2real任务中，BIFROST在视觉与动力学差距同时存在时仍能有效迁移，优于领域自适应和协同训练基线。

◆ 提出观察空间层面的sim2real迁移视角，直接从原始观察中挖掘仿真与现实的共享结构
◆ 设计跨域双模拟学习目标，将跨域等价轨迹映射到邻近潜在状态
◆ 学习统一的共享历史编码器，在仿真中训练即可零样本迁移到真实环境
◆ 在视觉与动力学差距同时存在时仍保持有效迁移，超越现有基线方法...[摘要不完整，待更新]</td></tr>
<tr><td>2026-07-01</td><td>Path Planning in Physically Viable World Models<br><a href='http://arxiv.org/pdf/2607.00673'>论文</a></td><td>该论文针对机器人在非结构化户外环境中使用过时地图进行长时规划的问题，提出了一种物理可行世界模型，使机器人能够在部署前评估未来地形变化对路径可行性的影响。系统将3D高斯溅射重建场景与基于物理的仿真相结合，无需重新采集数据或重建地图即可生成物理修改后的环境版本，用于&quot;假设性&quot;查询分析。论文实现了一个地形感知规划器，能够考虑物理仿真中的事件、障碍和形变，帮助操作人员判断在条件恶化时计划路径是否仍然安全可行。在德州中部真实户外场地的洪水仿真实验表明，物理可行世界模型能够揭示原始重建环境规划中无法发现的长期路径失败与重规划行为，从而在部署前评估未来地形变化对路径可行性的潜在影响。

◆ 提出融合3D高斯溅射重建与物理仿真的世界模型，无需重新采集数据即可生成地形变化后的环境
◆ 支持&quot;假设性&quot;查询，使机器人与操作员能在部署前评估未来地形变化对路径可行性的影响
◆ 设计地形感知规划器，集成物理事件、障碍和形变因素，提升受限环境下的安全决策能力
◆ 通过多级洪水仿真实验，揭示传统规划在长时序路径失败和重规划方面的盲点...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-07</td><td>MP-MPPI: A Motion Primitive Guided Sampling-Based Optimizer for Model Predictive Control<br><a href='http://arxiv.org/pdf/2607.06123'>论文</a></td><td>本文提出MP-MPPI方法，将运动基元引入到模型预测路径积分（MPPI）框架中，通过结构化采样增强算法向全局最优解的收敛能力。该方法在实时采样优化循环中同步评估运动基元与扰动控制序列，并在四旋翼飞行器障碍物场导航任务中得到了验证，有效解决了传统采样控制器路径规划能力不足的问题。

◆ 在标准MPPI基础上引入运动基元进行结构化采样，提升全局最优解搜索能力
◆ 将运动基元评估与扰动控制序列融合于统一的实时采样优化框架
◆ 弥补了采样式控制器在路径规划方面的固有局限
◆ 增强控制空间探索能力的同时保持实时快速反应性...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-05</td><td>Integrated Graph Search and Model Predictive Control for Smooth and Efficient Path Planning in Autonomous Vehicles<br><a href='http://arxiv.org/pdf/2607.04259'>论文</a></td><td>本文提出了一种融合图搜索与模型预测控制(MPC)的序列式路径规划框架，用于自动驾驶车辆的平滑高效路径生成。核心思路是先通过Dijkstra算法在离散网格上获得粗略路径，再以此构建空间变化的凸侧向安全走廊，将离散的避障决策转化为连续优化的可行性约束。在该走廊内构建MPC问题，通过惩罚侧向偏移的三阶空间导数(即急动度)来保证路径平滑性，并提升优化效率。在CarMaker高保真仿真平台上针对直道与弯道超车场景进行验证，结果表明该方法在侧向加速度、曲率和急动度等指标上均优于基于多项式拟合与二次规划的方法，同时计算成本在直道降低28.08%、弯道降低29.52%。

创新点：
◆ 将图搜索结果显式融入MPC框架，利用粗略路径引导优化方向
◆ 构建空间变化凸侧向安全走廊，将离散避障决策转化为连续可行性约束
◆ 采用三阶空间导数(急动度)惩罚项，提升路径平滑性与乘坐舒适性
◆ 在直道与弯道多车超车场景中实现计算效率与路径质量的双重提升...[摘要不完整，待更新]</td></tr>
...
<thead><tr><th>日期</th><th>标题</th><th>摘要</th></tr></thead>
<tbody>
<tr><td>2026-07-06</td><td>From Fixed to Free Cameras: Calibration-Free View-Robust Vision-Language-Action Model<br><a href='http://arxiv.org/pdf/2607.05396'>论文</a> | <a href='https://alibaba-damo-academy.github.io/CamVLA/'>代码</a></td><td>这篇论文针对真实机器人部署中相机位姿频繁变化的问题，提出了Camera-Centric VLA (CamVLA) 模型，旨在摆脱对相机外参标定的依赖。核心思想是将操作控制与相机几何解耦，模型并行预测两个输出：相机坐标系下的末端执行器动作，以及相机到机器人基座的6自由度手眼变换矩阵。这两个预测通过确定性几何变换合成最终的基坐标系动作，从而将&quot;如何运动&quot;与&quot;从何处观察&quot;两个子问题清晰分离。部署时该策略仅需单目RGB图像和任务指令，无需标定、无需深度信息，并支持单视角输入。在仿真和真实机器人实验中，CamVLA在多种未见视角下均显著提升了任务成功率，展现了优异的视角鲁棒性。

创新点：
◆ 提出CamVLA框架，将操作控制与相机几何解耦，部署时无需相机外参标定
◆ 同时预测相机中心末端动作与6自由度手眼矩阵，通过确定性几何变换合成最终控制信号
◆ 实现calibration-free、depth-free、single-view的极简部署范式，仅依赖单目RGB和任务指令
◆ 在仿真和真实机器人多种未见视角下均显著提升任务成功率，验证了视角鲁棒性...[摘要不完整，待更新]</td></tr>
<tr><td>2026-07-02</td><td>Bridging 3D Gaussians and Semantic Occupancy for Comprehensive Open-Vocabulary Scene Understanding from Unposed Images<br><a href='http://arxiv.org/pdf/2607.01633'>论文</a></td><td>本文提出COVScene框架，旨在从无位姿稀疏图像中实现全面的3D场景理解，涵盖可渲染几何、开放词汇语义与自由/占据空间建模。该方法将可渲染高斯原语与稠密语义占据场通过可微分体积提升进行耦合，解决了现有前馈高斯方法在未观测区域约束不足的问题。其核心在于将预测的语义高斯在训练计算图内进行体积提升，使体素正则化能够为高斯不透明度、几何形状与语义特征提供梯度，从而在统一表征下协同优化。实验表明，该方法在ScanNet和ScanNet++上保持了竞争力的渲染质量，并显著提升了开放词汇分割与语义占据预测能力，且无需直接的体素级监督。

◆ 提出可微分体积提升机制，在训练计算图内将高斯与占据场耦合，实现跨表征梯度传播
◆ 设计语义感知几何Transformer与多任务高斯解码，融合几何基础蒸馏与占据熵正则化
◆ 在单一高斯表征下统一支持新视角合成、开放词汇语义查询与语义占据预测任务
◆ 在无位姿输入条件下超越自监督基线的占据预测性能，同时保持渲染质量不退化...[摘要不完整，待更新]</td></tr>
...
<thead><tr><th>日期</th><th>标题</th><th>摘要</th></tr></thead>
<tbody>
<tr><td>2026-07-07</td><td>Why does Deep Learning Improve Visual SLAM?<br><a href='http://arxiv.org/pdf/2607.06023'>论文</a></td><td>本文针对深度学习视觉SLAM系统显著优于传统几何方法的现状,提出了一个根本性问题:性能提升究竟源自学习式二维数据关联、不确定性建模还是循环架构。作者通过受控实验设计,系统性地拆解并对比了这三个组件的独立贡献。研究发现,深度学习SLAM成功的关键在于学习式二维数据关联与不确定性建模,循环架构本身贡献有限。该结论为DL-SLAM系统设计提供了明确指导,凸显了学习化范式在核心组件中的必要性。代码将开源以便社区复现与进一步研究。

◆ 提出控制变量式的消融实验框架,定量解耦数据关联、不确定性建模与循环架构三个关键组件
◆ 实证揭示学习式二维数据关联与不确定性建模是性能提升的核心驱动因素
◆ 明确指出循环架构并非DL-SLAM成功的关键因素,为未来系统设计指明方向...[摘要不完整，待更新]</td></tr>
...
<tbody>
<tr><td><a href='https://github.com/hku-mars/FAST_LIO'>FAST_LIO</a></td><td>4903</td><td>A computationally efficient and robust LiDAR-inert</td></tr>
