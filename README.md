...
<tbody>
<tr><td>2026-07-14</td><td>Breaking Déjà Vu: Independent Auditing of Visual Place Recognition through Vision-Language Reasoning<br><a href='http://arxiv.org/pdf/2607.12818'>论文</a></td><td>本文针对视觉位置识别（VPR）在实际部署中依赖固定阈值、难以适应环境变化的核心痛点，提出了一种基于视觉语言模型（VLM）的后检索独立审计框架。
...
<tr><td>2026-07-14</td><td>DiffRadar: Differentiable Physics-Aware Radar SLAM with Gaussian Fields<br><a href='http://arxiv.org/pdf/2607.12265'>论文</a></td><td>现有雷达SLAM系统通过对离散化热图进行扫描匹配来估计运动，破坏了几何连续性，难以捕捉雷达感知特性，在退化或动态环境下位姿估计与建图质量不稳定。本文提出DiffRadar，一个实时雷达SLAM系统，将场景建模为各向异性Gaussian基元。该方法通过可微分、物理感知的雷达前向模型在距离-方位与多普勒-方位空间渲染雷达测量，实现机器人位姿与场景结构的端到端联合优化。作者在Radarize公共基准和针对走廊退化、运动模式转换、动态杂波及长距离回环等常见失效模式的压力测试套件上进行了系统评估。实验表明DiffRadar显著降低了轨迹误差，地图一致性提升超过一倍，并保持70 FPS的实时运行性能。

◆ 将雷达观测建模为可微分、物理感知的Gaussian场而非离散扫描，保留了几何连续性与物理一致性
◆ 用各向异性Gaussian基元表示场景，通过可微前向模型在距离-方位与多普勒-方位空间渲染雷达测量
◆ 在信号域实现机器人位姿与场景结构的端到端联合优化，直接从原始雷达测量反演运动与建图...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-13</td><td>GeoGS-SLAM: Online Monocular Reconstruction Using Gaussian Splatting with Geometric Priors<br><a href='http://arxiv.org/pdf/2607.11184'>论文</a> | <a href='https://rlgao.github.io/geogs_slam'>代码</a></td><td>GeoGS-SLAM是一种在线单目密集重建系统，创新性地将3D高斯溅射(3DGS)地图表示与前馈模型学习到的几何先验相融合，旨在解决现有3DGS-SLAM依赖外部深度传感器以及前馈方法丢弃RGB信息导致重建质量下降的问题。该方法利用前馈视觉几何模型从无标定RGB输入中预测相机位姿和场景几何先验，并从RGB与几何先验中直接采样高斯基元来构建和扩展高斯场景地图。系统通过从粗到精的联合优化策略同步最小化光度损失和几何损失，并结合在线回环检测与位姿图优化保证全局一致性。大量室内外基准实验表明，GeoGS-SLAM在保持在线实时性能的同时，取得了优于当前最先进方法的渲染质量和跟踪精度。

核心创新点：
◆ 提出将3DGS地图表示与前馈几何先验融合的在线单目SLAM统一框架
◆ 设计从RGB输入与几何先验中直接联合采样高斯基元的地图构建方法
◆ 引入粗到精策略同步优化光度与几何损失，提升重建质量
◆ 集成在线回环检测和位姿图优化，实现全局一致的稠密重建...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-14</td><td>Improving Autonomous Nano-drones Performance via Automated End-to-End Optimization and Deployment of DNNs<br><a href='http://arxiv.org/pdf/2607.12593'>论文</a></td><td>本文针对纳米级无人机(尺寸小于10厘米)部署CNN导航算法的难题,提出了一套从训练到闭环飞行评估的端到端自动化优化与部署流程,目标平台为搭载超低功耗多核SoC的Crazyflie 2.1。研究以PULP-Dronet视觉导航网络为对象,在保持预测精度的同时将其内存占用压缩为原来的二分之一,推理速度提升1.6倍,而计算功耗仅占整机预算的1.6%以下。实际飞行表现显著优于原始手工设计的基线,实现了峰值1.65 m/s的障碍物刹停、最高1.96 m/s的熟悉环境自由飞行,以及90度弯道的车道保持能力。论文同时开源了全部软硬件设计,提供即用型项目,便于复现和后续研究。

◆提出面向纳米无人机的端到端自动化CNN训练、量化与部署工具链,显著降低了视觉算法开发复杂度
◆在精度保持不变的前提下,实现PULP-Dronet内存占用减半、推理加速1.6倍
◆在真实闭环飞行任务中全面超越手工基线,涵盖高速避障、自由飞行与弯道跟随
◆开源完整软硬件设计,推动纳米无人机自主导航领域的可复现研究...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-13</td><td>More than a Manipulator: Planning Propellant-Free Attitude Maneuvers for Free-Floating Spacecraft<br><a href='http://arxiv.org/pdf/2607.12130'>论文</a></td><td>这篇论文研究利用机械臂运动实现无推进剂卫星姿态控制,挑战了机械臂仅作为扰动源的传统观点。论文将关节极限和避碰等关键约束纳入轨迹优化问题,采用内点法求解非线性规划,针对多种运动学和惯量构型的航天器-机械臂系统成功生成复杂的旋转和消旋轨迹。论文通过动量与力矩包络将机械臂的控制能力与反作用轮阵列直接对比,证明机械臂具备作为冗余乃至主用姿态控制执行机构的潜力。

◆ 提出将机械臂运动作为无推进剂姿态控制执行机构的新思路,改变其仅作为扰动源的定位
◆ 构建包含关节极限与避碰约束的轨迹优化框架,采用内点法求解实现复杂机动
◆ 通过动量与力矩包络定量比较机械臂与反作用轮的控制能力
◆ 面向在轨装配与制造等高相对质量载荷场景,拓展机械臂作为多用途姿态控制执行器的应用前景...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-10</td><td>SEAMLiS: Visibility-Aware Safety for Perception-Limited Multi-Robot Exploration<br><a href='http://arxiv.org/pdf/2607.09959'>论文</a></td><td>本文针对有限感知范围和视场受限下自主多机器人探索中分离规划与安全控制导致碰撞风险的问题,提出了SEAMLiS安全执行层框架。该框架保留上游探索栈(目标分配器与局部规划器),通过感知感知的姿态和位置滤波器在执行层强制安全保障。其中门控姿态滤波器在可见性促进偏航策略与速度跟踪备份策略间切换,确保对已知自由/未知边界的可见性并保持足够制动余量;控制屏障函数(CBF)位置滤波器则规避已知、新检测障碍物及其他机器人。论文给出了充分避碰条件,并在随机仿真、Isaac Sim和Crazyflie硬件实验中验证了多机器人无碰撞探索。

◆ 提出模块化执行层安全框架SEAMLiS,在不修改上游规划器的前提下为有限感知探索提供安全保障
◆ 设计门控偏航姿态滤波器,根据可见性与制动余量动态切换信息驱动与速度跟踪策略
◆ 利用CBF位置滤波器统一处理已知障碍、新检测障碍及机器人间避碰
◆ 给出充分避碰条件并在仿真与Crazyflie硬件上完成验证...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-14</td><td>Unveiling Complex Collective Behaviors from Simple Rewards<br><a href='http://arxiv.org/pdf/2607.12861'>论文</a></td><td>本文针对多智能体强化学习(MARL)中神经策略黑箱性导致的可解释性难题,揭示了简单奖励下涌现复杂群体行为的隐藏机制。核心贡献在于提出了一种两阶段的EEC解释框架,并设计了名为Agent Response Map(ARM)的创新分析工具,能够可视化智能体在空间中的决策模式,识别聚集与回避区域。研究发现机器人隐式地学习环境的几何场,并将其作为协调运动的目标结构。该方法在合作性多机器人形状装配和竞争性追逃博弈两类任务中得到验证:合作任务中,ARM识别出未占据的目标内部区域,并在中心被占据后自动转向边界;竞争任务中,猎物会收敛于捕食者Voronoi图的边界。综上,ARM能够有效发现MARL策略背后的隐藏几何结构。

◆ 提出两阶段EEC解释框架,为MARL策略分析提供系统化方法

◆ 设计Agent Response Map(ARM)这一创新分析工具,可视化智能体空间决策模式

◆ 揭示机器人隐式学习环境几何场并将其作为协调运动目标的机制

◆ 验证ARM在合作与竞争两类任务中发现隐藏几何结构的通用能力...[摘要不完整，待更新]</td></tr>
<tr><td>2026-07-14</td><td>Breaking Déjà Vu: Independent Auditing of Visual Place Recognition through Vision-Language Reasoning<br><a href='http://arxiv.org/pdf/2607.12818'>论文</a></td><td>本文针对视觉位置识别(VPR)系统依赖固定阈值、在环境变化下可靠性不足的痛点，提出了一种独立的后检索审计框架。核心思想是利用视觉语言模型(VLM)对查询图像和候选图像进行联合语义推理，从而判断检索结果是否真正匹配。该方法不依赖架构特定的置信度度量、数据集相关的阈值或部署环境的先验知识，因此能够适应未见过的环境。在6个基准数据集、5种VPR方法和4种VLM上的实验表明，该方法将recall@1平均提升13.6%，同时将误接受率降至12%，并保持精度高于95%、覆盖率超过75%。

◆ 提出VPR审计(Visual Place Recognition Auditing)框架，作为独立于检索模型的事后验证机制
◆ 利用VLM的视觉-语言联合推理能力，实现无需人工标注的实例级匹配验证
◆ 摆脱对固定阈值和置信度度量的依赖，提升系统对环境变化的鲁棒性...[摘要不完整，待更新]</td></tr>
<tr><td>2026-07-14</td><td>PixelLoop: Shortcut Topological Navigation with Pixel-Level Loops<br><a href='http://arxiv.org/pdf/2607.12811'>论文</a> | <a href='https://pixelloop-nav.github.io/'>代码</a></td><td>本文针对纯拓扑表示中回环检测的作用被忽视这一问题，提出了PixelLoop方法，在像素级稠密拓扑结构中直接引入回环闭合。传统SLAM中的回环主要用于对齐坐标系或图像级稀疏边，而本文将像素级回环作为稠密的拓扑捷径，直接改变路径规划的连通性和代价传播方式。这种稠密连通性使机器人能够实现稳定的任意点到任意点导航，并生成与几何最短路径高度吻合的代价地图。作者特别强调了将回环应用于细粒度像素拓扑而非图像级拓扑的独特优势。

◆ 在像素空间中直接引入回环闭合，构建稠密拓扑捷径，从根本上改变规划连通性而非仅仅对齐坐标
◆ 突破传统图像级拓扑的稀疏性限制，实现任意点到任意点的稳定导航，代价图与几何最短路径精确对齐
◆ 在仿真实验中，相比图像级相对基线方法，在成功率和SPL上均取得超过35%的绝对提升，捷径利用场景增益最显著
◆ 通过真实移动机器人部署验证了方法的实用性和鲁棒性，证明像素级稠密回环是拓扑视觉导航的可靠基础...[摘要不完整，待更新]</td></tr>
<tr><td>2026-07-14</td><td>Improving Autonomous Nano-drones Performance via Automated End-to-End Optimization and Deployment of DNNs<br><a href='http://arxiv.org/pdf/2607.12593'>论文</a></td><td>本文针对亚10厘米级纳米无人机上视觉CNN部署的挑战，提出了一套端到端自动化工具链，涵盖从模型训练到Crazyflie 2.1平台上ULP多核SoC部署的完整流程。实验以PULP-Dronet导航CNN为基础，在保证预测精度的前提下实现了2倍内存占用缩减和1.6倍推理加速，同时仅消耗无人机不到1.6%的功率预算。在闭环飞行测试中，改进后的系统在障碍物规避、自由飞行和车道跟随等任务上均显著超越手工设计的基线方案。

◆ 提出了完整的自动化端到端优化部署流程，将传统易错且耗时的人工迭代开发转变为可重复的自动化工具链
◆ 对PULP-Dronet CNN进行深度优化，在保持精度的同时实现2倍内存压缩与1.6倍推理加速，且功耗极低
◆ 在真实飞行场景中实现突破性性能：峰值制动速度1.65 m/s、自由飞行速度从0.5 m/s提升至1.96 m/s，并能完成90度转弯车道跟随
◆ 所有软件设计以即用型项目形式开源，与Crazyflie 2.1平台完全兼容，便于后续研究与新应用开发...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-10</td><td>Validating Virtual Reality for Studying Multimodal Human-Robot Interaction in Socially Aware Robot Navigation<br><a href='http://arxiv.org/pdf/2607.09261'>论文</a></td><td>该论文旨在验证虚拟现实(VR)作为研究社交感知机器人导航中多模态人机交互平台的可靠性。研究采用被试内设计,让21名参与者分别在配备动捕设备的真实实验场地和VR沉浸式环境中与PR2移动操控机器人进行正交交叉和擦肩而过两种典型共导航交互。通过主观评估(感知社交意识、交互舒适度)和客观行为数据(轨迹、头部朝向)的对比分析,发现VR环境与真实场景中参与者对机器人社交导航行为的感知高度一致,且行为响应模式也具有跨场景的相似性。这表明VR能够有效复现真实世界中人机共导航的多模态交互动态,为社交导航及人机交互研究提供了灵活可控的实验平台。

◆ 提出并验证了VR作为研究社交感知机器人导航中多模态人机交互的有效平台
◆ 采用被试内实验设计,在同一批参与者身上对比真实环境与VR虚拟环境下的交互行为
◆ 同时从主观感知和客观行为(轨迹、头部朝向)两个维度系统评估VR的生态效度
◆ 覆盖正交交叉和擦肩而过两种典型共导航场景,增强了结论的普适性...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-14</td><td>Model-Based Diffusion Optimal Control for Multi-Robot Motion Planning<br><a href='http://arxiv.org/pdf/2607.12423'>论文</a></td><td>该论文针对连续环境下多机器人运动规划的难题，提出了一种基于模型的扩散最优控制方法MDOC。现有扩散模型方法通常依赖大量演示数据学习得分函数，且难以严格保证动力学可行性和硬安全约束。MDOC将轨迹规划重构为模型驱动的扩散采样问题，结合已知动力学模型与控制屏障函数（CBF）约束投影，在采样过程中自然地实现安全保证。该方法通过冲突基搜索（CBS）自然扩展到多机器人协同规划场景。实验结果表明，MDOC在采样效率、轨迹平滑度、成功率和计算时间上均显著优于现有代表性基线方法，能生成完全无碰撞的动力学可行轨迹。

创新点：
◆ 提出无需演示数据即可生成动力学可行轨迹的MDOC模型驱动扩散规划框架
◆ 将控制屏障函数约束投影融入扩散采样过程，严格保证硬安全约束
◆ 通过冲突基搜索实现从单机器人到多机器人规划的自然扩展与高效协同...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-13</td><td>CR-Solver: GPU-Accelerated Kinematics Solver for Tendon-driven Continuum Robots<br><a href='http://arxiv.org/pdf/2607.11340'>论文</a></td><td>本文提出CR-Solver，一种用于肌腱驱动连续体机器人的两阶段优化运动求解器，将逆运动学、路径跟踪和轨迹规划统一到单一约束非线性优化框架中。该方法利用GPU并行加速，显著提升求解速度，在三个任务上验证获得毫米级精度和95%以上的成功率，并以纯Python实现，降低了使用门槛。

◆提出统一约束非线性优化框架，将逆运动学、路径跟踪与轨迹规划融合为单一求解流程

◆利用GPU并行计算大幅加速优化过程，相对传统CPU求解器获得显著速度提升

◆采用纯Python实现，兼顾高性能与易用性，降低连续体机器人运动规划的采用门槛...[摘要不完整，待更新]</td></tr>
...
<tr><td>2026-07-13</td><td>Diffusion MRI preprocessing affects ADC estimation and automatic PI-RADS v2.1 classification in bi-parametric prostate MRI<br><a href='http://arxiv.org/pdf/2607.11385'>论文</a></td><td>本论文系统研究了扩散加权成像(DWI)预处理策略对前列腺MRI中ADC估计和PI-RADS自动分类的影响。基于fastMRI前列腺队列的268例数据,作者依次施加去噪、吉布斯环形伪影校正和微分同胚配准进行敏感性畸变校正,发现不同预处理流水线对ADC值存在统计学显著差异,畸变校正使DWI重新对齐到T2加权解剖结构并显著改变ADC数值(相关系数约0.90)。研究还证实LLS与迭代加权LLS生成的ADC图数值等价,3类DenseNet分类器在完整预处理数据集上对高风险PI-RADS类别获得最佳AUROC和敏感度,且误诊分析显示其过度自信错误预测最少,具有重要的临床筛查价值。

◆ 系统比较多种DWI预处理组合(去噪、吉布斯校正、畸变校正)对ADC定量分析与深度学习分类的联合影响
◆ 证实LLS与迭代加权LLS在ADC估计中数值等价,为临床选择简化算法提供依据
◆ 揭示畸变校正是提升ADC图质量与高风险PI-RADS自动分类性能的关键步骤,支持前列腺MRI中标准化DWI预处理流程的建立...[摘要不完整，待更新]</td></tr>
...
<tbody>
<tr><td><a href='https://github.com/hku-mars/FAST_LIO'>FAST_LIO</a></td><td>4943</td><td>A computationally efficient and robust LiDAR-inert</td></tr>
<tr><td><a href='https://github.com/hku-mars/FAST-LIVO2'>FAST-LIVO2</a></td><td>4354</td><td>FAST-LIVO2: Fast, Direct LiDAR-Inertial-Visual Odo</td></tr>
<tr><td><a href='https://github.com/hku-mars/r3live'>r3live</a></td><td>2428</td><td>A Robust, Real-time, RGB-colored, LiDAR-Inertial-V</td></tr>
...
<tr><td><a href='https://github.com/hku-mars/LiDAR_IMU_Init'>LiDAR_IMU_Init</a></td><td>1448</td><td>[IROS2022] Robust Real-time LiDAR-inertial Initial</td></tr>
<tr><td><a href='https://github.com/hku-mars/Point-LIO'>Point-LIO</a></td><td>1282</td><td>Point-LIO</td></tr>
...
<tr><td><a href='https://github.com/hku-mars/SUPER'>SUPER</a></td><td>963</td><td>SUPER</td></tr>
...
<tr><td><a href='https://github.com/hku-mars/Voxel-SLAM'>Voxel-SLAM</a></td><td>643</td><td>Voxel-SLAM</td></tr>
...
<tr><td><a href='https://github.com/hku-mars/HBA'>HBA</a></td><td>596</td><td>[RAL 2023] A globally consistent LiDAR map optimiz</td></tr>
<tr><td><a href='https://github.com/hku-mars/ROG-Map'>ROG-Map</a></td><td>580</td><td>ROG-Map</td></tr>
...
<tr><td><a href='https://github.com/hku-mars/GS-SDF'>GS-SDF</a></td><td>513</td><td>[IROS 2025] LiDAR-Augmented Gaussian Splatting and</td></tr>
...
<tr><td><a href='https://github.com/hku-mars/BDM'>BDM</a></td><td>174</td><td>Memory-Efficient Boundary Map for Large-Scale Occu</td></tr>
...
<thead><tr><th>项目</th><th>Stars</th><th>简介</th></tr></thead>
<tbody>
<tr><td><a href='https://github.com/ethz-asl/maplab'>maplab</a></td><td>2850</td><td>A Modular and Multi-Modal Mapping Framework</td></tr>
<tr><td><a href='https://github.com/ethz-asl/voxblox'>voxblox</a></td><td>1653</td><td>A library for flexible voxel-based mapping, mainly</td></tr>
<tr><td><a href='https://github.com/ethz-asl/okvis'>okvis</a></td><td>1358</td><td>OKVIS: Open Keyframe-based Visual-Inertial SLAM.</td></tr>
...
<tr><td><a href='https://github.com/ethz-asl/dynablox'>dynablox</a></td><td>591</td><td>Real-time detection of diverse dynamic objects in </td></tr>
...
<tr><td><a href='https://github.com/ethz-asl/COIN-LIO'>COIN-LIO</a></td><td>495</td><td>🪙 COIN-LIO: Complementary Intensity-Augmented LiDA</td></tr>
...
<tr><td><a href='https://github.com/ethz-asl/BIEVR-LIO'>BIEVR-LIO</a></td><td>137</td><td>[RSS 2026] 🦫 BIEVR-LIO: Robust LiDAR-Inertial Odom</td></tr>
